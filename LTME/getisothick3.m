...
for i=1:120
    for j=1:110
        for l=1:360
            k=1;
            while(k>0)
                if (temp(i,j,k,l)>10.0);
                    k=k+1;
                elseif (isnan(temp(i,j,1,l))==1)
                    isodepth3(i,j,l)=NaN;
                    k=0;
                else
                    isodepth3(i,j,l)=(k-1)*10;
                    if (isnan(temp(i,j,k,l))==1)
                        isodepth3(i,j,l)=NaN;
                    end
                    k=0;
...
ncid= netcdf.create('isodepth_3dd.nc','NOCLOBBER');
...
iso2varid = netcdf.defVar(ncid,'isodepth2','double',[xdimid,ydimid,tdimid]);
iso3varid = netcdf.defVar(ncid,'isodepth3','double',[xdimid,ydimid,tdimid]);
iso4varid = netcdf.defVar(ncid,'isodepth4','double',[xdimid,ydimid,tdimid]);
