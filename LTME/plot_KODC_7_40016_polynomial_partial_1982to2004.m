...
[raw_p1 txt_p1]=xlsread('정선해양조사_400line_from1980to2019.xls','정선해양관측정보','');
[raw_p2 txt_p2]=xlsread('정선해양관측정보_400line_2020.xls','정선해양관측정보','');
...
if sum(~isnan(mon_clim_sur_no2_1)) == 0
    disp('nothing to interp');
elseif sum(~isnan(mon_clim_sur_no2_1)) ~= 0
...
if sum(~isnan(mon_clim_bot_no2_1)) == 0
    disp('nothing to interp');
elseif sum(~isnan(mon_clim_bot_no2_1)) ~= 0
...
if sum(~isnan(mon_clim_sur_no3_1)) == 0
    disp('nothing to interp');
elseif sum(~isnan(mon_clim_sur_no3_1)) ~= 0
...
if sum(~isnan(mon_clim_bot_no3_1)) == 0
    disp('nothing to interp');
elseif sum(~isnan(mon_clim_bot_no3_1)) ~= 0
...
if sum(~isnan(mon_clim_sur_po4_1)) == 0
    disp('nothing to interp');
elseif sum(~isnan(mon_clim_sur_po4_1)) ~= 0
...
if sum(~isnan(mon_clim_bot_po4_1)) == 0
    disp('nothing to interp');
elseif sum(~isnan(mon_clim_bot_po4_1)) ~= 0
...
if sum(~isnan(mon_clim_sur_si_1)) == 0
    disp('nothing to interp');
elseif sum(~isnan(mon_clim_sur_si_1)) ~= 0
...
if sum(~isnan(mon_clim_sur_si_2)) == 0
    disp('nothing to interp');
elseif sum(~isnan(mon_clim_sur_si_2)) ~= 0
...
if sum(~isnan(mon_clim_bot_si_1)) == 0
    disp('nothing to interp');
elseif sum(~isnan(mon_clim_bot_si_1)) ~= 0
...
if sum(~isnan(mon_clim_bot_si_2)) == 0
    disp('nothing to interp');
elseif sum(~isnan(mon_clim_bot_si_2)) ~= 0
...
save('kodc_input_fix_1982to2004_3sig(3regime)_40016_v7.mat');
...
part_sur_jan(nandx)
part_sur_mar(nandx)
part_bot_jan(nandx_bot)
part_bot_mar(nandx_bot)

merg_sur=part_sur_feb;
merg_bot=part_bot_feb;
merg_sur(nandx(1:2))=part_sur_jan(nandx(1:2));
merg_bot(nandx_bot(2))=part_bot_jan(nandx_bot(2));
merg_bot(nandx_bot([1,3]))=[34.38, 33.74];
merg_sur(nandx(3:4))=part_sur_mar(nandx(3:4));
merg_bot(nandx_bot(4:5))=part_bot_mar(nandx_bot(4:5));
...
xtickangle(45)
grid on; xlim([1 inf]); title('400-16 NIFS salinity feb')
legend('surf.','bot.');
...
xticklabels(1980:2020);
xtickangle(45)
grid on; xlim([1 inf]); title('400-16 NIFS salinity feb (jan, mar)')
plot(nandx,merg_sur(nandx),'ok','MarkerFaceColor','k');
plot(nandx_bot([2,4,5]),merg_bot(nandx_bot([2,4,5])),'or','MarkerFaceColor','r');
legend('surf.','bot.');
