...
save('yellow_clim_mod_2nd_regime.mat','clim_mod_*','*_r');
...
temp_range=[0:1:31];
salt_range=[0:.5:35]; salt_range_c=[30:.5:35];
% nh4_range=[0:0.5:20];
% no3_range=[0:1:10];
...
u_ref(267,261) = 0.5 * scale_vec;
% u_ref(161,21) = 0.1;

% save('plot_mth_1980_1986.mat');
% close all; clear all; clc;
% load('plot_mth_1980_1986.mat');
cd /data1/cshwa/ext_hdd/yellow/result/clim_pic/2nd_2001-2010_4season
...
m_text(lon(264,258),lat(264,258),'0.5m/s','fontsize',15,'Color','w');
...
% text(lon(264,258),lat(264,258),'0.5m/s','fontsize',16);
% plot_google_map('MapType','terrain','Scale',2,'Resize',2)   % overlay google map
...
figure; hold on;
%     m_contourf(xx,yy,e,20); m_contour(xx,yy,e,20); %% ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿?
title_name = strcat('yellow-',' 2001-2010.',i_season{i}, '  bot.');
