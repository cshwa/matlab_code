...
%make t-axis for yearly (~2010) for seperating regime (TP criteria)
clearvars t_tick
t_tick_pre=sum(eom_d,2);
for i = 1:22
    if i ==1
...
%divide the data
% 1989 ~ 2004
regime_tp_pre = raw_tp(1:t_tick(end));
...
% 2004 ~ 2019
regime_tp_af_pre = raw_tp(t_tick(end)+1:end);
...
% ~2004
clearvars idx1
idx1 = find(isnan(regime_tp_pre) == 0);
regime_tp=regime_tp_pre;
regime_tp(find(regime_tp_pre > mean(regime_tp_pre(idx1)) + sig*std(regime_tp_pre(idx1))))=NaN;
regime_tp(find(regime_tp_pre < mean(regime_tp_pre(idx1)) - sig*std(regime_tp_pre(idx1))))=NaN;
regm_tp = nanmean(regime_tp)
...
% 2004~
clearvars idx1
idx1 = find(isnan(regime_tp_af_pre) == 0);
regime_tp_af=regime_tp_af_pre;
regime_tp_af(find(regime_tp_af_pre > mean(regime_tp_af_pre(idx1)) + sig*std(regime_tp_af_pre(idx1))))=NaN;
regime_tp_af(find(regime_tp_af_pre < mean(regime_tp_af_pre(idx1)) - sig*std(regime_tp_af_pre(idx1))))=NaN;
regm_tp_af = nanmean(regime_tp_af)
...
for i = 1:length(regime_indx)
    if size(regime_indx{i},1) == 0   
        ...
        reg_clim_tp(i) = nanmean(com_tp(regime_indx{i}));
...
%tp
clearvars xp_w_tp pf_w_tp
xp = 1:366;
xp_w_tp = find(isnan(reg_clim_tp)==0);
pf_w_tp = polyfit(xp_w_tp,reg_clim_tp(xp_w_tp),3);
yp_w_tp_04 = polyval(pf_w_tp,xp);
yp_w_tp_04 = yp_w_tp_04 + regm_tp;
...
%% tp
figure;
scatter(1:366,reg_clim_tp + regm_tp);
hold on
plot(1:366, yp_w_tp_04,'--','color','r','linew',2)
xlabel('time(days)','fontsize',13)
ylabel('tp (mg/L)','fontsize',13)
title('sumjin(songjung)-polynomial tp.','fontsize',13)
grid on
set(gca,'fontsize',13)
ylim([0 0.1]) 
xlim([1 366])
...
xlabel('time(days)','fontsize',13)
ylabel('tn (mg/L)','fontsize',13)
title('sumjin(songjung)-polynomial tn.','fontsize',13)
grid on
set(gca,'fontsize',13)
ylim([0 4]) 
xlim([1 366])
...
xlabel('time(days)','fontsize',13)
ylabel('det (mg/L)','fontsize',13)
title('sumjin(songjung)-polynomial det.','fontsize',13)
grid on
set(gca,'fontsize',13)
ylim([0 1.2])
xlim([1 366])
...
title('sumjin(songjung)- TN, NO3+NH4, diff.','fontsize',13)
...
grid on;
xlim([1 length(raw_tn)])
title('sumjin(songjung)- tp raw.','fontsize',13)
legend('tp')
set(gca,'xtick',[t_tick_all(1:3:end)]);
set(gca,'xticklabel',1989:3:2019,'fontsize',10);
set(gca,'fontsize',13)
...
save('sumjin(songjung)_polynomial_climate_to2004_advanced(v4)_3sig_tp.mat'); 
...
%make t-axis for yearly 
t_tick_pre=sum(eom_d,2);
for i = 1:22  %1989~2010 (TP criteria)
    if i ==1
         t_tick(i) = t_tick_pre(i);
    else
        t_tick(i) = sum(t_tick_pre(1:i));
...
%make t-axis for yearly 
tx_tick_pre=sum(eom_d,2);
for i = 1:31  %1989~2010 (TP criteria)
    if i ==1
         tx_tick(i) = tx_tick_pre(i);
    else
        tx_tick(i) = sum(tx_tick_pre(1:i));
    end
end
tx_tick = [1 (tx_tick+1)];
...
% 2004 ~ 2019
regime_tp_af = raw_tp(t_tick(end)+1:end);
...
for i = 1:length(regime_indx)
    if size(regime_indx{i},1) == 0
        ...
        reg_clim_tp(i) = nanmean(regime_tp(regime_indx{i}));
...
for i = 1:length(regime_indx_af)
    if size(regime_indx_af{i},1) == 0  
        ...
        reg_af_clim_tp(i) = nanmean(regime_tp_af(regime_indx_af{i}));
...
%tp
clearvars idx1
idx1 = find(isnan(reg_clim_tp) == 0);
reg_clim_tp_te=reg_clim_tp;
upper_bc_tp(sig) = mean(reg_clim_tp(idx1)) + sig*std(reg_clim_tp(idx1));
lower_bc_tp(sig) = mean(reg_clim_tp(idx1)) - sig*std(reg_clim_tp(idx1));
reg_clim_tp_te(find(reg_clim_tp > mean(reg_clim_tp(idx1)) + sig*std(reg_clim_tp(idx1))))=NaN;
reg_clim_tp_te(find(reg_clim_tp < mean(reg_clim_tp(idx1)) - sig*std(reg_clim_tp(idx1))))=NaN;
nanmean(reg_clim_tp_te(idx1))

clearvars xp_w_tp pf_w_tp
xp = 1:366;
xp_w_tp = find(isnan(reg_clim_tp_te)==0);
pf_w_tp = polyfit(xp_w_tp,reg_clim_tp_te(xp_w_tp),3);
yp_w_tp_04 = polyval(pf_w_tp,xp);

clearvars idx1
idx1 = find(isnan(reg_af_clim_tp) == 0);
reg_af_clim_tp_te=reg_af_clim_tp;
upper_bc_tp_af(sig) = mean(reg_af_clim_tp(idx1)) + sig*std(reg_af_clim_tp(idx1));
lower_bc_tp_af(sig) = mean(reg_af_clim_tp(idx1)) - sig*std(reg_af_clim_tp(idx1));
reg_af_clim_tp_te(find(reg_af_clim_tp > mean(reg_af_clim_tp(idx1)) + sig*std(reg_af_clim_tp(idx1))))=NaN;
reg_af_clim_tp_te(find(reg_af_clim_tp < mean(reg_af_clim_tp(idx1)) - sig*std(reg_af_clim_tp(idx1))))=NaN;
nanmean(reg_af_clim_tp_te(idx1))

clearvars xp_w_tp pf_w_tp
xp = 1:366;
xp_w_tp = find(isnan(reg_af_clim_tp_te)==0);
pf_w_tp = polyfit(xp_w_tp,reg_af_clim_tp_te(xp_w_tp),3);
yp_w_tp_af = polyval(pf_w_tp,xp);
...
%% tp
figure;
scatter(1:366,reg_af_clim_tp_te);
hold on
plot(1:366, yp_w_tp_af,'--','color','r','linew',2)
xlabel('time(days)','fontsize',13)
ylabel('tp (mg/L)','fontsize',13)
title('sumjin(songjung)-polynomial tp.','fontsize',13)
grid on
...
np_ratio=raw_tn./raw_tp;  

figure; hold on;
plot(np_ratio,'.');
yline(16,'color','r','linew',2);
 xlabel('time(year)','fontsize',13)
ylabel('N/P (mg/L)','fontsize',13)
grid on; set(gca,'fontsize',13)
% title(['Namgang tp raw  ' num2str(sig) '-sigma'],'fontsize',13)
ylim([0 inf])
...
% raw
w_tp_04=w_tp(1:t_tick(end));
idx1 = find(isnan(w_tp_04) == 0);
w_tp_042=w_tp_04;
upper_bc(sig) = mean(w_tp_04(idx1)) + sig*std(w_tp_04(idx1));
lower_bc(sig) = mean(w_tp_04(idx1)) - sig*std(w_tp_04(idx1));
mean(w_tp_04(idx1))
w_tp_042(find(w_tp_04 > mean(w_tp_04(idx1)) + sig*std(w_tp_04(idx1))))=NaN;
w_tp_042(find(w_tp_04 < mean(w_tp_04(idx1)) - sig*std(w_tp_04(idx1))))=NaN;
 
%regression
%slope y = b1*x
nonan=w_tp_042(~isnan(w_tp_042));
idx = find(isnan(w_tp_042) == 0);
nanaxisx=find(isnan(w_tp_042) == 1);
...
yCalc2{sig} = [1:length(w_tp_04)].*b1(sig) + b0(sig);
plot(yCalc2{sig},'-.','color','k','linew',2)
% plot([1:1:length(w_tp_04)], upper_bc(sig).*ones(length(w_tp_04)),'color', color_spec(sig),'linew',2)
% plot([1:1:length(w_tp_04)], lower_bc(sig).*ones(length(w_tp_04)),'color', color_spec(sig),'linew',2)
line([tx_tick(13) length(w_tp_04)], [upper_bc(sig) upper_bc(sig)],'color', color_spec(sig),'linew',2)
line([tx_tick(13) length(w_tp_04)], [lower_bc(sig) lower_bc(sig)],'color', color_spec(sig),'linew',2)
yCalc3 = yCalc2{sig};
% plot([1:1:length(w_tp_04)],mean(w_tp_04(idx1)).*ones(length(w_tp_04)),'-.','color',color_spec(sig-1),'linew',2);
line([tx_tick(13) length(w_tp_04)],[mean(w_tp_04(idx1)) mean(w_tp_04(idx1))],'color',color_spec(sig-1),'linew',2);
%     for i = 1:length(yCalc3)
%             w_tp_recon(i) = yCalc3(i)  + yp_w_tp(indx_366{i});
%     end
% plot([1:length(w_tp)], w_tp_recon,'-.','color',color_spec(sig),'linew',1)
end

for sig=3:3  %sigma
clearvars b1 yCalc1 X b nonan idx nanaxisx *_mth_f idx1 sig_txt *_mi2 *_recon yCalc3
% raw
w_tp_04=w_tp(t_tick(end)+1:end);
idx1 = find(isnan(w_tp_04) == 0);
w_tp_042=w_tp_04;
upper_bc(sig) = mean(w_tp_04(idx1)) + sig*std(w_tp_04(idx1));
lower_bc(sig) = mean(w_tp_04(idx1)) - sig*std(w_tp_04(idx1));
...
b0(sig) = b(1);  b1(sig) = b(2);
yCalc2{sig} = [1:length(w_tp_04)].*b1(sig) + b0(sig);
plot([t_tick(end)+1:1:t_tick(end)+length(w_tp_04)],yCalc2{sig},'-.','color','k','linew',2)
% plot([tx_tick(25)+1:1:tx_tick(25)+length(w_tp_04)], upper_bc(sig).*ones(length(tx_tick(25)+1:1:tx_tick(25)+length(w_tp_04))),'color', color_spec(sig),'linew',2)
% plot([tx_tick(25)+1:1:tx_tick(25)+length(w_tp_04)], lower_bc(sig).*ones(length(tx_tick(25)+1:1:tx_tick(25)+length(w_tp_04))),'color', color_spec(sig),'linew',2)
line([t_tick(end)+1 t_tick(end)+length(w_tp_04)], [upper_bc(sig) upper_bc(sig)],'color', color_spec(sig),'linew',2)
line([t_tick(end)+1 t_tick(end)+length(w_tp_04)], [lower_bc(sig) lower_bc(sig)],'color', color_spec(sig),'linew',2)
yCalc3 = yCalc2{sig};
line([t_tick(end)+1 t_tick(end)+length(w_tp_04)],[mean(w_tp_04(idx1)) mean(w_tp_04(idx1))],'color',color_spec(sig-1),'linew',2);
...
title(['sumjin tp raw  ' num2str(sig) '-sigma'],'fontsize',13)
ylim([0 inf])
set(gca,'xtick',[tx_tick(13:3:end)]);
set(gca,'xlim',[tx_tick(13) tx_tick(end)]);
set(gca,'xticklabel',2001:3:2019);
