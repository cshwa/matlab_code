...
for year=2013:2016;
    ...
    daylimit=[4 9 14];
    selectind = [15, 15+eomday(year,1), 15+sum(eomday(year,1:2))]
    for selectmonth = 1:3
        for k= 1:3
            plot_dir = 'C:\Users\kyy\Desktop\video\';
            exp_name = 'sanchi_4';
            plot_type = 'drifter'
            vid_fps=5;
            vid_qty=100;
            ...
            if(flag_record_mp4),    % prepare video object
                vidName = [plot_dir,'',exp_name,'_',plot_type,'_',num2str(year),'_',char(calendar(selectmonth)),'_', num2str(daylimit(k)), '.mp4'];
        ...
        %         vidObj = VideoWriter(vidName,'wmv');
            ...
            %             m_line(lon(i,1:24:24*ind),lat(i,1:24:24*ind));
                        m_line(lon(i,indstart*24+1:24:indstart*24+(ind-1)*24),lat(i,indstart*24+1:24:indstart*24+(ind-1)*24)); 
                        m_line(lon(i,indstart*24+(ind-1)*24),lat(i,indstart*24+(ind-1)*24), ...
    ...
    %                     m_line(lon(i,indstart*24+1:24:indend*24),lat(i,indstart*24+1:24:indend*24));
                    ...
                    hold off;
                    titlename = strcat('(from ', char(calendar(selectmonth)),') ', num2str(year),', ',char(calendar(month)), ', ', num2str(day), '. (', num2str(daylimit(k)+1), ' particles)');
