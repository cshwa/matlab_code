...
inputyear = [1982:1988]; % % put year which you want to plot [year year ...]
...
maxlat=max(max(max(max(max(NIFS6.lat)))))+0.5;
minlat=min(min(min(min(min(NIFS6.lat)))))-0.5;
maxlon=max(max(max(max(max(NIFS6.lon)))))+0.5;
minlon=min(min(min(min(min(NIFS6.lon)))))-0.5;
...
%         J:\fennel_NWP\Output\norain\from1982
        filedir = ['J:\fennel_NWP\Output\',test_name,'\from1982\',num2str(tempyear),'\']
        % ex : rootdir\test37\data\2001\test37_monthly_2001_01.nc
        filename = strcat(filedir, ...
                'roms_monthly_avg.nc');
        filename_par = strcat('J:\fennel_NWP\lateral\', ...
                'roms_bndy_nwp_1_10_SODA_3_4_2_2011_test06.nc');    
        % read model data
        if (exist('lon')==0)
            ...
            Vtransform= ncread(filename_par,'Vtransform');
            Vstretching= ncread(filename_par,'Vstretching');
            theta_s= ncread(filename_par,'theta_s');
            theta_b= ncread(filename_par,'theta_b');
            hc= ncread(filename_par,'hc');
        ...
        zeta = ncread(filename,'zeta',[lon_min(1) lat_min(1) tempmonth], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1]);
...
%         model_depth_z=zlevs(model_depth,zeta,theta_s,theta_b,hc,N,'r');
        ...
        data = ncread(filename,varname,[lon_min(1) lat_min(1) 1 tempmonth], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 N 1]);
...
save(['model_salt_trend_',test_name,'_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'_fix75.mat'], 'MODEL6', 'NIFS6');
