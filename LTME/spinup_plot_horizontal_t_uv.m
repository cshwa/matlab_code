function status=plot_horizontal_t_uv(outfile, section, year, inputmonth, var, var_lim, inputdir)
% clear all;close all;
%==========================================================================
% % This function needs 
...
%     v=nc{'v'}(:);
    ...
    cut_u = ncread(file,'u',[y1(1) x1(1) 1 1], [y2(1)-y1(1)+1 x2(1)-x1(1)+1 40 1]);
    cut_v = ncread(file,'v',[y1(1) x1(1) 1 1], [y2(1)-y1(1)+1 x2(1)-x1(1)+1 40 1]);
    ...
    cut_lon_u=lon_u(x1(1):x2(1),y1(1):y2(1));
    cut_lat_u=lat_u(x1(1):x2(1),y1(1):y2(1));
    cut_lon_v=lon_v(x1(1):x2(1),y1(1):y2(1));
    cut_lat_v=lat_v(x1(1):x2(1),y1(1):y2(1));
%     for k=1:40
%         cut_u_rho(:,:,k)=griddata(cut_lon_u, cut_lat_u, cut_u(:), cut_lon, cut_lat);
%         cut_v_rho(:,:,k)=griddata(cut_lon_v, cut_lat_v, cut_v(:), cut_lon, cut_lat);
%     end
    cut_mask_rho=mask_rho(x1(1):x2(1),y1(1):y2(1));
    cut_mask_u=mask_u(x1(1):x2(1),y1(1):y2(1));
    cut_mask_v=mask_v(x1(1):x2(1),y1(1):y2(1));
    refdepth=(section(5)+section(6))/2.0
    vert_dist=abs(cut_depth-refdepth); %% (z,y,x)
   
    
    for i=1:length(vert_dist(1,:,1))
%         tic;
        for j=1:length(vert_dist(1,1,:))
            if (cut_mask_rho(i,j)==0 || cut_mask_u(i,j)==0 || cut_mask_v(i,j)==0)
                cut_data3(i,j)=NaN;
                cut_u_3(i,j)=NaN;
                cut_v_3(i,j)=NaN;
            else
                if (cut_depth(1,i,j)>refdepth)
                    ...
                    if (z_ind==1)
                        ...
                        cut_u_2(1:3)=cut_u(j,i,z_ind:z_ind+2);
                        cut_v_2(1:3)=cut_v(j,i,z_ind:z_ind+2);
                    else
                        cut_depth2(1:3)=cut_depth(z_ind-1:z_ind+1,i,j);
                        cut_data2(1:3)=cut_data(z_ind-1:z_ind+1,i,j);
                        cut_u_2(1:3)=cut_u(j,i,z_ind-1:z_ind+1);
                        cut_v_2(1:3)=cut_v(j,i,z_ind-1:z_ind+1);
                    end
                    cut_data3(i,j)=interpn(cut_depth2,cut_data2,refdepth,'cubic');
                    cut_u_3(i,j)=interpn(cut_depth2,cut_u_2,refdepth,'cubic');
                    cut_v_3(i,j)=interpn(cut_depth2,cut_v_2,refdepth,'cubic');
                end
            end
        end
%        toc;
    end
   cut_u_rho_3=griddata(cut_lon_u, cut_lat_u, cut_u_3, cut_lon, cut_lat);
   cut_v_rho_3=griddata(cut_lon_u, cut_lat_u, cut_v_3, cut_lon, cut_lat);
...
%     end
    ...
    elseif (var ==2)
        titlename = strcat(num2str(abs(refdepth)),'m Salt (',num2str(tempyear),'th year,',' ',char(calendar(tempmonth)), ')');
    elseif (var ==3)
        titlename = strcat(num2str(abs(refdepth)),'m U (',num2str(tempyear),'th year,',' ',char(calendar(tempmonth)), ')');
    elseif (var ==4)
        titlename = strcat(num2str(abs(refdepth)),'m V (',num2str(tempyear),'th year,',' ',char(calendar(tempmonth)), ')');
    end
    ...
    m_proj('mercator','lon',[section(1) section(2)],'lat',[section(3) section(4)]);
    m_grid('fontsize',17, 'box', 'fancy'); 
    uvplot=m_quiver(cut_lon(1:interval:end,1:interval:end),cut_lat(1:interval:end,1:interval:end),...
        cut_u_rho_3(1:interval:end,1:interval:end),cut_v_rho_3(1:interval:end,1:interval:end),1,'k'); 
    
    mspd=nanmean(nanmean(sqrt((cut_u_rho_3).^2+(cut_v_rho_3).^2)));
    disp(['mean speed : ', num2str(mspd), ' (m/s)' ]);
    m_gshhs_i('color','k')  
