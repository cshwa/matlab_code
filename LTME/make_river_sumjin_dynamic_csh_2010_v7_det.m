...
for t_year = 2010:2010 %before regime change
...
if cycle == 365
        ...
        River.Oxyg(:,:,r)=repmat(sumjin_do,1,20);
        River.chlorophyll(:,:,r)=repmat(sumjin_chl.* 0,1,20);
        River.tPO4(:,:,r)=repmat(riv6_TP_f'.*0.8 .*(1000/30.973762)  ,1,20);
        River.LDeN(:,:,r)=repmat(riv6_TN_f'*(999724/2/1000000)*(1000/14).*0.7,1,20);
        River.SDeN(:,:,r)=repmat(riv6_TN_f'*(999724/2/1000000)*(1000/14).*0.3,1,20);
...
%         r =8;
        ...
        River.lat(r) = 84;  % 행
        River.dir(r) = 1; % 남으로 흐름 -> River.lon(r) 값을 edtimark 값에 +1
        River.trans(:,r) = ([-riv6_dis_f -riv6_dis_f(end)])/86400; % negative: sourthward flow 
        River.temp(:,:,r)=repmat(temp_out_sumjin,1,20);
        River.salt(:,:,r)=repmat(salt_out,1,20);
        River.NH4(:,:,r)=repmat(riv6_TN_f'*(140/1000000)*(1000/14),1,20);
        River.NO3(:,:,r)=repmat(riv6_TN_f'*(136/1000000)*(1000/14),1,20);
...
temp=ncread(Fname,'river_NH4');
dis=ncread(Fname,'river_transport');
figure;plot(dis(1,:));hold on; plot(dis(2,:),'r');
figure;
for i = 1:10; plot(squeeze(temp(i,20,:)));hold on; end
ylim([0 1])

temp=ncread(Fname,'river_NO3');
dis=ncread(Fname,'river_transport');
figure;plot(dis(1,:));hold on; plot(dis(2,:),'r');
figure;
for i = 1:10; plot(squeeze(temp(i,20,:)));hold on; end
