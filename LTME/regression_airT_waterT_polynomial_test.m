...
%% polynomial fitting
for i = 1:7
pf = polyfit(1:length(a_temp),a_temp,i);
xp = 1:length(a_temp);
yp = polyval(pf,xp);

xp_w = find(isnan(w_temp)==0);
pf_w = polyfit(xp_w,w_temp(xp_w),i);
...
grid on;
xlim([1 366])
legend('gawha-water','jinju-air',[num2str(i),'-th'])
set(gca,'fontsize',15)
hold off;
print('-dpng', ['polynomial_fit_gawha- ',num2str(i),'th']);
end
