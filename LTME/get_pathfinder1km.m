function varargout = get_pathfinder1km(varargin)
% Gets Pathfinder 1-km sea surface temperature data series.
%
% Gets xxx data.
%
%  Usage:
...
% calculate time zone based on the range of longitudes
TZone=0.5*(DSI.Longitude(1)+DSI.Longitude(end))/15;
HH=str2num(datestr(Time,'HH'))+str2num(datestr(Time,'MM'))/60; % hour of satellite pass, UTC
HH=mod(HH+TZone,24); % hour of satellite pass, local time.
HHNightDay=6; % boundary between night and day images, local time. 
if(isempty(R.Passes)) 
    errordlg(['No satellite passes selected.']);
    return
elseif isempty(strmatch('night',R.Passes)) 
    j=find(HH>=HHNightDay & HH < HHNightDay+12);
    iTime=iTime(j); % exclude night passes
%    Time(iTime,:)
elseif isempty(strmatch('day',R.Passes))
    j=find(HH<HHNightDay  | HH >= HHNightDay+12);
    iTime=iTime(j); % exclude day passes
%    Time(iTime,:)
end
...
URLFILE=eval(['DSI.URLFILE.' FIELD ';']);       
URLCVAR=eval(['DSI.URLCVAR.' FIELD ';']);
CTIME=['[' num2str(kT-1) ':1:' num2str(kT-1) ']']; % Constraint Time
URL=[DSI.URLSITE DSI.URLPATH URLFILE '?' URLCVAR CTIME R.CLAT R.CLON]; % 2D variables       
