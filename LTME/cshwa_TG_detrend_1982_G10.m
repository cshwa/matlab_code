...
st_name = {'Gunsan', 'Mokpo', 'Heuksando', 'Yeosu', 'Tongyeong', 'Busan', ...
    'Jeju', 'Geomundo', 'Ulsan', 'Pohang', 'Mukho', 'Sokcho', 'Ulleungdo' };
st_num =[4, 6, 7, 10, 11, 13, 15, 17, 18, 19, 20, 21, 22];
lat_st =[35.9756, 34.7297, 34.5842, 34.7472, 34.8278, 35.0564, ...
    33.5675, 33.9983, 35.4719, 36.0372, 37.6003, 38.2072, 37.5614];


for i = 1:length(st_name)
    clearvars -except st_name i st_num lat_st
    [raw txt]=xlsread([st_name{i},'_TG_1982.xlsx'], st_name{i},'');      
    elev=str2num(char(txt(2:end,3)));

       % Load the example.
%          load Yeosu_1982_tide.mat  % this is a local time
% %          load('1980_regrid_v3_zeta.mat','total_temp');
% %          load('cshwa_1982_time_fix.mat');
         load cshwa_1982_time_v2.mat
...
%%%%
%        t1 = datetime(1981,12,31,15,00,00); % >> 21,00,00 (GMT)
%        t2 = datetime(1982,12,31,14,00,00); % >> 22,00,00  (GMT)
%        t = t1:hours(1):t2
%        time = datenum(t)
%        save('cshwa_1981_time_v2','time')
%
...
% 34.74721840
       ...
       'latitude', lat_st(i),...               % Latitude of obs
...
csh_name = {'M2', 'S2', 'N2', 'K2', 'K1', 'O1','P1','Q1','MF','MM'};
for j = 1:length(csh_name)
       srt_ind(j)=find(strcmp(csh_name(j),tidestruc.name)==1);
end
tidestruc.name(srt_ind,:)

tidestruc_10 = struct('name', tidestruc.name(srt_ind,:), ...
    'freq', tidestruc.freq(srt_ind,:), 'tidecon',tidestruc.tidecon(srt_ind,:), 'type',tidestruc.type);

       pout_10=t_predic(time_f,tidestruc_10,...
                     'latitude',lat_st(i),...
                     'anallength','nodal',...
                     'synthesis',1);
 save(['TG_1982_G10_det_fix_',st_name{i},'.mat'],'pout_10','tidestruc_10');
end
return
