...
a = 1 : size(d1,1);
x = find(p(a) > 0.15 & c(a) > 30);
y = find(p(a) == max(p));
b = [x(find(diff(x) ~= 1)+5) : y(1)]; d1 = d(1) * ones(size(b,2),1);
st1_2 = sortrows([d1 p(b) t(b) s(b)]);
j = [1 : size(st1_2,1)-1];
k = find(diff(st1_2(j,2)) == 0);
i = 1 : size(k,1);
st1_2(k(i,1),:) = [];

fid = fopen('st1_2.txt','w');
fprintf(fid, '%f %f %f %f\n', st1_2');
fclose(fid);
...
m12 = [st1_2;st2_2;st4_2;st5_2;st6_2;st7_2];
...
gtem2 = m12;
gtem2(:,2) = -1 .* gtem2(:,2);
fid = fopen('gtem2.txt','w');
fprintf(fid, '%f %f %f %f\n', gtem2');
fclose(fid);


figure; plot(st1_2(:,3),st1_2(:,2)); hold on;
plot(st2_2(:,3),st2_2(:,2),'k');
plot(st4_2(:,3),st4_2(:,2),'y');
plot(st5_2(:,3),st5_2(:,2),'r');
plot(st6_2(:,3),st6_2(:,2),'g');
plot(st7_2(:,3),st7_2(:,2),'c');
