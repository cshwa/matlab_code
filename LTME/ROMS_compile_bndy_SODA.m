...
warning off;
run /home/yjtak/Dropbox/source/matlab/Model/ROMS/Roms_tools/Run/romstools_param;
linux=1; windows=0;
if (windows ==1)
    % % for windows
...
%     dropboxpath='/home01/kimyy/Dropbox'; %% ROMS server
    dropboxpath='/home/yjtak/Dropbox'; %% DAMO server
    addpath(genpath([dropboxpath '/source/matlab/Common/m_map']));
    addpath(genpath([dropboxpath '/source/matlab/Common/Figure']));
...
casename = 'auto';
testname = 'test1';
addpath(genpath('/home/yjtak/Dropbox/source/matlab/Common/netcdf_old/'))
quot = '''';

ROMS_title = 'ECSandYS Model';
if (windows==1)
    bryname = ['E:\Data\Model\ROMS\nwp_1_20\input\test38\','roms_bndy_',casename,'_SODA_3_4_2_',num2str(yyyy), '_',testname,'.nc'];
elseif (linux==1)
    bryname = ['/data1/yjtak/make_SODA/','roms_bndy_',casename,'_SODA_3_4_2_',num2str(yyyy), '_',testname,'.nc'];
end
...
for mi = mm(1):mm(end);
    ...
    elseif (linux==1)
        filename = ['/data1/yjtak/make_SODA/','SODA_Y', num2str(yyyy,4), 'M', num2str(mi), '_YS.nc'];
    end
        nc = netcdf(filename);
