...
%
% grdname=['..\grid\grid_pohang_csh_fine_flat_tide.nc'];
% grdname=['..\grid\grid_pohang_tide_runoff_8_BC_2m.nc'];
grdname=['E:\SAR_data\sar_grid_1m_reduced.nc'];

frcname=['.\sar_tide_csh_1m_reduced.nc'];

tidename=['E:\ROMS\roms_tools\TPXO7\TPXO7.nc'];
%
...
%===================================================================
%
Ymin          = 1980;               % first forcing year
Ymax          = 1980;               % last  forcing year
% Mmin          = 3;                  % first forcing month %% 1st file
Mmin          = 1;  
...
close(nc);
nc_add_tides(frcname,Ntides,etar,xir,date_mjd,components)
...
%
  ...
  ncfrc{'tide_Eamp'}(itide,:,:)=abs(ei)*correc_amp*1.2; % * 12 equal 10times larger
%
% Process U
%
  disp('  u...')
...
%
  ...
  ncfrc{'tide_Cmin'}(itide,:,:)=0;
  ncfrc{'tide_Cmax'}(itide,:,:)=0;
