...
Fname =['river_' num2str(t_year) '_realts_biofennel_GY_sewer_det_v9.nc'];
...
% sumj=load('sumjin(songjung)_polynomial_climate_to2004_advanced(v4)_3sig.mat','yp_w_*');   % polynomial_fitting_on_ecological_variable_to2004_new_v5_3sigma.m 
% ~2003 : advanced 전체기간
% 2004~ : 04~ data's climate
sumj=load('sumjin(songjung)_polynomial_climate_to2004_3sig.mat','yp_w_*'); % ~2004 : 1st regime, 2004~2018 : 2nd regime
sumjin_do = sumj.yp_w_do_af'*0.7*44.661;
sumjin_chl = sumj.yp_w_chl_af';
sumjin_no3 = sumj.yp_w_no3_af'*1000/14;
sumjin_nh4 = sumj.yp_w_nh4_af'*1000/14;


sumj_p=load('D:\장기생태\Dynamic\06_river\환경과학원\sumjin(songjung)_polynomial_climate_to2011_3sig_po4.mat'); % ~2010 : 1st regime(yp_w_po4_04), 2011~2018 : 2nd(yp_w_po4_af)
sumjin_po4 = sumj_p.yp_w_po4_04'.*1000./30.973762;

clearvars yp_w_*
% namg=load('Namgang_polynomial_climate_to2004_advanced(v4)_3sig.mat','yp_w_*');
namg=load('Namgang_polynomial_climate_to2004_3sig.mat','yp_w_*'); % ~2004 : 1st regime, 2004~2018 : 2nd regime
nam_do = namg.yp_w_do_af'*0.7*44.661;  %% mg/L to mM/m^3;
nam_chl = namg.yp_w_chl_af'; 
% nam_chl(364:366) = nam_chl(363); % negative concen. to be closest sample.
nam_no3 = namg.yp_w_no3_af'*1000/14; %% mg/L to mM/m^3;
nam_nh4 = namg.yp_w_nh4_af'*1000/14; %% mg/L to mM/m^3;
...
if cycle == 365
        ...
        River.temp(:,:,r)=repmat(temp_out_sumjin,1,20);
        River.salt(:,:,r)=repmat(salt_out,1,20);
        River.NH4(:,:,r)=repmat(clim_tn_g_ns_d'*(0.3)*(1000/14),1,20); % 30% of TN
        River.NO3(:,:,r)=repmat(clim_tn_g_ns_d'*(0.6)*(1000/14),1,20); % 60% of TN
        River.Oxyg(:,:,r)=repmat(sumjin_do,1,20);
        ...
        River.LDeN(:,:,r)=repmat(clim_tn_g_ns_d'*(0.07)*(1000/14),1,20); % 7% of TN
        River.SDeN(:,:,r)=repmat(clim_tn_g_ns_d'*(0.03)*(1000/14),1,20); % 3% of TN
        River.vshape(r,1:N)=1/N;
...
%         % r = 4; 
        ...
        River.temp(:,:,r)=repmat(temp_out_sumjin,1,20);
        River.salt(:,:,r)=repmat(salt_out,1,20);
        River.NH4(:,:,r)=repmat(clim_tn_gc_ns_d'*(0.3)*(1000/14),1,20);
        River.NO3(:,:,r)=repmat(clim_tn_gc_ns_d'*(0.6)*(1000/14),1,20);
        River.Oxyg(:,:,r)=repmat(sumjin_do,1,20);
        River.tPO4(:,:,r)=repmat(clim_tp_gc_ns_d'.*0.8 .*(1000/30.973762)  ,1,20);
        River.chlorophyll(:,:,r)=repmat(sumjin_chl.* 0,1,20);
        % River.DON(:,:,r)=repmat(riv1_TN_f'*(999724/2/1000000)*(1000/14),1,20);
        River.LDeN(:,:,r)=repmat(clim_tn_gc_ns_d'*(0.07)*(1000/14),1,20);
        River.SDeN(:,:,r)=repmat(clim_tn_gc_ns_d'*(0.03)*(1000/14),1,20);
        River.vshape(r,1:N)=1/N;
...
%         % r = 5; 
        ...
        River.temp(:,:,r)=repmat(temp_out_sumjin,1,20);
        River.salt(:,:,r)=repmat(salt_out,1,20);
        River.NH4(:,:,r)=repmat(clim_tn_gh_ns_d'*(0.3)*(1000/14),1,20);
        River.NO3(:,:,r)=repmat(clim_tn_gh_ns_d'*(0.6)*(1000/14),1,20);
        River.Oxyg(:,:,r)=repmat(sumjin_do,1,20);
        River.tPO4(:,:,r)=repmat(clim_tp_gh_ns_d'.*0.8 .*(1000/30.973762)  ,1,20);
        River.chlorophyll(:,:,r)=repmat(sumjin_chl.* 0,1,20);
        River.LDeN(:,:,r)=repmat(clim_tn_gh_ns_d'*(0.07)*(1000/14),1,20);
        River.SDeN(:,:,r)=repmat(clim_tn_gh_ns_d'*(0.03)*(1000/14),1,20);
        River.vshape(r,1:N)=1/N;
...
%         r = 4;
        ...
        River.temp(:,:,r)=repmat(temp_out_sumjin,1,20);
        River.salt(:,:,r)=repmat(salt_out,1,20);
        River.NH4(:,:,r)=repmat(clim_tn_gh2_ns_d'*(0.3)*(1000/14),1,20);
        River.NO3(:,:,r)=repmat(clim_tn_gh2_ns_d'*(0.6)*(1000/14),1,20);
        River.Oxyg(:,:,r)=repmat(sumjin_do,1,20);
        River.tPO4(:,:,r)=repmat(clim_tp_gh2_ns_d'.*0.8 .*(1000/30.973762)  ,1,20);
        River.chlorophyll(:,:,r)=repmat(sumjin_chl.* 0,1,20);
        % River.DON(:,:,r)=repmat(riv4_TN_f'*(999724/2/1000000)*(1000/14),1,20);
        River.LDeN(:,:,r)=repmat(clim_tn_gh2_ns_d'*(0.07)*(1000/14),1,20);
        River.SDeN(:,:,r)=repmat(clim_tn_gh2_ns_d'*(0.03)*(1000/14),1,20);
        River.vshape(r,1:N)=1/N;
        %--- 여수월내 -----------------------------------------
        % 1990-12-31
        r = 7; 
...
%         r = 5; 
        ...
        River.temp(:,:,r)=repmat(temp_out_sumjin,1,20);
        River.salt(:,:,r)=repmat(salt_out,1,20);
        River.NH4(:,:,r)=repmat(temp_clim_tn'*(0.3)*(1000/14),1,20);
        River.NO3(:,:,r)=repmat(temp_clim_tn'*(0.6)*(1000/14),1,20);
        River.Oxyg(:,:,r)=repmat(sumjin_do,1,20);
        River.tPO4(:,:,r)=repmat(temp_clim_tp'.*0.8 .*(1000/30.973762)  ,1,20);
        River.chlorophyll(:,:,r)=repmat(sumjin_chl.* 0,1,20);
        % River.DON(:,:,r)=repmat(riv5_TN_f'*(999724/2/1000000)*(1000/14),1,20);
        River.LDeN(:,:,r)=repmat(temp_clim_tn'*(0.07)*(1000/14),1,20);
        River.SDeN(:,:,r)=repmat(temp_clim_tn'*(0.03)*(1000/14),1,20);
        River.vshape(r,1:N)=1/N;
        ...
        River.temp(:,:,r)=repmat(temp_out_sumjin,1,20);
        River.salt(:,:,r)=repmat(salt_out,1,20);
        River.NH4(:,:,r)=repmat(clim_tn_y_ns_d'*(0.3)*(1000/14),1,20);
        River.NO3(:,:,r)=repmat(clim_tn_y_ns_d'*(0.6)*(1000/14),1,20);
        River.Oxyg(:,:,r)=repmat(sumjin_do,1,20);
        River.tPO4(:,:,r)=repmat(clim_tp_y_ns_d'.*0.8 .*(1000/30.973762)  ,1,20);
        River.chlorophyll(:,:,r)=repmat(sumjin_chl.* 0,1,20);
        % River.DON(:,:,r)=repmat(riv5_TN_f'*(999724/2/1000000)*(1000/14),1,20);
        River.LDeN(:,:,r)=repmat(clim_tn_y_ns_d'*(0.07)*(1000/14),1,20);
        River.SDeN(:,:,r)=repmat(clim_tn_y_ns_d'*(0.03)*(1000/14),1,20);
        River.vshape(r,1:N)=1/N;
...
%         r =6;
        if t_year <= 2007
            temp_clim_tp=clim_tp_w_1st_ns_d;
            disp('jungheung-TP-1st tyear <= 2007 ')
        elseif t_year >= 2008
            temp_clim_tp=clim_tp_w_2nd_ns_d;
            disp('jungheung-TP-2nd tyear >= 2008')
        end
        
        if t_year <= 2003
            temp_clim_tn=clim_tn_w_1st_ns_d;
            disp('jungheung-TN-1st tyear <= 2003')
        elseif t_year >= 2004 & t_year <= 2008
            temp_clim_tn=clim_tn_w_2nd_ns_d;
            disp('jungheung-TN-2nd 2004 <= tyear <= 2008')
        elseif t_year >= 2009
            temp_clim_tn=clim_tn_w_3rd_ns_d;
            disp('jungheung-TN-3rd tyear >= 2009')
        end 
        ...
        River.temp(:,:,r)=repmat(temp_out_sumjin,1,20);
        River.salt(:,:,r)=repmat(salt_out,1,20);
        River.NH4(:,:,r)=repmat(clim_tn_ye_ns_d'*(0.3)*(1000/14),1,20);
        River.NO3(:,:,r)=repmat(clim_tn_ye_ns_d'*(0.6)*(1000/14),1,20);
        River.Oxyg(:,:,r)=repmat(sumjin_do,1,20);
        River.tPO4(:,:,r)=repmat(clim_tp_ye_ns_d'.*0.8 .*(1000/30.973762)  ,1,20);
        River.chlorophyll(:,:,r)=repmat(sumjin_chl.* 0,1,20);
        % River.DON(:,:,r)=repmat(temp_clim_tn'*(999724/2/1000000)*(1000/14),1,20);
        % River.PON(:,:,r)=repmat(riv7_TN_f'*(999724/2/1000000)*(1000/14),1,20);
        River.LDeN(:,:,r)=repmat(clim_tn_ye_ns_d'*(0.07)*(1000/14),1,20);
        River.SDeN(:,:,r)=repmat(clim_tn_ye_ns_d'*(0.03)*(1000/14),1,20);
        River.vshape(r,1:N)=1/N;
        ...
        River.temp(:,:,r)=repmat(temp_out_sumjin,1,20);
        River.salt(:,:,r)=repmat(salt_out,1,20);
        River.NH4(:,:,r)=repmat(clim_tn_jw_ns_d'*(0.3)*(1000/14),1,20);
        River.NO3(:,:,r)=repmat(clim_tn_jw_ns_d'*(0.6)*(1000/14),1,20);
        River.Oxyg(:,:,r)=repmat(sumjin_do,1,20);
        River.tPO4(:,:,r)=repmat(clim_tp_jw_ns_d'.*0.8 .*(1000/30.973762)  ,1,20);
        River.chlorophyll(:,:,r)=repmat(sumjin_chl.* 0,1,20);
        % River.DON(:,:,r)=repmat(temp_clim_tn'*(999724/2/1000000)*(1000/14),1,20);
        % River.PON(:,:,r)=repmat(riv7_TN_f'*(999724/2/1000000)*(1000/14),1,20);
        River.LDeN(:,:,r)=repmat(clim_tn_jw_ns_d'*(0.07)*(1000/14),1,20);
        River.SDeN(:,:,r)=repmat(clim_tn_jw_ns_d'*(0.03)*(1000/14),1,20);
        River.vshape(r,1:N)=1/N;
