...
for j=1989:2019
        pre_t_ax_mth(j-1988)=sum(eomday(j,1:12));
end

for i = 1:length(1989:2019)
...
b = X\nonan';
yCalc2 = X*b;
plot(nanaxisx(end)+1:nanaxisx(end)+length(idx),yCalc2,'--','color','r','linew',2)
gtext(['y = ',num2str(b(2),2),'x + ',num2str(b(1),2)],'Color','k','FontSize',16)
...
title(['sumjin(hadong)-polynomial DO yearly mean'],'fontsize',13)
grid on; set(gca,'fontsize',13)
set(gca,'xtick',[1:365*3:31*365]);
set(gca,'xlim',[1 31*365]);
set(gca,'xticklabel',1989:3:2019);
return
%regression
%slope y = b1*x
