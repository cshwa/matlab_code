...
% port
% name_tag_1{1} = [num2str(01,'%02d'),'번'] %205-01
name_tag_1{1} = [num2str(16,'%02d'),'번'] %400-16
...
indx_date_s = cell(length(name_tag_1),length(ref_yymm));
indx_date_b = cell(length(name_tag_1),length(ref_yymm));
for j = 1:length(name_tag_1) % st. axis
    for i = 1:length(ref_yymm) % date axis
       if  sum(strcmp(ref_yymm{i}, date_ymd{j})) ~= 0
           clearvars indx_date
...
% temp
t_s_nonan = find(isnan(temp_sur)==0)
temp_sur(t_s_nonan)'
ref_yymm(t_s_nonan)'
...
% salt
s_s_nonan = find(isnan(salt_sur)==0)
salt_sur(s_s_nonan)'
ref_yymm(s_s_nonan)'

s_b_nonan = find(isnan(salt_bot)==0)
salt_bot(s_b_nonan)'
ref_yymm(s_b_nonan)'

%no3
no3_s_nonan = find(isnan(no3_sur)==0)
no3_sur(no3_s_nonan)'
ref_yymm(no3_s_nonan)'

no3_b_nonan = find(isnan(no3_bot)==0)
no3_bot(no3_b_nonan)'
ref_yymm(no3_b_nonan)'

%po4
po4_s_nonan = find(isnan(po4_sur)==0)
po4_sur(po4_s_nonan)'
ref_yymm(po4_s_nonan)'

po4_b_nonan = find(isnan(po4_bot)==0)
po4_bot(po4_b_nonan)'
ref_yymm(po4_b_nonan)'
%%
...
set(gca,'xtick',[1:12*3:12*40]);
set(gca,'xlim',[1 12*40]);
...
figure; hold on;
 plot(temp_sur,'-','color','b','linew',2);
 plot(temp_bot,'-','color','r','linew',2);   

 figure; hold on;
 plot(salt_sur,'-','color','b','linew',2);
 plot(salt_bot,'-','color','r','linew',2);
