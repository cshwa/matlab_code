...
% sp_gy = [4,22,28,29,30,32,33,34,35]; 's spatial mean time series has to
% be calculated by regime shift method.

 sp_gy = [4,22,28,29,30,32,33,34,35];

reg_sp_gy_no3(1,:) = nanmean(regime_no3(sp_gy,:)); 
reg_sp_gy_nh4(1,:) = nanmean(regime_nh4(sp_gy,:)); 
reg_sp_gy_do(1,:) = nanmean(regime_do(sp_gy,:)); 
reg_sp_gy_chl(1,:) = nanmean(regime_chl(sp_gy,:)); 
reg_sp_gy_temp(1,:) = nanmean(regime_temp(sp_gy,:)); 
reg_sp_gy_salt(1,:) = nanmean(regime_salt(sp_gy,:)); 
%bot
reg_sp_gy_no3_b(1,:) = nanmean(regime_no3_b(sp_gy,:)); 
reg_sp_gy_nh4_b(1,:) = nanmean(regime_nh4_b(sp_gy,:)); 
reg_sp_gy_do_b(1,:) = nanmean(regime_do_b(sp_gy,:)); 
reg_sp_gy_chl_b(1,:) = nanmean(regime_chl_b(sp_gy,:)); 
reg_sp_gy_temp_b(1,:) = nanmean(regime_temp_b(sp_gy,:)); 
reg_sp_gy_salt_b(1,:) = nanmean(regime_salt_b(sp_gy,:)); 

in_reg_shift_d = ref_date; 
reg_no3_nan = find(isnan(reg_sp_gy_no3)==1);
reg_nh4_nan = find(isnan(reg_sp_gy_nh4)==1);
reg_chl_nan = find(isnan(reg_sp_gy_chl)==1);
reg_do_nan = find(isnan(reg_sp_gy_do)==1);
reg_temp_nan = find(isnan(reg_sp_gy_temp)==1);
reg_salt_nan = find(isnan(reg_sp_gy_salt)==1);

reg_no3_nan_b = find(isnan(reg_sp_gy_no3_b)==1);
reg_nh4_nan_b = find(isnan(reg_sp_gy_nh4_b)==1);
reg_chl_nan_b = find(isnan(reg_sp_gy_chl_b)==1);
reg_do_nan_b = find(isnan(reg_sp_gy_do_b)==1);
reg_temp_nan_b = find(isnan(reg_sp_gy_temp_b)==1);
reg_salt_nan_b = find(isnan(reg_sp_gy_salt_b)==1);

reg_sp_gy_no3(reg_no3_nan) = [];
in_reg_shift_d_no3 = in_reg_shift_d;
in_reg_shift_d_no3(reg_no3_nan) = [];

reg_sp_gy_nh4(reg_nh4_nan) = [];
in_reg_shift_d_nh4 = in_reg_shift_d;
in_reg_shift_d_nh4(reg_nh4_nan) = [];

reg_sp_gy_chl(reg_chl_nan) = [];
in_reg_shift_d_chl = in_reg_shift_d;
in_reg_shift_d_chl(reg_chl_nan) = [];

reg_sp_gy_do(reg_do_nan) = [];
in_reg_shift_d_do = in_reg_shift_d;
in_reg_shift_d_do(reg_do_nan) = [];

reg_sp_gy_temp(reg_temp_nan) = [];
in_reg_shift_d_temp = in_reg_shift_d;
in_reg_shift_d_temp(reg_temp_nan) = [];

reg_sp_gy_salt(reg_salt_nan) = [];
in_reg_shift_d_salt = in_reg_shift_d;
in_reg_shift_d_salt(reg_salt_nan) = [];

%bot
reg_sp_gy_no3_b(reg_no3_nan_b) = [];
in_reg_shift_d_no3_b = in_reg_shift_d;
in_reg_shift_d_no3_b(reg_no3_nan_b) = [];

reg_sp_gy_nh4_b(reg_nh4_nan_b) = [];
in_reg_shift_d_nh4_b = in_reg_shift_d;
in_reg_shift_d_nh4_b(reg_nh4_nan_b) = [];

reg_sp_gy_chl_b(reg_chl_nan_b) = [];
in_reg_shift_d_chl_b = in_reg_shift_d;
in_reg_shift_d_chl_b(reg_chl_nan_b) = [];

reg_sp_gy_do_b(reg_do_nan_b) = [];
in_reg_shift_d_do_b = in_reg_shift_d;
in_reg_shift_d_do_b(reg_do_nan_b) = [];

reg_sp_gy_temp_b(reg_temp_nan) = [];
in_reg_shift_d_temp_b = in_reg_shift_d;
in_reg_shift_d_temp_b(reg_temp_nan_b) = [];

reg_sp_gy_salt_b(reg_salt_nan) = [];
in_reg_shift_d_salt_b = in_reg_shift_d;
in_reg_shift_d_salt_b(reg_salt_nan_b) = [];


find_t_ind_no3 = {'2009-12'};
find_t_ind_no3_b = {'2009-12'};
find_t_ind_nh4 = {'2004-08'};
find_t_ind_nh4_b = {'2002-12'};
find_t_ind_salt_b = {'2003-12'};

clearvars find_date
find_date=find(strcmp(ref_date,find_t_ind_no3) == 1);
yr10_filter_no3(1:find_date) = 1;
yr10_filter_no3(find_date+1:length(ref_date)) = NaN;

clearvars find_date
find_date=find(strcmp(ref_date,find_t_ind_no3_b) == 1);
yr10_filter_no3_b(1:find_date) = 1;
yr10_filter_no3_b(find_date+1:length(ref_date)) = NaN;

clearvars find_date
find_date=find(strcmp(ref_date,find_t_ind_nh4) == 1);
yr10_filter_nh4(1:find_date) = 1;
yr10_filter_nh4(find_date+1:length(ref_date)) = NaN;

clearvars find_date
find_date=find(strcmp(ref_date,find_t_ind_nh4_b) == 1);
yr10_filter_nh4_b(1:find_date) = 1;
yr10_filter_nh4_b(find_date+1:length(ref_date)) = NaN;

clearvars find_date
find_date=find(strcmp(ref_date,find_t_ind_salt_b) == 1);
yr10_filter_salt_b(1:find_date) = 1;
yr10_filter_salt_b(find_date+1:length(ref_date)) = NaN;

for i = 1:65
regime_no3(i,:) = regime_no3(i,:) .* yr10_filter_no3; 
regime_nh4(i,:) = regime_nh4(i,:) .* yr10_filter_nh4; 
...
regime_no3_b(i,:) = regime_no3_b(i,:) .* yr10_filter_no3_b; 
regime_nh4_b(i,:) = regime_nh4_b(i,:) .* yr10_filter_nh4_b; 
...
regime_salt_b(i,:) = regime_salt_b(i,:) .* yr10_filter_salt_b; 
end
...
%%%%%%%%%%
return
cd D:\장기생태\Dynamic\KOEM\gy_2001\mp_p_sewer_det\climate_regime %save pic location.
...
% cd D:\장기생태\Dynamic\KOEM\gy_2001\mp_p_sewer_det\climate
%% GY plot
