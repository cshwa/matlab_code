...
load(['D:\장기생태\Dynamic\KOEM\NIFS\model_salt_trend_',test_name,'_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'_fix75.mat']);
...
%         msaltplot2=plot(xData,line_trend2,'Color','r')
        ...
        txt3=text(xData(end-5), min(meanplotlev)+1 ,['bias = ', num2str(round(mean_salt_bias,2))], 'FontSize', m_quiver_ref_text_fontsize); 
...
%         msaltplot2=plot(xData,line_trend2,'Color','r')
        ...
        set(gca,'YTick',min(meanplotlev):1:max(meanplotlev))
        txt1=text(xData(3), min(meanplotlev)+1 ,['R = ', num2str(round(constant_cor(1,2),2)), ', '], 'FontSize', m_quiver_ref_text_fontsize); 
        txt2=text(xData(20), min(meanplotlev)+1 ,['RMSD = ', num2str(round(mean_salt_RMS,2)), ', '], 'FontSize', m_quiver_ref_text_fontsize); 
        txt3=text(xData(38), min(meanplotlev)+1 ,['bias = ', num2str(round(mean_salt_bias,2))], 'FontSize', m_quiver_ref_text_fontsize); 
        lgd=legend(['Model(',testname,')'],'NIFS');
