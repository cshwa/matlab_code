...
for i = 1:length(name_tag_1)
clearvars temp temp_ymd temp_ymd_c temp_yymmdd temp_yymmdd_c temp_mm temp_mm_c
temp = char(date_st{i});
temp_mm =temp(:,end-14:end-12);
temp_ymd=temp(:,1:end-12);
temp_yymmdd=temp(:,1:end-9);

for j = 1:length(temp_mm)
    temp_mm_c{j} = temp_mm(j,:);
    temp_ymd_c{j} = temp_ymd(j,:);
    temp_yymmdd_c{j} = temp_yymmdd(j,:);
                    
end
date_mm{i,1} = temp_mm_c;
...
for j = 1:length(name_tag_1) % st. axis
    for i = 1:length(ref_mm) % date axis
       if  sum(strcmp(ref_mm{i}, date_ymd{j})) ~= 0
           clearvars indx_date
           indx_date = find([strcmp(ref_mm{i}, date_mm{j})] == 1); 
...
indx_date_s = cell(1,length(ref_mm));
indx_date_b = cell(1,length(ref_mm));
for j = 1:length(name_tag_1) % st. axis
    for i = 1:length(ref_mm) % date axis
       if  sum(strcmp(ref_mm{i}, date_mm{j})) ~= 0
           clearvars indx_date
           indx_date = find([strcmp(ref_mm{i}, date_mm{j})] == 1); 
           indx_date_s{j,i} = indx_date(find(depth_kodc{j}(1,indx_date) == 0));
           indx_date_b{j,i} = indx_date(find(depth_kodc{j}(1,indx_date) == max(depth_kodc{j}(1,indx_date))));
%        elseif sum(strcmp(ref_mm{i}, date_mm{j})) == 0
%            indx_date_s{j,i} = NaN;
%            indx_date_b{j,i} = NaN;
       end
    end
...
clearvars mmdd_indx
for i = 1:length(mm_c)
       mm_indx{i} = find([strcmp(mm_c{i}, ref_mm)] == 1)
end


%confirm how may data on each the days
size_c = [];
for i = 1:12; size_c(i)=length(mm_indx{i}); end
bar(size_c)

% make 1980~present
mon_end=length(1980:2000)*12
mon_end2=length(1980:2010)*12+1
% ~2000.12.31 has to be NaN; also 2011.01.01 ~ has to be NaN;
% % 1:tx_tick(22)-1  : ~2000.12.31 
% % tx_tick(31)+1:end  : 2011.01.01

do_sur(1:mon_end) = NaN; do_sur(mon_end2:end) = NaN;
do_bot(1:mon_end) = NaN; do_bot(mon_end2:end) = NaN;
no3_sur(1:mon_end) = NaN; no3_sur(mon_end2:end) = NaN;
no3_bot(1:mon_end) = NaN; no3_bot(mon_end2:end) = NaN;
temp_sur(1:mon_end) = NaN; temp_sur(mon_end2:end) = NaN;
temp_bot(1:mon_end) = NaN; temp_bot(mon_end2:end) = NaN;
salt_sur(1:mon_end) = NaN; salt_sur(mon_end2:end) = NaN;
salt_bot(1:mon_end) = NaN; salt_bot(mon_end2:end) = NaN;
...
regime_do = regime_do;
regime_no3 = regime_no3;
regime_temp = regime_temp;
regime_salt = regime_salt;

regime_do_b = regime_do_b;
regime_no3_b = regime_no3_b;
regime_temp_b = regime_temp_b;
regime_salt_b = regime_salt_b;
...
idx1 = find(isnan(salt_bot) == 0);
regime_salt_b=salt_bot;
 mean(salt_bot(idx1)) + std(salt_bot(idx1))
regime_salt_b(find(salt_bot < mean(salt_bot(idx1)) - sig*std(salt_bot(idx1))))=NaN;
regm_salt_b = nanmean(regime_salt)

% make quasi_climate 1980~2019
for i = 1:length(mm_indx)
    if size(mm_indx{i},1) == 0     
        reg_clim_do(i) = NaN;
        reg_clim_no3(i) = NaN;
        reg_clim_temp(i) = NaN; 
        reg_clim_salt(i) = NaN;
        ...
        reg_clim_do(i) = nanmean(regime_do(mm_indx{i}));
        reg_clim_no3(i) = nanmean(regime_no3(mm_indx{i}));
        reg_clim_temp(i) = nanmean(regime_temp(mm_indx{i}));
        reg_clim_salt(i) = nanmean(regime_salt(mm_indx{i}));
        reg_clim_do_b(i) = nanmean(regime_do(mm_indx{i}));
        reg_clim_no3_b(i) = nanmean(regime_no3(mm_indx{i}));
        reg_clim_temp_b(i) = nanmean(regime_temp(mm_indx{i}));
        reg_clim_salt_b(i) = nanmean(regime_salt(mm_indx{i}));
        std_clim_do(i) = nanstd(regime_do(mm_indx{i}));
        std_clim_no3(i) = nanstd(regime_no3(mm_indx{i}));
        std_clim_temp(i) = nanstd(regime_temp(mm_indx{i}));
        std_clim_salt(i) = nanstd(regime_salt(mm_indx{i}));
        std_clim_do_b(i) = nanstd(regime_do(mm_indx{i}));
        std_clim_no3_b(i) = nanstd(regime_no3(mm_indx{i}));
        std_clim_temp_b(i) = nanstd(regime_temp(mm_indx{i}));
        std_clim_salt_b(i) = nanstd(regime_salt(mm_indx{i}));
    end
end
...
%% DO
figure;
scatter(1:12,reg_clim_do); hold on;
plot(1:12,reg_clim_do + std_clim_do,'-r')
hold on
...
%% salt
figure;
scatter(1:12,reg_clim_salt); hold on;
plot(1:12,reg_clim_salt + std_clim_salt,'-r')
hold on
xlabel('time(days)','fontsize',13)
...
lon_20501 = 127.8083;
lat_20501 = 34.3717;
save('kodc_20501_2001_2010_just_mean_monthly.mat','-v7.3');
...
c_t = 1:length(no3_sur);

[t t_in]=sort(xx_con_mer);
yCal_con_int_f = NaN(length(c_t),1);
yCal_con_int_f(t) = yCal_con_int(t_in);

yCal_con_int_f(isnan(yCal_con_int_f)) = interp1( c_t(~isnan(yCal_con_int_f)), yCal_con_int_f(~isnan(yCal_con_int_f)), c_t(isnan(yCal_con_int_f)) ); 

interp_nh4_b = temp_b;

 plot(temp_b,'color','r');
 plot(yCal_con_int_f,'color','g');  
 ylim([-inf inf])
xlabel('time(year)','fontsize',13)
ylabel('do (mg/L)','fontsize',13)
grid on; set(gca,'fontsize',13)
...
% make input bottom DO 
temp_bb_do = temp_b;
% temp_bb_do(367:1011) = yCal_con_int_f(367:1011);
temp_bb_do(367:945) = yCal_con_int_f(367:945);

temp_bb_do(946:1010) = NaN; % to make it natural
temp_bb_do(isnan(temp_bb_do)) = interp1( c_t(~isnan(temp_bb_do)), temp_bb_do(~isnan(temp_bb_do)), c_t(isnan(temp_bb_do)) ); 

plot(temp_bb_do,'m','linew',2);

do_re_b = temp_bb_do;
save('do_b_kodc_input_205_01.mat','tx_tick','do_re_b');
