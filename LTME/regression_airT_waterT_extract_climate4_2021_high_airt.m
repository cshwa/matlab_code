...
temp_air=[rawa1(:,6); rawa2(:,6); rawa3(:,6); rawa4(:,7);];  % high air_temp
...
save('gawha_regression_extract_climate_high_airT_2020.mat'); 
...
cd D:\장기생태\Dynamic\06_river_physics
[raw_n1 txt_n1]=xlsread('남강댐1_2019.xls','수질(일반측정망)','');
[raw_n2 txt_n2]=xlsread('남강댐1_2020.xls','수질(일반측정망)','');
% [raw_n3 txt_n3]=xlsread('남강댐1_2021.xls','수질(일반측정망)','');
...
r_txt_ud = flipud(txt);
pre_date = char(r_txt_ud(1:end-1,5));

r_date_txt=[pre_date(:,1:4) repmat(dash_c,length(pre_date),1) pre_date(:,6:7) ...
    repmat(dash_c,length(pre_date),1) pre_date(:,9:10)];
% r_date_txt = [r_date_txt; n_2018_ymd; n_2019_ymd; n_2020_ymd;]; % concaternate (merge)
r_date_txt = [r_date_txt; n_2018_ymd; n_2019_ymd; ]; % concaternate (merge)

r_temp_txt=[r_txt_ud(1:end-1,13)];
r_temp=str2num(char(r_temp_txt));
r_temp = [r_temp; raw_n1(:,14); raw_n2(:,14); ]; % concaternate (merge)
% temp_air=[rawa1(:,3); rawa2(:,3); rawa3(:,3); rawa4(:,4);];
temp_air=[rawa1(:,6); rawa2(:,6); rawa3(:,6); rawa4(:,7);]; % high airT
temp_date=[txta1(2:end,2); txta2(2:end,2); txta3(2:end,2); txta4(2:end,3);]; %air temp date
...
save('gawha(namgang1)_regression_extract_climate_high_airT_2021.mat'); 
...
cd D:\장기생태\Dynamic\06_river_physics\환경과학원
[raw txt]=xlsread('수질측정지점_진월.xls','검색결과','');
[raw2 txt2]=xlsread('진월_수질_일반측정망_201901-201912.xls','수질(일반측정망)','');
[raw3 txt3]=xlsread('진월_수질_일반측정망_202001-202012.xls','수질(일반측정망)','');
...
grid on;
xlim([1 366])
legend('sumjin(jinwal)','yeosu-air')
set(gca,'fontsize',15)

save('jinwal_regression_yeosu_extract_climate_2021_high_airT.mat'); 
...
save('agyang_regression_yeosu_extract_climate_2020_high_airT.mat'); 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
close all; clear; clc; 
cd D:\장기생태\Dynamic\06_river_physics
