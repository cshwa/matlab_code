function create_river_NH4(grd,Fname,cycle,grd_file,s_rho,river,ntime)
disp('  ')
...
if isempty(nc)
  ...
  nc.source   = ncchar('pohang_rivers_discharge');
  nc.details  = ncchar('The pohang river');
...
nc('river') = river;
time_dimension = 'time';
nc(time_dimension) =ntime; % UNLIMITED
...
nc{theVarname} = ncfloat('river');
nc{theVarname}.long_name = ncchar('river XI-position');
nc{theVarname}.LwSrc_meaning = ncchar('i-index grid-cell of w-face source/sink');
nc{theVarname}.units = ncchar('non-dimensional');
...
nc{theVarname} = ncfloat('river');
nc{theVarname}.long_name = ncchar('river ETA-position');
nc{theVarname}.LwSrc_meaning = ncchar('j-index grid-cell of w-face source/sink');
nc{theVarname}.units = ncchar('non-dimensional');
nc{theVarname}.valid_min = 1;
...
nc{theVarname} = ncfloat('time','s_rho','river');
nc{theVarname}.long_name = ncchar('river runoff Chlo');
nc{theVarname}.units = ncchar('milligrams/m3');
nc{theVarname}.field = ncchar('river_Chlo, scalar, series');
nc{theVarname}.time = ncchar('river_time');
...
nc{theVarname} = ncfloat('time','s_rho','river');
nc{theVarname}.long_name = ncchar('river runoff LDeN');
nc{theVarname}.units = ncchar('microMole L-1');
nc{theVarname}.field = ncchar('river_LDeN, scalar, series');
nc{theVarname}.time = ncchar('river_time');

theVarname = 'river_SDeN';
nc{theVarname} = ncfloat('time','s_rho','river');
nc{theVarname}.long_name = ncchar('river runoff SDeN');
nc{theVarname}.units = ncchar('microMole L-1');
nc{theVarname}.field = ncchar('river_SDeN, scalar, series');
nc{theVarname}.time = ncchar('river_time');
