...
merg_prec = {}; mergmerg_prec_date_prec = {}; 
 for i = 1:length(list_file_txt)
      tempo_mat=[];
    tempo_mat=readmatrix(list_file_txt(i).name,'Delimiter',',');
    tempo_mat_d = tempo_mat(:,3);
        tempo_idx=[]; 
     for j = 1:length(pick_st)
        check_empty = 0;
        tempo_idx =find(tempo_mat(:,2) == pick_st(j));
        if sum(isnan(tempo_mat_d(tempo_idx))) > 0
            disp( [list_file_txt(i).name(end-7:end-4), ' point on ', num2str(j), ' code ', num2str(pick_st(j)), ' size ' num2str(sum(isnan(tempo_mat_d(tempo_idx)))) ]);
            if sum(isnan(tempo_mat_d(tempo_idx))) < 20
                ...
                if sum(isnan(tempo_mat_d(tempo_idx))) > 0
                    disp( [list_file_txt(i).name(end-7:end-4), ' point on ', num2str(j), ' code ', num2str(pick_st(j)), ' size ' num2str(sum(isnan(tempo_mat_d(tempo_idx)))) ,'!!! re-nan' ]);
                end
            end
            %% make it empty
            if sum(isnan(tempo_mat_d(tempo_idx))) > 0
        ...
        if check_empty ~= 1
            merg_prec{i,j} = tempo_mat_d(tempo_idx);
            merg_prec_date{i,j} = tempo_mat(tempo_idx,1);
            % warning for lack of data
            if length(tempo_mat_d(tempo_idx)) < 365
               disp([list_file_txt(i).name(end-7:end-4), ' point on ', num2str(j), ' code ', num2str(pick_st(j)), ' size ' num2str(sum(isnan(tempo_mat_d(tempo_idx)))) ,' has lack of length' ])
            end
 ...
 %% check 366x1 right or 365x1 right?
...
for i = t_year(1):t_year(end)
    ...
    clearvars tempo_temp tempo_trans
tempo_prec=merg_prec{t_year(order_i)-1979};
tempo_prec = movmean(tempo_prec,5);
tempo_trans = sj_dis.dis_pre_total{t_year(order_i)-1979}; %dis_pre_total
...
for i =t_year(1):t_year(end)
     order_i = order_i+1
train_prec{order_i,1} = movmean( merg_prec{t_year(order_i)-1979}',5);
...
for i =t_year(1):t_year(end)
     order_i = order_i+1
test_prec{order_i,1} =movmean( merg_prec{t_year(order_i)-1979}',5);
