...
for i = 1:length(mth_d_txt_c)
    if size(indx{i},1) == 0
        ...
        w_po4_mi(i) = nanmean(r_po4(indx{i})) - yp_w_po4(indx_366{i});   
        w_tn_mi(i) = nanmean(r_tn(indx{i})) - yp_w_tn(indx_366{i});   
    end
end
...
for i = 1:length(mth_d_txt_c)
        ...
        po4_day_clim(i) = yp_w_po4(indx_366{i});
        tn_day_clim(i) = yp_w_tn(indx_366{i});
end
...
for i = 1:length(ref_yymm_c)
    if size(indx_mth{i},1) == 0
        ...
        w_po4_mth(i) = nanmean(r_po4(indx_mth{i}));
    end
end
...
for i = 1:length(ref_yy_c)
    if size(indx_yy{i},1) == 0
        ...
        w_po4_yy(i) = nanmean(r_po4(indx_yy{i}));
    end
end

return
save('songjung_bio_v3_day_mon_yr.mat');
