...
for i = 1:10
    tx_tick(21+i) % if i=1, 2001 1/1 
    last_d_mon(i,:) = tx_tick(21+i)-1 + eom_d_each(21+i,:); % if i=1, 2001 : last day on each mon
    first_d_mon(i,1) = tx_tick(21+i);
...
do_sur(1:tx_tick(22)-1) = NaN; do_sur(tx_tick(31)+1:end) = NaN;
do_bot(1:tx_tick(22)-1) = NaN; do_bot(tx_tick(31)+1:end) = NaN;
no3_sur(1:tx_tick(22)-1) = NaN; no3_sur(tx_tick(31)+1:end) = NaN;
no3_bot(1:tx_tick(22)-1) = NaN; no3_bot(tx_tick(31)+1:end) = NaN;
temp_sur(1:tx_tick(22)-1) = NaN; temp_sur(tx_tick(31)+1:end) = NaN;
temp_bot(1:tx_tick(22)-1) = NaN; temp_bot(tx_tick(31)+1:end) = NaN;
salt_sur(1:tx_tick(22)-1) = NaN; salt_sur(tx_tick(31)+1:end) = NaN;
salt_bot(1:tx_tick(22)-1) = NaN; salt_bot(tx_tick(31)+1:end) = NaN;
...
regime_salt_b(regime_salt_b>40) = NaN;
...
for i = 1:length(mm_indx)
    if size(mm_indx{i},1) == 0     
        ...
        reg_clim_do_b(i) = nanmean(regime_do_b(mm_indx{i}));
        reg_clim_no3_b(i) = nanmean(regime_no3_b(mm_indx{i}));
        reg_clim_temp_b(i) = nanmean(regime_temp_b(mm_indx{i}));
        reg_clim_salt_b(i) = nanmean(regime_salt_b(mm_indx{i}));
        std_do(i) = nanstd(regime_do(mm_indx{i}));
        std_no3(i) = nanstd(regime_no3(mm_indx{i}));
        std_temp(i) = nanstd(regime_temp(mm_indx{i}));
        std_salt(i) = nanstd(regime_salt(mm_indx{i}));
        std_do_b(i) = nanstd(regime_do_b(mm_indx{i}));
        std_no3_b(i) = nanstd(regime_no3_b(mm_indx{i}));
        std_temp_b(i) = nanstd(regime_temp_b(mm_indx{i}));
        std_salt_b(i) = nanstd(regime_salt_b(mm_indx{i}));
    end
end
...
lon_20501 = 127.8083;
lat_20501 = 34.3717;
save('kodc_40016_2001_2010_just_mean_v2.mat','-v7.3');
...
temp_bb_do(367:948) = yCal_con_int_f(367:948);

temp_bb_do(949:1010) = NaN; % to make it natural
...
do_re_b = temp_bb_do;
save('do_b_kodc_input_400_16.mat','tx_tick','do_re_b');
