function create_roms_forcing_P(fname,Pair,time,yy,cycle)%,tframe,cycle)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
nw{var_time}(:)=time';
 ...
 nw{'Pair'} = ncfloat(var_time, 'eta_rho', 'xi_rho');
 nw{'Pair'}.long_name = ncchar('ECMWF mean sea level air pressure');
 nw{'Pair'}.units = ncchar('mbar');
 nw{'Pair'}.time = ncchar(var_time);
for i=1:1:yy
 nw{'Pair'}(i,:,:)= Pair(i,:,:);
end
 close(nw);
