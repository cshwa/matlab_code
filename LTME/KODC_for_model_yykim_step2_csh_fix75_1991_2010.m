...
for yearij = 1:length(inputyear)
    for monthij = 1:length(inputmonth)
        ...
        filename = strcat(filedir,'\', num2str(tempyear,'%04i'), '\', ...
                'yellow_monthly_', num2str(tempyear,'%04i'),'_',num2str(tempmonth,'%02i'), '.nc');
        elseif tempyear >= 2001
        filedir=['D:\장기생태\Dynamic\07_boundary_ts\yjtak_boundary'];
        filename = strcat(filedir,'\', num2str(tempyear,'%04i'), '\', ...
                'spinup1_monthly',num2str(tempmonth,'%02i'), '.nc');
%         ['D:\장기생태\Dynamic\07_boundary_ts\yjtak_boundary\',num2str(1990+i),'/spinup1_monthly',num2str(j,'%02d'),'.nc']      
        end
        % read model data
        if (exist('lon')==0)
...
save(['model_salt_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'_fix75.mat'], 'MODEL6', 'NIFS6');
