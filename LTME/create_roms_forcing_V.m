function create_roms_forcing_V(fname,Vwind,time,yy,cycle)%,tframe,cycle)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
nw{var_time}(:)=time';
 ...
 nw{'Vwind'} = ncfloat(var_time, 'eta_rho', 'xi_rho');
 nw{'Vwind'}.long_name = ncchar('ECMWF Ten meter V');
 nw{'Vwind'}.units = ncchar('m/s');
 nw{'Vwind'}.time = ncchar(var_time);
for i=1:1:yy
 nw{'Vwind'}(i,:,:)= Vwind(i,:,:);
end
 close(nw);
