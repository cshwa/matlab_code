...
%%% load file        
nc=netcdf(['./Gwangyang_add_uv_bry/','Gwangyang_new_Y',num2str(yeart),'.nc'],'w');
temp_s=nc{'temp_south'}(:);
temp_e=nc{'temp_east'}(:);

salt_s=nc{'salt_south'}(:);
salt_e=nc{'salt_east'}(:);
...
DO=permute(squeeze(do(:,:,:,:)),[4 3 2 1])*0.7*44.661;  %% mg/L to millimole_oxygen meter-3
NH4=permute(squeeze(nh4(:,:,:,:)),[4 3 2 1])/1000*1000/14; %% ug/L to millimole_N meter-3
NO3=permute(squeeze(no3(:,:,:,:)),[4 3 2 1])/1000*1000/14; %% ug/L to millimole_N meter-3
Chla=permute(squeeze(chla(:,:,:,:)),[4 3 2 1]); %% ug/L == mg/m^3
% t z y x
DO_s=squeeze(DO(:,:,1,:));
DO_e=squeeze(DO(:,:,:,end));

NH4_s=squeeze(NH4(:,:,1,:));
NH4_e=squeeze(NH4(:,:,:,end));

NO3_s=squeeze(NO3(:,:,1,:));
NO3_e=squeeze(NO3(:,:,:,end));

Chla_s=squeeze(Chla(:,:,1,:));
Chla_e=squeeze(Chla(:,:,:,end));

    

nc1=netcdf(bryname,'w');
nc1{'temp_south'}(:)=temp_s;
nc1{'temp_east'}(:)=temp_e;

nc1{'salt_south'}(:)=salt_s;
nc1{'salt_east'}(:)=salt_e;
