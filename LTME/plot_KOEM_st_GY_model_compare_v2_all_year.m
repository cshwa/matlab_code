...
mod_file='D:\Àå±â»ıÅÂ\Dynamic\result\2001\mpdata_3\merg_avg.nc';
...
check_NH4 = obs_nh4(:,eom_d_each(1,5)+1:eom_d_each(1,8),15:16)
...
%   for   i = 4:4
   if ind_out(i) == 1
       fig = figure; hold on;
        for k = 1:2 
            plot(check_NH4(i,:,k)./14,'r*','linew',1);  hold on;
%             xlim([1 365]);
        end
...
squeeze(nansum(check_NH4,1))  





%confirm when value is exist 
m_regime_chl =nanmean(regime_chl,1)
m_regime_no3 =nanmean(regime_no3,1)
m_regime_nh4 =nanmean(regime_nh4,1)
% 15~16th years (1997:2018 is full time) are 2011~2012. all values are
% exist(7mon)
% 6 =x, 7=2, 8=all, 9=x, 10=x
% August(8 mon) has all value for all year;
...
for k = 1:22 % num. of year.    
for i=1:12 % month %size(regime_no3,2)
    if i==1
       %surf
       obs_no3(j,1:eom_d_each(1,i),k) = regime_no3(j,(k-1)*12+i);
       obs_nh4(j,1:eom_d_each(1,i),k) = regime_nh4(j,(k-1)*12+i);
       obs_do(j,1:eom_d_each(1,i),k) = regime_do(j,(k-1)*12+i);
       obs_chl(j,1:eom_d_each(1,i),k) = regime_chl(j,(k-1)*12+i);
       obs_temp(j,1:eom_d_each(1,i),k) = regime_temp(j,(k-1)*12+i);
       obs_salt(j,1:eom_d_each(1,i),k) = regime_salt(j,(k-1)*12+i);
       % bot
       obs_no3_b(j,1:eom_d_each(1,i),k) = regime_no3_b(j,(k-1)*12+i);
       obs_nh4_b(j,1:eom_d_each(1,i),k) = regime_nh4_b(j,(k-1)*12+i);
       obs_do_b(j,1:eom_d_each(1,i),k) = regime_do_b(j,(k-1)*12+i);
       obs_chl_b(j,1:eom_d_each(1,i),k) = regime_chl_b(j,(k-1)*12+i);
       obs_temp_b(j,1:eom_d_each(1,i),k) = regime_temp_b(j,(k-1)*12+i);
       obs_salt_b(j,1:eom_d_each(1,i),k) = regime_salt_b(j,(k-1)*12+i);
    else
       obs_no3(j,eom_d_each(1,i-1)+1:eom_d_each(1,i),k) = regime_no3(j,(k-1)*12+i);
       obs_nh4(j,eom_d_each(1,i-1)+1:eom_d_each(1,i),k) = regime_nh4(j,(k-1)*12+i);
       obs_do(j,eom_d_each(1,i-1)+1:eom_d_each(1,i),k) = regime_do(j,(k-1)*12+i);
       obs_chl(j,eom_d_each(1,i-1)+1:eom_d_each(1,i),k) = regime_chl(j,(k-1)*12+i);
       obs_temp(j,eom_d_each(1,i-1)+1:eom_d_each(1,i),k) = regime_temp(j,(k-1)*12+i);
       obs_salt(j,eom_d_each(1,i-1)+1:eom_d_each(1,i),k) = regime_salt(j,(k-1)*12+i);
       % bot
       obs_no3_b(j,eom_d_each(1,i-1)+1:eom_d_each(1,i),k) = regime_no3_b(j,(k-1)*12+i);
       obs_nh4_b(j,eom_d_each(1,i-1)+1:eom_d_each(1,i),k) = regime_nh4_b(j,(k-1)*12+i);
       obs_do_b(j,eom_d_each(1,i-1)+1:eom_d_each(1,i),k) = regime_do_b(j,(k-1)*12+i);
       obs_chl_b(j,eom_d_each(1,i-1)+1:eom_d_each(1,i),k) = regime_chl_b(j,(k-1)*12+i);
       obs_temp_b(j,eom_d_each(1,i-1)+1:eom_d_each(1,i),k) = regime_temp_b(j,(k-1)*12+i);
       obs_salt_b(j,eom_d_each(1,i-1)+1:eom_d_each(1,i),k) = regime_salt_b(j,(k-1)*12+i);
    end
end
end
end
...
%   for   i = 4:4
   if ind_out(i) == 1
       fig = figure; hold on;
        ...
        for k = 1:22 
            plot(obs_no3(i,:,k)./14,'r-','linew',1); xlim([1 365]); hold on;
        end
...
for j = 1:22 % num. of year
for i = 1:365
    %std
    obs_std_gy_no3(i,j)=nanstd(squeeze(obs_gy_no3(:,i,j)));
    obs_std_gy_no3_b(i,j)=nanstd(squeeze(obs_gy_no3_b(:,i,j)));
    obs_std_gy_nh4(i,j)=nanstd(squeeze(obs_gy_nh4(:,i,j)));
    obs_std_gy_nh4_b(i,j)=nanstd(squeeze(obs_gy_nh4_b(:,i,j)));
    obs_std_gy_chl(i,j)=nanstd(squeeze(obs_gy_chl(:,i,j)));
    obs_std_gy_chl_b(i,j)=nanstd(squeeze(obs_gy_chl_b(:,i,j)));
    obs_std_gy_temp(i,j)=nanstd(squeeze(obs_gy_temp(:,i,j)));
    obs_std_gy_temp_b(i,j)=nanstd(squeeze(obs_gy_temp_b(:,i,j)));
    obs_std_gy_salt(i,j)=nanstd(squeeze(obs_gy_salt(:,i,j)));
    obs_std_gy_salt_b(i,j)=nanstd(squeeze(obs_gy_salt_b(:,i,j)));
    obs_std_gy_do(i,j)=nanstd(squeeze(obs_gy_do(:,i,j)));
    obs_std_gy_do_b(i,j)=nanstd(squeeze(obs_gy_do_b(:,i,j)));

    obs_std_sgy_no3(i,j)=nanstd(squeeze(obs_sgy_no3(:,i,j)));
    obs_std_sgy_no3_b(i,j)=nanstd(squeeze(obs_sgy_no3_b(:,i,j)));
    obs_std_sgy_nh4(i,j)=nanstd(squeeze(obs_sgy_nh4(:,i,j)));
    obs_std_sgy_nh4_b(i,j)=nanstd(squeeze(obs_sgy_nh4_b(:,i,j)));
    obs_std_sgy_chl(i,j)=nanstd(squeeze(obs_sgy_chl(:,i,j)));
    obs_std_sgy_chl_b(i,j)=nanstd(squeeze(obs_sgy_chl_b(:,i,j)));
    obs_std_sgy_temp(i,j)=nanstd(squeeze(obs_sgy_temp(:,i,j)));
    obs_std_sgy_temp_b(i,j)=nanstd(squeeze(obs_sgy_temp_b(:,i,j)));
    obs_std_sgy_salt(i,j)=nanstd(squeeze(obs_sgy_salt(:,i,j)));
    obs_std_sgy_salt_b(i,j)=nanstd(squeeze(obs_sgy_salt_b(:,i,j)));
    obs_std_sgy_do(i,j)=nanstd(squeeze(obs_sgy_do(:,i,j)));
    obs_std_sgy_do_b(i,j)=nanstd(squeeze(obs_sgy_do_b(:,i,j)));

    obs_std_egy_no3(i,j)=nanstd(squeeze(obs_egy_no3(:,i,j)));
    obs_std_egy_no3_b(i,j)=nanstd(squeeze(obs_egy_no3_b(:,i,j)));
    obs_std_egy_nh4(i,j)=nanstd(squeeze(obs_egy_nh4(:,i,j)));
    obs_std_egy_nh4_b(i,j)=nanstd(squeeze(obs_egy_nh4_b(:,i,j)));
    obs_std_egy_chl(i,j)=nanstd(squeeze(obs_egy_chl(:,i,j)));
    obs_std_egy_chl_b(i,j)=nanstd(squeeze(obs_egy_chl_b(:,i,j)));
    obs_std_egy_temp(i,j)=nanstd(squeeze(obs_egy_temp(:,i,j)));
    obs_std_egy_temp_b(i,j)=nanstd(squeeze(obs_egy_temp_b(:,i,j)));
    obs_std_egy_salt(i,j)=nanstd(squeeze(obs_egy_salt(:,i,j)));
    obs_std_egy_salt_b(i,j)=nanstd(squeeze(obs_egy_salt_b(:,i,j)));
    obs_std_egy_do(i,j)=nanstd(squeeze(obs_egy_do(:,i,j)));
    obs_std_egy_do_b(i,j)=nanstd(squeeze(obs_egy_do_b(:,i,j)));

    obs_std_jj_no3(i,j)=nanstd(squeeze(obs_jj_no3(:,i,j)));
    obs_std_jj_no3_b(i,j)=nanstd(squeeze(obs_jj_no3_b(:,i,j)));
    obs_std_jj_nh4(i,j)=nanstd(squeeze(obs_jj_nh4(:,i,j)));
    obs_std_jj_nh4_b(i,j)=nanstd(squeeze(obs_jj_nh4_b(:,i,j)));
    obs_std_jj_chl(i,j)=nanstd(squeeze(obs_jj_chl(:,i,j)));
    obs_std_jj_chl_b(i,j)=nanstd(squeeze(obs_jj_chl_b(:,i,j)));
    obs_std_jj_temp(i,j)=nanstd(squeeze(obs_jj_temp(:,i,j)));
    obs_std_jj_temp_b(i,j)=nanstd(squeeze(obs_jj_temp_b(:,i,j)));
    obs_std_jj_salt(i,j)=nanstd(squeeze(obs_jj_salt(:,i,j)));
    obs_std_jj_salt_b(i,j)=nanstd(squeeze(obs_jj_salt_b(:,i,j)));
    obs_std_jj_do(i,j)=nanstd(squeeze(obs_jj_do(:,i,j)));
    obs_std_jj_do_b(i,j)=nanstd(squeeze(obs_jj_do_b(:,i,j)));
    
    %mean
    obm_gy_no3(i,j)=nanmean(squeeze(obs_gy_no3(:,i,j)));
    obm_gy_no3_b(i,j)=nanmean(squeeze(obs_gy_no3_b(:,i,j)));
    obm_gy_nh4(i,j)=nanmean(squeeze(obs_gy_nh4(:,i,j)));
    obm_gy_nh4_b(i,j)=nanmean(squeeze(obs_gy_nh4_b(:,i,j)));
    obm_gy_chl(i,j)=nanmean(squeeze(obs_gy_chl(:,i,j)));
    obm_gy_chl_b(i,j)=nanmean(squeeze(obs_gy_chl_b(:,i,j)));
    obm_gy_temp(i,j)=nanmean(squeeze(obs_gy_temp(:,i,j)));
    obm_gy_temp_b(i,j)=nanmean(squeeze(obs_gy_temp_b(:,i,j)));
    obm_gy_salt(i,j)=nanmean(squeeze(obs_gy_salt(:,i,j)));
    obm_gy_salt_b(i,j)=nanmean(squeeze(obs_gy_salt_b(:,i,j)));
    obm_gy_do(i,j)=nanmean(squeeze(obs_gy_do(:,i,j)));
    obm_gy_do_b(i,j)=nanmean(squeeze(obs_gy_do_b(:,i,j)));

    obm_sgy_no3(i,j)=nanmean(squeeze(obs_sgy_no3(:,i,j)));
    obm_sgy_no3_b(i,j)=nanmean(squeeze(obs_sgy_no3_b(:,i,j)));
    obm_sgy_nh4(i,j)=nanmean(squeeze(obs_sgy_nh4(:,i,j)));
    obm_sgy_nh4_b(i,j)=nanmean(squeeze(obs_sgy_nh4_b(:,i,j)));
    obm_sgy_chl(i,j)=nanmean(squeeze(obs_sgy_chl(:,i,j)));
    obm_sgy_chl_b(i,j)=nanmean(squeeze(obs_sgy_chl_b(:,i,j)));
    obm_sgy_temp(i,j)=nanmean(squeeze(obs_sgy_temp(:,i,j)));
    obm_sgy_temp_b(i,j)=nanmean(squeeze(obs_sgy_temp_b(:,i,j)));
    obm_sgy_salt(i,j)=nanmean(squeeze(obs_sgy_salt(:,i,j)));
    obm_sgy_salt_b(i,j)=nanmean(squeeze(obs_sgy_salt_b(:,i,j)));
    obm_sgy_do(i,j)=nanmean(squeeze(obs_sgy_do(:,i,j)));
    obm_sgy_do_b(i,j)=nanmean(squeeze(obs_sgy_do_b(:,i,j)));

    obm_egy_no3(i,j)=nanmean(squeeze(obs_egy_no3(:,i,j)));
    obm_egy_no3_b(i,j)=nanmean(squeeze(obs_egy_no3_b(:,i,j)));
    obm_egy_nh4(i,j)=nanmean(squeeze(obs_egy_nh4(:,i,j)));
    obm_egy_nh4_b(i,j)=nanmean(squeeze(obs_egy_nh4_b(:,i,j)));
    obm_egy_chl(i,j)=nanmean(squeeze(obs_egy_chl(:,i,j)));
    obm_egy_chl_b(i,j)=nanmean(squeeze(obs_egy_chl_b(:,i,j)));
    obm_egy_temp(i,j)=nanmean(squeeze(obs_egy_temp(:,i,j)));
    obm_egy_temp_b(i,j)=nanmean(squeeze(obs_egy_temp_b(:,i,j)));
    obm_egy_salt(i,j)=nanmean(squeeze(obs_egy_salt(:,i,j)));
    obm_egy_salt_b(i,j)=nanmean(squeeze(obs_egy_salt_b(:,i,j)));
    obm_egy_do(i,j)=nanmean(squeeze(obs_egy_do(:,i,j)));
    obm_egy_do_b(i,j)=nanmean(squeeze(obs_egy_do_b(:,i,j)));

    obm_jj_no3(i,j)=nanmean(squeeze(obs_jj_no3(:,i,j)));
    obm_jj_no3_b(i,j)=nanmean(squeeze(obs_jj_no3_b(:,i,j)));
    obm_jj_nh4(i,j)=nanmean(squeeze(obs_jj_nh4(:,i,j)));
    obm_jj_nh4_b(i,j)=nanmean(squeeze(obs_jj_nh4_b(:,i,j)));
    obm_jj_chl(i,j)=nanmean(squeeze(obs_jj_chl(:,i,j)));
    obm_jj_chl_b(i,j)=nanmean(squeeze(obs_jj_chl_b(:,i,j)));
    obm_jj_temp(i,j)=nanmean(squeeze(obs_jj_temp(:,i,j)));
    obm_jj_temp_b(i,j)=nanmean(squeeze(obs_jj_temp_b(:,i,j)));
    obm_jj_salt(i,j)=nanmean(squeeze(obs_jj_salt(:,i,j)));
    obm_jj_salt_b(i,j)=nanmean(squeeze(obs_jj_salt_b(:,i,j)));
    obm_jj_do(i,j)=nanmean(squeeze(obs_jj_do(:,i,j)));
    obm_jj_do_b(i,j)=nanmean(squeeze(obs_jj_do_b(:,i,j)));
...
fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(gy_no3,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_gy_no3(:,i)./14,'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_gy_no3(:,i)),obm_gy_no3(:,i)./14 + obs_std_gy_no3(:,i)./14,'m-','linew',1);hold on;
        plot(1:length(obm_gy_no3(:,i)),obm_gy_no3(:,i)./14 - obs_std_gy_no3(:,i)./14,'m-','linew',1);hold on;
end
        title(['GY 1997~2018 daily KOEM OBS vs. MODEL NO3']);
        xlabel('time(days)','fontsize',13)
        ylabel('NO3 (umol/m^3)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_no3'),'-dpng')

fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(gy_nh4,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_gy_nh4(:,i)./14,'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_gy_nh4(:,i)),obm_gy_nh4(:,i)./14 + obs_std_gy_nh4(:,i)./14,'m-','linew',1);hold on;
        plot(1:length(obm_gy_nh4(:,i)),obm_gy_nh4(:,i)./14 - obs_std_gy_nh4(:,i)./14,'m-','linew',1);hold on;
end
        title(['GY 1997~2018 daily KOEM OBS vs. MODEL nh4']);
        xlabel('time(days)','fontsize',13)
        ylabel('nh4 (umol/m^3)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_nh4'),'-dpng')
        
        
fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(gy_chl,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_gy_chl(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_gy_chl(:,i)),obm_gy_chl(:,i) + obs_std_gy_chl(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_gy_chl(:,i)),obm_gy_chl(:,i) - obs_std_gy_chl(:,i),'m-','linew',1);hold on;
end
        title(['GY 1997~2018 daily KOEM OBS vs. MODEL chl']);
        xlabel('time(days)','fontsize',13)
        ylabel('chl (ug/L)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_chl'),'-dpng')
        
        
 fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(gy_temp,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_gy_temp(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_gy_temp(:,i)),obm_gy_temp(:,i) + obs_std_gy_temp(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_gy_temp(:,i)),obm_gy_temp(:,i) - obs_std_gy_temp(:,i),'m-','linew',1);hold on;
end
        title(['GY 1997~2018 daily KOEM OBS vs. MODEL temp']);
        xlabel('time(days)','fontsize',13)
        ylabel('temp (o^C)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_temp'),'-dpng')
        
        
fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(gy_salt,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_gy_salt(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_gy_salt(:,i)),obm_gy_salt(:,i) + obs_std_gy_salt(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_gy_salt(:,i)),obm_gy_salt(:,i) - obs_std_gy_salt(:,i),'m-','linew',1);hold on;
end
        title(['GY 1997~2018 daily KOEM OBS vs. MODEL salt']);
        xlabel('time(days)','fontsize',13)
        ylabel('salt (psu)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_salt'),'-dpng')      
 %% bot
 
 fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(gy_no3_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_gy_no3_b(:,i)./14,'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_gy_no3_b(:,i)),obm_gy_no3_b(:,i)./14 + obs_std_gy_no3_b(:,i)./14,'m-','linew',1);hold on;
        plot(1:length(obm_gy_no3_b(:,i)),obm_gy_no3_b(:,i)./14 - obs_std_gy_no3_b(:,i)./14,'m-','linew',1);hold on;
end
        title(['GY 1997~2018 daily KOEM OBS vs. MODEL NO3 bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('NO3 (umol/m^3)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_no3_bot'),'-dpng')

fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(gy_nh4_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_gy_nh4_b(:,i)./14,'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_gy_nh4_b(:,i)),obm_gy_nh4_b(:,i)./14 + obs_std_gy_nh4_b(:,i)./14,'m-','linew',1);hold on;
        plot(1:length(obm_gy_nh4_b(:,i)),obm_gy_nh4_b(:,i)./14 - obs_std_gy_nh4_b(:,i)./14,'m-','linew',1);hold on;
end
        title(['GY 1997~2018 daily KOEM OBS vs. MODEL nh4 bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('nh4 (umol/m^3)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_nh4_bot'),'-dpng')
        
        
fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(gy_chl_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_gy_chl_b(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_gy_chl_b(:,i)),obm_gy_chl_b(:,i) + obs_std_gy_chl_b(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_gy_chl_b(:,i)),obm_gy_chl_b(:,i) - obs_std_gy_chl_b(:,i),'m-','linew',1);hold on;
end
        title(['GY 1997~2018 daily KOEM OBS vs. MODEL chl bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('chl (ug/L)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_chl_bot'),'-dpng')
        
        
 fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(gy_temp_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_gy_temp_b(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_gy_temp_b(:,i)),obm_gy_temp_b(:,i) + obs_std_gy_temp_b(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_gy_temp_b(:,i)),obm_gy_temp_b(:,i) - obs_std_gy_temp_b(:,i),'m-','linew',1);hold on;
end
        title(['GY 1997~2018 daily KOEM OBS vs. MODEL temp bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('temp (o^C)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_temp_bot'),'-dpng')
        
        
fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(gy_salt_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_gy_salt_b(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_gy_salt_b(:,i)),obm_gy_salt_b(:,i) + obs_std_gy_salt_b(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_gy_salt_b(:,i)),obm_gy_salt_b(:,i) - obs_std_gy_salt_b(:,i),'m-','linew',1);hold on;
end
        title(['GY 1997~2018 daily KOEM OBS vs. MODEL salt bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('salt (psu)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_salt_bot'),'-dpng')
...
fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(sgy_no3,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_sgy_no3(:,i)./14,'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_sgy_no3(:,i)),obm_sgy_no3(:,i)./14 + obs_std_sgy_no3(:,i)./14,'m-','linew',1);hold on;
        plot(1:length(obm_sgy_no3(:,i)),obm_sgy_no3(:,i)./14 - obs_std_sgy_no3(:,i)./14,'m-','linew',1);hold on;
end
        title(['sgy 1997~2018 daily KOEM OBS vs. MODEL NO3']);
        xlabel('time(days)','fontsize',13)
        ylabel('NO3 (umol/m^3)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_no3_south_gy'),'-dpng')

fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(sgy_nh4,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_sgy_nh4(:,i)./14,'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_sgy_nh4(:,i)),obm_sgy_nh4(:,i)./14 + obs_std_sgy_nh4(:,i)./14,'m-','linew',1);hold on;
        plot(1:length(obm_sgy_nh4(:,i)),obm_sgy_nh4(:,i)./14 - obs_std_sgy_nh4(:,i)./14,'m-','linew',1);hold on;
end
        title(['sgy 1997~2018 daily KOEM OBS vs. MODEL nh4']);
        xlabel('time(days)','fontsize',13)
        ylabel('nh4 (umol/m^3)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_nh4_south_gy'),'-dpng')
        
        
fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(sgy_chl,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_sgy_chl(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_sgy_chl(:,i)),obm_sgy_chl(:,i) + obs_std_sgy_chl(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_sgy_chl(:,i)),obm_sgy_chl(:,i) - obs_std_sgy_chl(:,i),'m-','linew',1);hold on;
end
        title(['sgy 1997~2018 daily KOEM OBS vs. MODEL chl']);
        xlabel('time(days)','fontsize',13)
        ylabel('chl (ug/L)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_chl_south_gy'),'-dpng')
        
        
 fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(sgy_temp,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_sgy_temp(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_sgy_temp(:,i)),obm_sgy_temp(:,i) + obs_std_sgy_temp(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_sgy_temp(:,i)),obm_sgy_temp(:,i) - obs_std_sgy_temp(:,i),'m-','linew',1);hold on;
end
        title(['sgy 1997~2018 daily KOEM OBS vs. MODEL temp']);
        xlabel('time(days)','fontsize',13)
        ylabel('temp (o^C)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_temp_south_gy'),'-dpng')
        
        
fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(sgy_salt,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_sgy_salt(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_sgy_salt(:,i)),obm_sgy_salt(:,i) + obs_std_sgy_salt(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_sgy_salt(:,i)),obm_sgy_salt(:,i) - obs_std_sgy_salt(:,i),'m-','linew',1);hold on;
end
        title(['sgy 1997~2018 daily KOEM OBS vs. MODEL salt']);
        xlabel('time(days)','fontsize',13)
        ylabel('salt (psu)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_salt_south_gy'),'-dpng')      
 %% bot
 
 fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(sgy_no3_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_sgy_no3_b(:,i)./14,'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_sgy_no3_b(:,i)),obm_sgy_no3_b(:,i)./14 + obs_std_sgy_no3_b(:,i)./14,'m-','linew',1);hold on;
        plot(1:length(obm_sgy_no3_b(:,i)),obm_sgy_no3_b(:,i)./14 - obs_std_sgy_no3_b(:,i)./14,'m-','linew',1);hold on;
end
        title(['sgy 1997~2018 daily KOEM OBS vs. MODEL NO3 bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('NO3 (umol/m^3)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_no3_bot_south_gy'),'-dpng')

fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(sgy_nh4_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_sgy_nh4_b(:,i)./14,'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_sgy_nh4_b(:,i)),obm_sgy_nh4_b(:,i)./14 + obs_std_sgy_nh4_b(:,i)./14,'m-','linew',1);hold on;
        plot(1:length(obm_sgy_nh4_b(:,i)),obm_sgy_nh4_b(:,i)./14 - obs_std_sgy_nh4_b(:,i)./14,'m-','linew',1);hold on;
end
        title(['sgy 1997~2018 daily KOEM OBS vs. MODEL nh4 bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('nh4 (umol/m^3)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_nh4_bot_south_gy'),'-dpng')
        
        
fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(sgy_chl_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_sgy_chl_b(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_sgy_chl_b(:,i)),obm_sgy_chl_b(:,i) + obs_std_sgy_chl_b(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_sgy_chl_b(:,i)),obm_sgy_chl_b(:,i) - obs_std_sgy_chl_b(:,i),'m-','linew',1);hold on;
end
        title(['sgy 1997~2018 daily KOEM OBS vs. MODEL chl bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('chl (ug/L)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_chl_bot_south_gy'),'-dpng')
        
        
 fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(sgy_temp_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_sgy_temp_b(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_sgy_temp_b(:,i)),obm_sgy_temp_b(:,i) + obs_std_sgy_temp_b(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_sgy_temp_b(:,i)),obm_sgy_temp_b(:,i) - obs_std_sgy_temp_b(:,i),'m-','linew',1);hold on;
end
        title(['sgy 1997~2018 daily KOEM OBS vs. MODEL temp bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('temp (o^C)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_temp_bot_south_gy'),'-dpng')
        
        
fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(sgy_salt_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_sgy_salt_b(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_sgy_salt_b(:,i)),obm_sgy_salt_b(:,i) + obs_std_sgy_salt_b(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_sgy_salt_b(:,i)),obm_sgy_salt_b(:,i) - obs_std_sgy_salt_b(:,i),'m-','linew',1);hold on;
end
        title(['sgy 1997~2018 daily KOEM OBS vs. MODEL salt bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('salt (psu)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_salt_bot_south_gy'),'-dpng')    
        
%%east gy plot

fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(egy_no3,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_egy_no3(:,i)./14,'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_egy_no3(:,i)),obm_egy_no3(:,i)./14 + obs_std_egy_no3(:,i)./14,'m-','linew',1);hold on;
        plot(1:length(obm_egy_no3(:,i)),obm_egy_no3(:,i)./14 - obs_std_egy_no3(:,i)./14,'m-','linew',1);hold on;
end
        title(['egy 1997~2018 daily KOEM OBS vs. MODEL NO3']);
        xlabel('time(days)','fontsize',13)
        ylabel('NO3 (umol/m^3)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_no3_east_gy'),'-dpng')

fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(egy_nh4,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_egy_nh4(:,i)./14,'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_egy_nh4(:,i)),obm_egy_nh4(:,i)./14 + obs_std_egy_nh4(:,i)./14,'m-','linew',1);hold on;
        plot(1:length(obm_egy_nh4(:,i)),obm_egy_nh4(:,i)./14 - obs_std_egy_nh4(:,i)./14,'m-','linew',1);hold on;
end
        title(['egy 1997~2018 daily KOEM OBS vs. MODEL nh4']);
        xlabel('time(days)','fontsize',13)
        ylabel('nh4 (umol/m^3)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_nh4_east_gy'),'-dpng')
        
        
fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(egy_chl,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_egy_chl(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_egy_chl(:,i)),obm_egy_chl(:,i) + obs_std_egy_chl(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_egy_chl(:,i)),obm_egy_chl(:,i) - obs_std_egy_chl(:,i),'m-','linew',1);hold on;
end
        title(['egy 1997~2018 daily KOEM OBS vs. MODEL chl']);
        xlabel('time(days)','fontsize',13)
        ylabel('chl (ug/L)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_chl_east_gy'),'-dpng')
        
        
 fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(egy_temp,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_egy_temp(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_egy_temp(:,i)),obm_egy_temp(:,i) + obs_std_egy_temp(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_egy_temp(:,i)),obm_egy_temp(:,i) - obs_std_egy_temp(:,i),'m-','linew',1);hold on;
end
        title(['egy 1997~2018 daily KOEM OBS vs. MODEL temp']);
        xlabel('time(days)','fontsize',13)
        ylabel('temp (o^C)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_temp_east_gy'),'-dpng')
        
        
fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(egy_salt,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_egy_salt(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_egy_salt(:,i)),obm_egy_salt(:,i) + obs_std_egy_salt(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_egy_salt(:,i)),obm_egy_salt(:,i) - obs_std_egy_salt(:,i),'m-','linew',1);hold on;
end
        title(['egy 1997~2018 daily KOEM OBS vs. MODEL salt']);
        xlabel('time(days)','fontsize',13)
        ylabel('salt (psu)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_salt_east_gy'),'-dpng')      
 %% bot
 
 fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(egy_no3_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_egy_no3_b(:,i)./14,'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_egy_no3_b(:,i)),obm_egy_no3_b(:,i)./14 + obs_std_egy_no3_b(:,i)./14,'m-','linew',1);hold on;
        plot(1:length(obm_egy_no3_b(:,i)),obm_egy_no3_b(:,i)./14 - obs_std_egy_no3_b(:,i)./14,'m-','linew',1);hold on;
end
        title(['egy 1997~2018 daily KOEM OBS vs. MODEL NO3 bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('NO3 (umol/m^3)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_no3_bot_east_gy'),'-dpng')

fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(egy_nh4_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_egy_nh4_b(:,i)./14,'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_egy_nh4_b(:,i)),obm_egy_nh4_b(:,i)./14 + obs_std_egy_nh4_b(:,i)./14,'m-','linew',1);hold on;
        plot(1:length(obm_egy_nh4_b(:,i)),obm_egy_nh4_b(:,i)./14 - obs_std_egy_nh4_b(:,i)./14,'m-','linew',1);hold on;
end
        title(['egy 1997~2018 daily KOEM OBS vs. MODEL nh4 bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('nh4 (umol/m^3)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_nh4_bot_east_gy'),'-dpng')
        
        
fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(egy_chl_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_egy_chl_b(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_egy_chl_b(:,i)),obm_egy_chl_b(:,i) + obs_std_egy_chl_b(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_egy_chl_b(:,i)),obm_egy_chl_b(:,i) - obs_std_egy_chl_b(:,i),'m-','linew',1);hold on;
end
        title(['egy 1997~2018 daily KOEM OBS vs. MODEL chl bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('chl (ug/L)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_chl_bot_east_gy'),'-dpng')
        
        
 fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(egy_temp_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_egy_temp_b(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_egy_temp_b(:,i)),obm_egy_temp_b(:,i) + obs_std_egy_temp_b(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_egy_temp_b(:,i)),obm_egy_temp_b(:,i) - obs_std_egy_temp_b(:,i),'m-','linew',1);hold on;
end
        title(['egy 1997~2018 daily KOEM OBS vs. MODEL temp bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('temp (o^C)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_temp_bot_east_gy'),'-dpng')
        
        
fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(egy_salt_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_egy_salt_b(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_egy_salt_b(:,i)),obm_egy_salt_b(:,i) + obs_std_egy_salt_b(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_egy_salt_b(:,i)),obm_egy_salt_b(:,i) - obs_std_egy_salt_b(:,i),'m-','linew',1);hold on;
end
        title(['egy 1997~2018 daily KOEM OBS vs. MODEL salt bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('salt (psu)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_salt_bot_east_gy'),'-dpng')
...
fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(jj_no3,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_jj_no3(:,i)./14,'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_jj_no3(:,i)),obm_jj_no3(:,i)./14 + obs_std_jj_no3(:,i)./14,'m-','linew',1);hold on;
        plot(1:length(obm_jj_no3(:,i)),obm_jj_no3(:,i)./14 - obs_std_jj_no3(:,i)./14,'m-','linew',1);hold on;
end
        title(['jj 1997~2018 daily KOEM OBS vs. MODEL NO3']);
        xlabel('time(days)','fontsize',13)
        ylabel('NO3 (umol/m^3)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_no3_jinju'),'-dpng')

fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(jj_nh4,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_jj_nh4(:,i)./14,'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_jj_nh4(:,i)),obm_jj_nh4(:,i)./14 + obs_std_jj_nh4(:,i)./14,'m-','linew',1);hold on;
        plot(1:length(obm_jj_nh4(:,i)),obm_jj_nh4(:,i)./14 - obs_std_jj_nh4(:,i)./14,'m-','linew',1);hold on;
end
        title(['jj 1997~2018 daily KOEM OBS vs. MODEL nh4']);
        xlabel('time(days)','fontsize',13)
        ylabel('nh4 (umol/m^3)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_nh4_jinju'),'-dpng')
        
        
fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(jj_chl,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_jj_chl(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_jj_chl(:,i)),obm_jj_chl(:,i) + obs_std_jj_chl(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_jj_chl(:,i)),obm_jj_chl(:,i) - obs_std_jj_chl(:,i),'m-','linew',1);hold on;
end
        title(['jj 1997~2018 daily KOEM OBS vs. MODEL chl']);
        xlabel('time(days)','fontsize',13)
        ylabel('chl (ug/L)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_chl_jinju'),'-dpng')
        
        
 fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(jj_temp,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_jj_temp(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_jj_temp(:,i)),obm_jj_temp(:,i) + obs_std_jj_temp(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_jj_temp(:,i)),obm_jj_temp(:,i) - obs_std_jj_temp(:,i),'m-','linew',1);hold on;
end
        title(['jj 1997~2018 daily KOEM OBS vs. MODEL temp']);
        xlabel('time(days)','fontsize',13)
        ylabel('temp (o^C)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_temp_jinju'),'-dpng')
        
        
fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(jj_salt,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_jj_salt(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_jj_salt(:,i)),obm_jj_salt(:,i) + obs_std_jj_salt(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_jj_salt(:,i)),obm_jj_salt(:,i) - obs_std_jj_salt(:,i),'m-','linew',1);hold on;
end
        title(['jj 1997~2018 daily KOEM OBS vs. MODEL salt']);
        xlabel('time(days)','fontsize',13)
        ylabel('salt (psu)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_salt_jinju'),'-dpng')      
 %% bot
 
 fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(jj_no3_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_jj_no3_b(:,i)./14,'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_jj_no3_b(:,i)),obm_jj_no3_b(:,i)./14 + obs_std_jj_no3_b(:,i)./14,'m-','linew',1);hold on;
        plot(1:length(obm_jj_no3_b(:,i)),obm_jj_no3_b(:,i)./14 - obs_std_jj_no3_b(:,i)./14,'m-','linew',1);hold on;
end
        title(['jj 1997~2018 daily KOEM OBS vs. MODEL NO3 bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('NO3 (umol/m^3)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_no3_bot_jinju'),'-dpng')

fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(jj_nh4_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_jj_nh4_b(:,i)./14,'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_jj_nh4_b(:,i)),obm_jj_nh4_b(:,i)./14 + obs_std_jj_nh4_b(:,i)./14,'m-','linew',1);hold on;
        plot(1:length(obm_jj_nh4_b(:,i)),obm_jj_nh4_b(:,i)./14 - obs_std_jj_nh4_b(:,i)./14,'m-','linew',1);hold on;
end
        title(['jj 1997~2018 daily KOEM OBS vs. MODEL nh4 bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('nh4 (umol/m^3)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_nh4_bot_jinju'),'-dpng')
        
        
fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(jj_chl_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_jj_chl_b(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_jj_chl_b(:,i)),obm_jj_chl_b(:,i) + obs_std_jj_chl_b(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_jj_chl_b(:,i)),obm_jj_chl_b(:,i) - obs_std_jj_chl_b(:,i),'m-','linew',1);hold on;
end
        title(['jj 1997~2018 daily KOEM OBS vs. MODEL chl bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('chl (ug/L)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_chl_bot_jinju'),'-dpng')
        
        
 fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(jj_temp_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_jj_temp_b(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_jj_temp_b(:,i)),obm_jj_temp_b(:,i) + obs_std_jj_temp_b(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_jj_temp_b(:,i)),obm_jj_temp_b(:,i) - obs_std_jj_temp_b(:,i),'m-','linew',1);hold on;
end
        title(['jj 1997~2018 daily KOEM OBS vs. MODEL temp bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('temp (o^C)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_temp_bot_jinju'),'-dpng')
        
        
fig = figure; hold on;
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        plot(jj_salt_b,'b','linew',2); hold on;
for i = 1:22 
        plot(obm_jj_salt_b(:,i),'r-','linew',1); xlim([1 365]); hold on;
        plot(1:length(obm_jj_salt_b(:,i)),obm_jj_salt_b(:,i) + obs_std_jj_salt_b(:,i),'m-','linew',1);hold on;
        plot(1:length(obm_jj_salt_b(:,i)),obm_jj_salt_b(:,i) - obs_std_jj_salt_b(:,i),'m-','linew',1);hold on;
end
        title(['jj 1997~2018 daily KOEM OBS vs. MODEL salt bot']);
        xlabel('time(days)','fontsize',13)
        ylabel('salt (psu)','fontsize',13)
        grid on
        set(gca,'fontsize',13)
        print(fig,strcat('1997~2018_daily_KOEM_OBS_vs_MODEL_salt_bot_jinju'),'-dpng')
...
%         plot(zeros(365,1),'g--','linew',2); % zero lines
        ...
        for k = 1:22 
            plot(obs_no3(i,:,k),'r-','linew',1); xlim([1 365]); hold on;
        end
        title([name_tag{i},'- 2001 daily KOEM OBS vs. MODEL NO3']);
        xlabel('time(days on 2001)','fontsize',13)
        ylabel('NO3 (umol/m^3)','fontsize',13)
