function create_roms_file_month2_w(fname,temp,salt,zeta,latent,sensible,swrad,ubar,vbar,u,v,w)%,tframe,cycle)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%  nw{'zeta'}.time = ncchar('level');
 ...
 % case 'latent'
 nw{'latent'} = ncfloat(  'eta_rho', 'xi_rho');
 nw{'latent'}.long_name = ncchar('time-averaged latent heat flux');
 nw{'latent'}.units = ncchar('Watt m^-2');
 nw{'latent'}(:)= latent;
 
 % case 'sensible'
 nw{'sensible'} = ncfloat(  'eta_rho', 'xi_rho');
 nw{'sensible'}.long_name = ncchar('time-averaged sensible heat flux');
 nw{'sensible'}.units = ncchar('Watt m^-2');
%  nw{'sensible'}.time = ncchar('level');
 nw{'sensible'}(:)= sensible;
 
 % case 'swrad'
 nw{'swrad'} = ncfloat(  'eta_rho', 'xi_rho');
 nw{'swrad'}.long_name = ncchar('time-averaged shortwave radiation heat flux');
 nw{'swrad'}.units = ncchar('Watt m^-2');
 nw{'swrad'}(:)= swrad;
...
% case 'w'
 nw{'w'} = ncfloat('s_w',  'eta_rho', 'xi_rho');
 nw{'w'}.long_name = ncchar('time-averaged w-momentum component');
 nw{'w'}.units = ncchar('meter second-1');
 nw{'w'}(:)= w;
