...
hanriv=load('han(hangang)_1996_2017.txt');
keumriv=load('keum(kyuam)_1996_2017.dat');

% riv2= riv(12:end-1,:);
riv2= riv ;
riv3_yr= reshape(riv2(:,1),12,length(riv2)/12)+30;
riv3_riv= reshape(riv2(:,2),12,length(riv2)/12);

hriv2= hanriv ;
hriv3_yr= reshape(hriv2(:,1),12,length(hriv2)/12);
hriv3_riv= reshape(hriv2(:,2),12,length(hriv2)/12);

kriv2= keumriv ;
kriv3_yr= reshape(kriv2(:,1),12,length(kriv2)/12);
kriv3_riv= reshape(kriv2(:,2),12,length(kriv2)/12);
...
hidx = find(floor(hriv3_yr(1,:)) == year(i));
hdis = hriv3_riv(:,hidx);

kidx = find(floor(kriv3_yr(1,:)) == year(i));
kdis = kriv3_riv(:,kidx);
...
season_cycle=[ 378  296  299  209  216  271  907  1481  1345  1084  619  440 ];
...
% roms grid
grd=grd('auto8');
grd_file=grd.grd_file;
% grd_file='roms_grd_auto_rdrg2_new8_smooth.nc';
Vtransform=2;
Vstretching=4;
theta_s=grd.theta_s;
theta_b=grd.theta_b;
...
h=grd.h
N=grd.N
% theta_s=ncread('D:\장기생태\yellow_sea\roms_ini_auto_fennel_po4_woa13jan_new8.nc','theta_s');
% theta_b=ncread('D:\장기생태\yellow_sea\roms_ini_auto_fennel_po4_woa13jan_new8.nc','theta_b');
% hc=ncread('D:\장기생태\yellow_sea\roms_ini_auto_fennel_po4_woa13jan_new8.nc','hc');
% h=ncread('D:\장기생태\yellow_sea\roms_grd_auto_rdrg2_new8_smooth.nc','h');
% N=40;
depth_z=zlevs(h,0*h,theta_s,theta_b,hc,N,Vtransform,Vstretching,'w');
hz=depth_z(2:N+1,:,:)-depth_z(1:N,:,:);
...
foot='_yangtze80s_po4_8rivers.nc';
Fname = [head,mid,foot];
...
for r=1:Nrivers  
   River.vshape(r,1:N)=hz(:,River.Ypos(r),River.Xpos(r))/h(River.Ypos(r),River.Xpos(r));  % 두께 고려하여 vshape 나눔.
