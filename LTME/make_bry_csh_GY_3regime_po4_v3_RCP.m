...
period = {'2021to2030','2031to2040','2041to2050', '2051to2060',...
    '2061to2070','2071to2080','2081to2090','2091to2100'};

for i = 1:length(period)
    yeart=i;
% bryname=['bndy_auto_NWP_1_10_test6_clim_',num2str(i),'regime.nc'];
bryname=['bndy_csh_RCM_clim_test66_',char(period{i}),'.nc'];
...
load(['D:\장기생태\Dynamic\KOEM\boundary_bio_input_to06to15_3regime_v7.mat']);
do_tem=do_input;
...
% load RCP delta bio-value
load('IPSL-CM5A-MR_boundary_delta_t_rcp85_bio.mat');


for var_name_pre = {'no3','chl','nh4','o2','po4'}
    clearvars var_name
    var_name = char(var_name_pre);
    if strcmp(var_name, 'chl');
        disp([var_name, ' unit conversion']);
eval(['bnd_', var_name,'_e_in = permute(bnd_', var_name,'_',num2str(i+2),'_e_t,[3 2 1]) .* 10^6;']); % kg/m^3 to ug/L
eval(['bnd_', var_name,'_s_in = permute(bnd_', var_name,'_',num2str(i+2),'_s_t,[3 2 1]) .* 10^6;']); % kg/m^3 to ug/L
    else
eval(['bnd_', var_name,'_e_in = permute(bnd_', var_name,'_',num2str(i+2),'_e_t,[3 2 1]) .* 1000;']); % mol/m^3 to mmol/m^3
eval(['bnd_', var_name,'_s_in = permute(bnd_', var_name,'_',num2str(i+2),'_s_t,[3 2 1]) .* 1000;']); % mol/m^3 to mmol/m^3
    end
end
...
nc1{'NO3_east'}(:)=(no3_we + bnd_no3_e_in);
...
nc1{'NO3_south'}(:)=(no3_sn + bnd_no3_s_in);
...
nc1{'NH4_east'}(:)=(nh4_we + bnd_nh4_e_in);
...
nc1{'NH4_south'}(:)=(nh4_sn + bnd_nh4_s_in);
...
nc1{'tPO4_south'}(:)=(po4_sn + bnd_po4_s_in);
...
nc1{'tPO4_east'}(:)=(po4_we + bnd_po4_e_in);
...
nc1{'oxygen_south'}(:)=(do_sn + bnd_o2_s_in);
...
nc1{'oxygen_east'}(:)=(do_we + bnd_o2_e_in);
...
nc1{'chlo_east'}(:)=(chl_we + bnd_chl_e_in);
...
nc1{'chlo_south'}(:)=(chl_sn + bnd_chl_s_in);
