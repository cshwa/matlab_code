...
load(['D:\장기생태\Dynamic\KOEM\NIFS\model_fennelNWP_rain_hsimt_2yr_salt_trend_2011_2011_fix75.mat']);
...
for numline=1:size(NIFS6.line,2)
    if numline ==1
        NIFS6.salt_surf_line_comb = NIFS6.salt_surf_line_comb;
        MODEL6.salt_surf_line_comb = MODEL6.salt_surf_line_comb';
    end
...
%         %origin %comment 2020.09.24
%         p=polyfit(xData,NIFS6.salt_surf_line_comb(~idx,numline)',1);
        line_trend2=xData*p(1)+p(2); %comment 2020.09.24
...
%         msaltplot2=plot(xData,line_trend2,'Color','r')
        ...
        title([num2str(lineline3), ', surf mean salt(',num2str(min(inputyear),'%04i'),'-',num2str(max(inputyear),'%04i'),'), ',num2str(round(NIFS6.salt_surf_line_comb_trend,4)), ' /y'])
        ...
        txt1=text(xData(1), min(meanplotlev)+1 ,['R = ', num2str(round(constant_cor(1,2),2)), ', '], 'FontSize', m_quiver_ref_text_fontsize); 
        txt2=text(xData(3), min(meanplotlev)+1 ,['RMSD = ', num2str(round(mean_salt_RMS,2)), ', '], 'FontSize', m_quiver_ref_text_fontsize); 
        txt3=text(xData(5), min(meanplotlev)+1 ,['bias = ', num2str(round(mean_salt_bias,2))], 'FontSize', m_quiver_ref_text_fontsize); 
...
for numline=1:size(NIFS6.line,2)
    if numline ==1
        NIFS6.salt75_line_comb = NIFS6.salt75_line_comb;
        MODEL6.salt75_line_comb = MODEL6.salt75_line_comb';
    end
...
%         msaltplot2=plot(xData,line_trend2,'Color','r')
        ...
        xlabel('year')
        ylabel('Mean Salinity along NIFS line')
        title([num2str(lineline3), ', 0-75m mean salt(',num2str(min(inputyear),'%04i'),'-',num2str(max(inputyear),'%04i'),'), ',num2str(round(NIFS6.salt75_line_comb_trend,4)), ' /y'])
        datetick('x','yymmm','keepticks')
        axis tight;
        ylim(meanplotlev)
        set(msaltplot,'LineWidth',2);
