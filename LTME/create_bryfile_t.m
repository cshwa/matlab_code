function create_bryfile_t(bryname,brytype,grdname,year,cycle,time,vtransform)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
% filename=['roms_bry_',brytype,'-Y',num2str(year),'M1.nc'];
filename=[brytype,'_Y',num2str(year),'M1.nc'];
% filename=bryname
% filename=['roms_add3_bndy_add2_Y1993M1.nc'];
nc1=netcdf(filename);
N=length(nc1{'Cs_r'}(:));
hc=nc1{'hc'}(:);
theta_s=nc1{'theta_s'}(:);
...
nc{'ocean_time'}.long_name = ncchar('subsurface temp/salt observations time');
nc{'ocean_time'}.long_name = 'time for temperature climatology';
nc{'ocean_time'}.units = ncchar('day');
nc{'ocean_time'}.units = 'day';
nc{'ocean_time'}.calendar = ncchar('considering leap years/365 or 366');
nc{'ocean_time'}.calendar = 'considering leap years/365 or 366';
...
%% scoordinate -> original used function
...
nc{'Cs_w'}(:) = Cs_w;
    ...
    for mon=1:1:12
%       filename=['roms_bry_',brytype,'-Y',num2str(year),'M',num2str(mon),'
%       .nc'];
      filename=['SODA_','Y',num2str(year),'M',num2str(mon),'.nc'];
%       filename=['roms_add3_bndy_add2_Y',num2str(year),'M',num2str(mon),'.nc'];
      nc_m=netcdf(filename);
%       time(mon,1)=nc_m{'bry_time'}(:);
        if  brytype == 'SODA'
        nc{'zeta_east'}(mon,:,:)=nc_m{'zeta_east'}(:);
        nc{'zeta_west'}(mon,:,:)=nc_m{'zeta_west'}(:);
        nc{'zeta_south'}(mon,:,:)=nc_m{'zeta_south'}(:);
        nc{'zeta_north'}(mon,:,:)=nc_m{'zeta_north'}(:);
        
        nc{'ubar_east'}(mon,:,:)=nc_m{'ubar_east'}(:);
        nc{'ubar_west'}(mon,:,:)=nc_m{'ubar_west'}(:);
        nc{'ubar_south'}(mon,:,:)=nc_m{'ubar_south'}(:);
        nc{'ubar_north'}(mon,:,:)=nc_m{'ubar_north'}(:);        

        nc{'vbar_east'}(mon,:,:)=nc_m{'vbar_east'}(:);
        nc{'vbar_west'}(mon,:,:)=nc_m{'vbar_west'}(:);
        nc{'vbar_south'}(mon,:,:)=nc_m{'vbar_south'}(:);
        nc{'vbar_north'}(mon,:,:)=nc_m{'vbar_north'}(:);

        nc{'u_east'}(mon,:,:)=nc_m{'u_east'}(:);
        nc{'u_west'}(mon,:,:)=nc_m{'u_west'}(:);
        nc{'u_south'}(mon,:,:)=nc_m{'u_south'}(:);
        nc{'u_north'}(mon,:,:)=nc_m{'u_north'}(:);
        
        nc{'v_east'}(mon,:,:)=nc_m{'v_east'}(:);
        nc{'v_west'}(mon,:,:)=nc_m{'v_west'}(:);
        nc{'v_south'}(mon,:,:)=nc_m{'v_south'}(:);
        nc{'v_north'}(mon,:,:)=nc_m{'v_north'}(:);
        
        nc{'temp_east'}(mon,:,:)=nc_m{'temp_east'}(:);
        nc{'temp_west'}(mon,:,:)=nc_m{'temp_west'}(:);
        nc{'temp_south'}(mon,:,:)=nc_m{'temp_south'}(:);
        nc{'temp_north'}(mon,:,:)=nc_m{'temp_north'}(:);
        
        nc{'salt_east'}(mon,:,:)=nc_m{'salt_east'}(:);
        nc{'salt_west'}(mon,:,:)=nc_m{'salt_west'}(:);
        nc{'salt_south'}(mon,:,:)=nc_m{'salt_south'}(:);
        nc{'salt_north'}(mon,:,:)=nc_m{'salt_north'}(:); 
        
            else if brytype == 'ECCO'
            nc{'zeta_east'}(mon,:,:)=mean(nc_m{'zeta_east'}(:,:));
            nc{'zeta_west'}(mon,:,:)=mean(nc_m{'zeta_west'}(:,:));
            nc{'zeta_south'}(mon,:,:)=mean(nc_m{'zeta_south'}(:,:));
            nc{'zeta_north'}(mon,:,:)=mean(nc_m{'zeta_north'}(:,:));
        
            nc{'ubar_east'}(mon,:,:)=mean(nc_m{'ubar_east'}(:,:));
            nc{'ubar_west'}(mon,:,:)=mean(nc_m{'ubar_west'}(:,:));
            nc{'ubar_south'}(mon,:,:)=mean(nc_m{'ubar_south'}(:,:));
            nc{'ubar_north'}(mon,:,:)=mean(nc_m{'ubar_north'}(:,:));        

            nc{'vbar_east'}(mon,:,:)=mean(nc_m{'vbar_east'}(:,:));
            nc{'vbar_west'}(mon,:,:)=mean(nc_m{'vbar_west'}(:,:));
            nc{'vbar_south'}(mon,:,:)=mean(nc_m{'vbar_south'}(:,:));
            nc{'vbar_north'}(mon,:,:)=mean(nc_m{'vbar_north'}(:,:));

            nc{'u_east'}(mon,:,:)=mean(nc_m{'u_east'}(:,:,:));
            nc{'u_west'}(mon,:,:)=mean(nc_m{'u_west'}(:,:,:));
            nc{'u_south'}(mon,:,:)=mean(nc_m{'u_south'}(:,:,:));
            nc{'u_north'}(mon,:,:)=mean(nc_m{'u_north'}(:,:,:));
        
            nc{'v_east'}(mon,:,:)=mean(nc_m{'v_east'}(:,:,:));
            nc{'v_west'}(mon,:,:)=mean(nc_m{'v_west'}(:,:,:));
            nc{'v_south'}(mon,:,:)=mean(nc_m{'v_south'}(:,:,:));
            nc{'v_north'}(mon,:,:)=mean(nc_m{'v_north'}(:,:,:));
        
            nc{'temp_east'}(mon,:,:)=mean(nc_m{'temp_east'}(:,:,:));
            nc{'temp_west'}(mon,:,:)=mean(nc_m{'temp_west'}(:,:,:));
            nc{'temp_south'}(mon,:,:)=mean(nc_m{'temp_south'}(:,:,:));
            nc{'temp_north'}(mon,:,:)=mean(nc_m{'temp_north'}(:,:,:));
            
            nc{'salt_east'}(mon,:,:)=mean(nc_m{'salt_east'}(:,:,:));
            nc{'salt_west'}(mon,:,:)=mean(nc_m{'salt_west'}(:,:,:));
            nc{'salt_south'}(mon,:,:)=mean(nc_m{'salt_south'}(:,:,:));
            nc{'salt_north'}(mon,:,:)=mean(nc_m{'salt_north'}(:,:,:));
            end
        end
      close(nc_m);
    end
