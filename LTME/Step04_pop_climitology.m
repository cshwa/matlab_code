...
ym_red = red;
ym_red(:,7) = red(:,7) - y1;
sst_o_s = ym_red; % climatology 뺀 sst가 들어가 있는 자료 

%--- climitology 빠진것 확인용 그림 ---------
% figure()
% plot(x,y,'k.',x,y1,'r.');
% hold on;
% plot(x,ym_red(:,7),'b.');
% legend('Raw','SplineFit','location','northwest');
% ylabel('C^{\circ}','fontsize',14);
% xlabel('Time (day)','fontsize',14);
% xt = cumsum(eomday(21985,1:12));
% set(gca,'xlim',[1 365],'ylim',[-5 30],'xtick',[1 xt(1:end-1)],'xticklabel', ...
%     {'Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'});
%--- save figure --------------------
% outpath = '.\figure_season\';
% out=[outpath,num2str(fname(ss)),'_o_s'];
...
% legend('2월','4월','8월','10월','location','south','Orientation','horizontal')
axis([min(ye)-1 max(ye)+1 -5 5]);
t_name=[num2str(fname(ss)),'  ',num2str(0), ' m depth'];
title(t_name,'fontweight','bold','fontsize',14,'color','b');

%--- save figure --------------------
% outpath = '.\figure_season\';
% out=[outpath,num2str(fname(ss)),'_lf_0'];
% set(gcf,'renderer','painter');
% set(gcf, 'PaperUnits', 'inches');
% x_width=6;
% y_width=4;
