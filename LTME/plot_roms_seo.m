...
figdir =strcat('D:\OneDrive - 서울대학교\MEPL\project\MICT_pollack\3rd year\figure\avg_ens_10km_mean\'); % % where figure files will be saved
inputdir = strcat('E:\Data\Reanalysis\nwp_1_10_seo\', testname, '\'); % % where data files are
...
for i=1:length(year)   
    ...
    status=plot_seo_ROMS_monthly_horizontal_t_uv(testname, hor_tuv_file, inputdir, [125 135 34 42 -100 -100], tempyear, month, [0 30], [5:5:10], 'temp',  'fig_param_kyy_MICT_EKWC');
    
    hor_tuv_file =[figdir, 'EJS\', 'hor_temp\', 'EJS_hor_temp_0000m']; % % ~/SUV_EJS_testname_year_month.jpg
    status=plot_seo_ROMS_monthly_horizontal_t_uv(testname, hor_tuv_file, inputdir, [125 135 34 42 0 0], tempyear, month, [0 30], [5:5:10], 'temp',  'fig_param_kyy_MICT_EKWC');
end
...
%     status=plot_seo_ROMS_monthly_horizontal_data_pollack(testname, hor_temp_file, inputdir, [127 130 38 41 -50 -50], tempyear, month, ...
%                                     [2 5], 2:1:5, 'temp', 'fig_param_kyy_EKB');
%     status=plot_seo_ROMS_monthly_horizontal_data(testname, hor_temp_file, inputdir, [127 130 38 41 -50 -50], tempyear, month, ...
