function status=scatter_isodepth_v_HYCOM(testname, outfile, filedir, section, section2, year, inputmonth, refvalue, param_script, option)
% clear all;close all;
%==========================================================================
% vert_param;
var='vert_temp';
...
seasonal_surf_vel = reshape(sectiondata(:,1,:),[12,size(sectiondata,1)/12, size(sectiondata,3)]);
...
mean_surf_vel = mean(sectiondata(:,1,section_left:section_right),3);
max_surf_vel = max(sectiondata(:,1,section_left:section_right), [], 3);
...
%         sc=scatter(mean_isodepth, mean_surf_vel,marker_size,jet(size(mean_isodepth,1)), 'filled');
        ...
        lsl=lsline;
        [r p rl ru] = corrcoef(squeeze(mean_isodepth), squeeze(mean_surf_vel), 'Alpha',0.05)
    case(2)
