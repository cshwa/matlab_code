...
sig = 3; %% sigma
clearvars idx1
idx1 = find(isnan(r_do) == 0);
regime_do=r_do;
regime_do(find(r_do > mean(r_do(idx1)) + sig*std(r_do(idx1))))=NaN;
regime_do(find(r_do < mean(r_do(idx1)) - sig*std(r_do(idx1))))=NaN;
regm_do = nanmean(regime_do)

clearvars idx1
idx1 = find(isnan(r_chl) == 0);
regime_chl=r_chl;
regime_chl(find(r_chl > mean(r_chl(idx1)) + sig*std(r_chl(idx1))))=NaN;
regime_chl(find(r_chl < mean(r_chl(idx1)) - sig*std(r_chl(idx1))))=NaN;
regm_chl = nanmean(regime_chl)

clearvars idx1
idx1 = find(isnan(r_no3) == 0);
regime_no3=r_no3;
regime_no3(find(r_no3 > mean(r_no3(idx1)) + sig*std(r_no3(idx1))))=NaN;
regime_no3(find(r_no3 < mean(r_no3(idx1)) - sig*std(r_no3(idx1))))=NaN;
regm_no3 = nanmean(regime_no3)

clearvars idx1
idx1 = find(isnan(r_nh4) == 0);
regime_nh4=r_nh4;
regime_nh4(find(r_nh4 > mean(r_nh4(idx1)) + sig*std(r_nh4(idx1))))=NaN;
regime_nh4(find(r_nh4 < mean(r_nh4(idx1)) - sig*std(r_nh4(idx1))))=NaN;
...
for i = 1:length(mth_d_txt_c)
    if size(indx{i},1) == 0
        ...
        w_do(i) = nanmean(regime_do(indx{i}));
        w_chl(i) = nanmean(regime_chl(indx{i}));
        w_no3(i) = nanmean(regime_no3(indx{i}));
        w_nh4(i) = nanmean(regime_nh4(indx{i}));
    end
end
...
for i = 1:length(mth_d_txt_c)
    if size(indx{i},1) == 0
        ...
        w_do_c{i} = regime_do(indx{i});
        w_chl_c{i} = regime_chl(indx{i});
        w_no3_c{i} = regime_no3(indx{i});
        w_nh4_c{i} = regime_nh4(indx{i});
    end
end
...
xlim([1 366])
return
save('namgang_polynomial_climate_3sig.mat'); 
