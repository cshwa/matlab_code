...
inputyear = [2011]; % % put year which you want to plot [year year ...]
inputmonth = [2 4 6 8 10 12]; % % put month which you want to plot [month month ...]

varname ='salt'
run('nwp_polygon_point.m');  %recent_cshwa
% run('nwp_polygon_point'); %matlab2010


% % % for EKB
% regionname='EKB';
% lonlat = [127, 129.5, 38, 40.5];
load(['D:\장기생태\Dynamic\KOEM\NIFS\model_fennelNWP_salt_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'_fix75.mat']);
load(['D:\장기생태\Dynamic\KOEM\NIFS\NIFS_salt_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'.mat']);
% valnum=0;
% % %     valid cell number
...
figrawdir =strcat('D:\장기생태\Dynamic\KOEM\NIFS\fig\fennel_NWP\');
...
param_script ='fig_param_kyy_AKP4.m'   %recent_cshwa
% param_script ='fig_param_kyy_AKP4'  %matlab2010
run(param_script);

% NIFS surface salt trend plot
    m_proj(m_proj_name,'lon',[lonlat(1)-0.5 lonlat(2)+0.5],'lat',[lonlat(3)-0.5 lonlat(4)+0.5]);
%     m_proj('proj name','lon',[lonlat(1)-0.5 lonlat(2)+0.5],'lat',[lonlat(3)-0.5 lonlat(4)+0.5]);  
    hold on;
...
%     for month=1:12
%         xxData((12*(i-1))+month) = datenum([num2str(tempyear),'-',num2str(month,'%02i'),'-15',]);
%     end
end
...
for numline=1:size(NIFS6.line,2)
    if (isnan(sum(NIFS6.salt_surf_line_comb(:,numline),'omitnan'))==1)
        ...
        p=polyfit(xData(~idx),NIFS6.salt_surf_line_comb(~idx,numline)',1); %origin
%         p=polyfit(xData,NIFS6.salt_surf_line_comb(~idx,numline)',1);
        line_trend2=xData*p(1)+p(2);
        msaltplot=plot(xData,MODEL6.salt_surf_line_comb(numline,:)','color','k');
        hold on
        msaltplot2=plot(xData,NIFS6.salt_surf_line_comb(:,numline)','o','color','r','markerfacecolor','k','markersize',2); %origin
%         msaltplot2=plot(xData,NIFS6.salt_surf_line_comb(~idx,numline)','o','color','r','markerfacecolor','k','markersize',2);
%         msaltplot2=plot(xData,line_trend2,'Color','r')
        idx2=isnan(MODEL6.salt_surf_line_comb(numline,:)');
        idx3=idx+idx2;
        idx3(idx3==2)=1;
        constant_cor=corrcoef(NIFS6.salt_surf_line_comb(~idx3,numline)',MODEL6.salt_surf_line_comb(numline,~idx3)');
        
        salt_bias=MODEL6.salt_surf_line_comb(numline,~idx3)' - NIFS6.salt_surf_line_comb(~idx3,numline)';
...
%         msaltplot2=plot(xData,line_trend2,'Color','r')
        ...
        idx3=idx+idx2;
        idx3(idx3==2)=1;
        constant_cor=corrcoef(NIFS6.salt75_line_comb(~idx3,numline)',MODEL6.salt75_line_comb(numline,~idx3)');
        
        salt_bias=MODEL6.salt75_line_comb(numline,~idx3)' - NIFS6.salt75_line_comb(~idx3,numline)';
        mean_salt_bias=mean(salt_bias);
        mean_salt_RMS=mean(abs(salt_bias));
        
        line_skill_score = (1 - abs(salt_bias') ./ NIFS6.salt_surf_line_comb(~idx3,numline))*100; % KIMST demand it
        line_skill_score_spm=squeeze(nanmean(line_skill_score));
        
       if csh_id == 1
        obs_all = NIFS6.salt_surf_line_comb(~idx3,numline);
        salt_bias_all = salt_bias;
       else
        obs_all=[obs_all; NIFS6.salt_surf_line_comb(~idx3,numline)];
        salt_bias_all = [salt_bias_all salt_bias];
       end
        hold off
        ...
        set(gca,'YTick',min(meanplotlev):1:max(meanplotlev))
        txt1=text(xData(3), min(meanplotlev)+1 ,['R = ', num2str(round(constant_cor(1,2),2)), ', '], 'FontSize', m_quiver_ref_text_fontsize); 
        txt2=text(xData(32), min(meanplotlev)+1 ,['RMSD = ', num2str(round(mean_salt_RMS,2)), ', '], 'FontSize', m_quiver_ref_text_fontsize); 
        txt3=text(xData(73), min(meanplotlev)+1 ,['bias = ', num2str(round(mean_salt_bias,2))], 'FontSize', m_quiver_ref_text_fontsize); 
        lgd=legend(['Model(',testname,')'],'NIFS');
        ...
        %plot skill score
        if size(line_skill_score,1) + size(line_skill_score,2) > 2
            msaltplotsc=plot(xData(~idx),line_skill_score,'color','b','markerfacecolor','k','markersize',2);
            jpgname=strcat(outfile,testname,'_',num2str(lineline3), '0_75m_line_mean_skillscore_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'), '.jpg'); %% ~_year_month.jpg
            xlabel('year')
            ylabel('Mean Salinity along NIFS line')
            title([num2str(lineline3), ', 0-75m mean salt(',num2str(min(inputyear),'%04i'),'-',num2str(max(inputyear),'%04i'),'), skill score'])
            datetick('x','yymmm','keepticks')
            axis tight;
            ylim([90 100])
            set(msaltplotsc,'LineWidth',2);
            set(gca,'FontSize',15);
            set(gca,'YTick',90:1:100)
            txt3=text(xData(3), 90+1 ,['skill score = ', num2str(round(line_skill_score_spm,2)),' %' ], 'FontSize', m_quiver_ref_text_fontsize); 
            set(txt3,'FontSize',20);
            set(gcf, 'PaperUnits', 'points');
            set(gcf, 'PaperSize', [1000, 400]);
            set(gcf,'PaperPosition', [0 0 1000 400])
...
skill_score = (1 - abs(salt_bias_all') ./ obs_all)*100; % KIMST demand it
skill_score_spm=squeeze(nanmean(skill_score));
mean(skill_score_spm)
