...
sig = 3; %% sigma
clearvars idx1
idx1 = find(isnan(w_do) == 0);
regime_do=w_do;
regime_do(find(w_do > mean(w_do(idx1)) + sig*std(w_do(idx1))))=NaN;
regime_do(find(w_do < mean(w_do(idx1)) - sig*std(w_do(idx1))))=NaN;
regm_do = nanmean(regime_do)

clearvars idx1
idx1 = find(isnan(w_chl) == 0);
regime_chl=w_chl;
regime_chl(find(w_chl > mean(w_chl(idx1)) + sig*std(w_chl(idx1))))=NaN;
regime_chl(find(w_chl < mean(w_chl(idx1)) - sig*std(w_chl(idx1))))=NaN;
regm_chl = nanmean(regime_chl)

clearvars idx1
idx1 = find(isnan(w_no3) == 0);
regime_no3=w_no3;
regime_no3(find(w_no3 > mean(w_no3(idx1)) + sig*std(w_no3(idx1))))=NaN;
regime_no3(find(w_no3 < mean(w_no3(idx1)) - sig*std(w_no3(idx1))))=NaN;
regm_no3 = nanmean(regime_no3)

clearvars idx1
idx1 = find(isnan(w_nh4) == 0);
regime_nh4=w_nh4;
regime_nh4(find(w_nh4 > mean(w_nh4(idx1)) + sig*std(w_nh4(idx1))))=NaN;
regime_nh4(find(w_nh4 < mean(w_nh4(idx1)) - sig*std(w_nh4(idx1))))=NaN;
...
xp = 1:366;
xp_w_do = find(isnan(regime_do)==0);
pf_w_do = polyfit(xp_w_do,regime_do(xp_w_do),3);
yp_w_do = polyval(pf_w_do,xp);
yp_w_do = yp_w_do + regm_do;

xp = 1:366;
xp_w_chl = find(isnan(regime_chl)==0);
pf_w_chl = polyfit(xp_w_chl,regime_chl(xp_w_chl),3);
yp_w_chl = polyval(pf_w_chl,xp);
yp_w_chl = yp_w_chl + regm_chl;

xp = 1:366;
xp_w_no3 = find(isnan(regime_no3)==0);
pf_w_no3 = polyfit(xp_w_no3,regime_no3(xp_w_no3),3);
yp_w_no3 = polyval(pf_w_no3,xp);
yp_w_no3 = yp_w_no3 + regm_no3;

xp = 1:366;
xp_w_nh4 = find(isnan(regime_nh4)==0);
pf_w_nh4 = polyfit(xp_w_nh4,regime_nh4(xp_w_nh4),3);
yp_w_nh4 = polyval(pf_w_nh4,xp);
yp_w_nh4 = yp_w_nh4 + regm_nh4;
...
%% DO
figure;
scatter(1:366,regime_do + regm_do);
hold on
...
%% chl
figure;
scatter(1:366,regime_chl + regm_chl);
hold on
...
%% no3
figure;
scatter(1:366,regime_no3 + regm_no3);
hold on
...
%% nh4
figure;
scatter(1:366,regime_nh4 + regm_nh4);
hold on
...
set(gca,'fontsize',13)
xlim([1 366])
save('sumjin(songjung)_polynomial_climate_to2004_v2_3sig.mat'); 
...
set(gca,'fontsize',13)
xlim([1 366])
save('Namgang_polynomial_climate_to2004_v2_3sig.mat'); 
