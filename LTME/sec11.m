...
x = find(p(a) > 0.20 & c(a) > 40);
y = find(p(a) == max(p));
b = [x(find(diff(x) ~= 1)+5) : y(1)]; d3 = d(3) * ones(size(b,2),1);
...
x = find(p(a) > 0.20 & c(a) > 40);
y = find(p(a) == max(p));
b = [x(find(diff(x) ~= 1)+4) : y(1)]; d4 = d(4) * ones(size(b,2),1);
...
x = find(p(a) > 0.20 & c(a) > 40);
y = find(p(a) == max(p));
b = [x(find(diff(x) ~= 1)+2) : y(1)-1]; d5 = d(5) * ones(size(b,2),1);
...
a = 1 : size(d1,1);
y = find(p(a) == max(p));
b = [120 : y(1)]; d6 = d(6) * ones(size(b,2),1);
...
m11 = [st1_1;st2_1;st4_1;st5_1;st6_1;st7_1];
...
gtem1 = m11;
gtem1(:,2) = -1 * gtem1(:,2);
fid = fopen('gtem1.txt','w');
fprintf(fid, '%f %f %f %f\n', gtem1');
fclose(fid);



figure; plot(st1_1(:,3),st1_1(:,2)); hold on;
plot(st2_1(:,3),st2_1(:,2),'k');
plot(st4_1(:,3),st4_1(:,2),'y');
plot(st5_1(:,3),st5_1(:,2),'r');
plot(st6_1(:,3),st6_1(:,2),'g');
plot(st7_1(:,3),st7_1(:,2),'c');
