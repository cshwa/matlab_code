...
for i = 1:length(r_date_txt)
    ...
    r_raw_txt_c{i,1} = r_date_txt(i,:); % raw (yy.mm.dd)
    r_mmdd_txt_c{i,1} = r_date_txt(i,1:end-3); % yy.mm
end
...
k=0; m=0;
for i = 1:length(2007:2019)
    l=0
    for n = 1:12
        m = m+1;
        yymm_txt(m,:)=[num2str(i+2007-1) '.' num2str(n,'%02d')];
...
% for i = 1:length(yymm_txt)
%     yymm_txt_c{i,1} = yymm_txt(i,:); % raw
% end
...
% for i = 1:length(yymm_txt_c)
%        indx_raw_mmdd{i} = find([strcmp(yymm_txt_c{i}, r_mmdd_txt_c)] == 1);
% end
...
% % % monthly (yy.mm)
% % for i = 1:length(yymm_txt_c)
% %     if size(indx_raw_mmdd{i},1) == 0
% %         raw_mon_do(i) = NaN;
% %         raw_mon_chl(i) = NaN; 
% %         raw_mon_no3(i) = NaN; 
% %         raw_mon_nh4(i) = NaN; 
% %     else
% %         raw_mon_do(i) = nanmean(r_do(indx_raw_mmdd{i}));
% %         raw_mon_chl(i) = nanmean(r_chl(indx_raw_mmdd{i}));
% %         raw_mon_no3(i) = nanmean(r_no3(indx_raw_mmdd{i}));
% %         raw_mon_nh4(i) = nanmean(r_nh4(indx_raw_mmdd{i}));
% %     end
% % end
...
%make t-axis monthly end of day
t_indx_pre=[];
...
% save('jinwall_yymm_monthly_data_07to19_3sig.mat','mon*');
...
save('jinwall_reconst_climate_3sig.mat','yp_est_*'); %% reconst. climate

save('jinwall_raw_data_daily.mat','raw_*');  %% raw data
