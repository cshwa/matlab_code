...
pf = polyfit(1:length(a_temp),a_temp,3);
xp = 1:length(a_temp);
yp = polyval(pf,xp);

xp_w = find(isnan(w_temp)==0);
pf_w = polyfit(xp_w,w_temp(xp_w),3);
...
for i = 1:7
mean_diff=yp_w-yp;
pf_diff = polyfit(1:366,mean_diff,i);
yp_diff = polyval(pf_diff,1:366);
figure;
plot(mean_diff,'c','linew',1.5); hold on;
plot(yp_diff,'r','linew',1.5);
xlabel('Days','fontsize',13)
ylabel('temp. (^oC)','fontsize',13)
title('compare daily climate air & water temp.','fontsize',13)
grid on;
xlim([1 366])
legend('gawha(w) - jinju(a)',[num2str(i), '-th polynomial'])
set(gca,'fontsize',15)
end
...
grid on;
xlim([1 366])
legend('sumjin(w) - yeosu(a)',[num2str(i), '-th polynomial'])
set(gca,'fontsize',15)
% save(gca,
end
