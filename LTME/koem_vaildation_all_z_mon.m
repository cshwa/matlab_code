...
clearvars mod_m_*
if size(sp_gy_no3_mm,3) >= 365
        for i =1:12
        if i ==1 
            sp_gy_mon_no3_mm(:,:,i)=mean(sp_gy_no3_mm(:,:,1:eom_d_each(1,i)),3);
            sp_gy_mon_nh4_mm(:,:,i)=mean(sp_gy_nh4_mm(:,:,1:eom_d_each(1,i)),3);
%             sp_gy_mon_do_mm(:,:,i)=mean(sp_gy_do_mm(:,:,1:eom_d_each(1,i)),3);
            sp_gy_mon_chl_mm(:,:,i)=mean(sp_gy_chl_mm(:,:,1:eom_d_each(1,i)),3);
            sp_gy_mon_temp_mm(:,:,i)=mean(sp_gy_temp_mm(:,:,1:eom_d_each(1,i)),3);
            sp_gy_mon_salt_mm(:,:,i)=mean(sp_gy_salt_mm(:,:,1:eom_d_each(1,i)),3);
            sp_gy_mon_no3_2reg_mm(:,:,i)=mean(sp_gy_no3_2reg_mm(:,:,1:eom_d_each(1,i)),3);
            sp_gy_mon_nh4_2reg_mm(:,:,i)=mean(sp_gy_nh4_2reg_mm(:,:,1:eom_d_each(1,i)),3);
%             sp_gy_mon_do_2reg_mm(:,:,i)=mean(sp_gy_do_2reg_mm(:,:,1:eom_d_each(1,i)),3);
            sp_gy_mon_chl_2reg_mm(:,:,i)=mean(sp_gy_chl_2reg_mm(:,:,1:eom_d_each(1,i)),3);
            sp_gy_mon_temp_2reg_mm(:,:,i)=mean(sp_gy_temp_2reg_mm(:,:,1:eom_d_each(1,i)),3);
            sp_gy_mon_salt_2reg_mm(:,:,i)=mean(sp_gy_salt_2reg_mm(:,:,1:eom_d_each(1,i)),3);
%             sp_gy_mon_no3_3reg_mm(:,:,i)=mean(sp_gy_no3_3reg_mm(:,:,1:eom_d_each(1,i)),3);
%             sp_gy_mon_nh4_3reg_mm(:,:,i)=mean(sp_gy_nh4_3reg_mm(:,:,1:eom_d_each(1,i)),3);
%             sp_gy_mon_do_3reg_mm(:,:,i)=mean(sp_gy_do_3reg_mm(:,:,1:eom_d_each(1,i)),3);
%             sp_gy_mon_chl_3reg_mm(:,:,i)=mean(sp_gy_chl_3reg_mm(:,:,1:eom_d_each(1,i)),3);
%             sp_gy_mon_temp_3reg_mm(:,:,i)=mean(sp_gy_temp_3reg_mm(:,:,1:eom_d_each(1,i)),3);
%             sp_gy_mon_salt_3reg_mm(:,:,i)=mean(sp_gy_salt_3reg_mm(:,:,1:eom_d_each(1,i)),3);
        else
            sp_gy_mon_no3_mm(:,:,i)=mean(sp_gy_no3_mm(:,:,eom_d_each(1,i-1)+1:eom_d_each(1,i)),3);
            sp_gy_mon_nh4_mm(:,:,i)=mean(sp_gy_nh4_mm(:,:,eom_d_each(1,i-1)+1:eom_d_each(1,i)),3);
%             sp_gy_mon_do_mm(:,:,i)=mean(sp_gy_do_mm(:,:,eom_d_each(1,i-1)+1:eom_d_each(1,i)),3);
            sp_gy_mon_chl_mm(:,:,i)=mean(sp_gy_chl_mm(:,:,eom_d_each(1,i-1)+1:eom_d_each(1,i)),3);
            sp_gy_mon_temp_mm(:,:,i)=mean(sp_gy_temp_mm(:,:,eom_d_each(1,i-1)+1:eom_d_each(1,i)),3);
            sp_gy_mon_salt_mm(:,:,i)=mean(sp_gy_salt_mm(:,:,eom_d_each(1,i-1)+1:eom_d_each(1,i)),3);
            sp_gy_mon_no3_2reg_mm(:,:,i)=mean(sp_gy_no3_2reg_mm(:,:,eom_d_each(1,i-1)+1:eom_d_each(1,i)),3);
            sp_gy_mon_nh4_2reg_mm(:,:,i)=mean(sp_gy_nh4_2reg_mm(:,:,eom_d_each(1,i-1)+1:eom_d_each(1,i)),3);
%             sp_gy_mon_do_2reg_mm(:,:,i)=mean(sp_gy_do_2reg_mm(:,:,eom_d_each(1,i-1)+1:eom_d_each(1,i)),3);
            sp_gy_mon_chl_2reg_mm(:,:,i)=mean(sp_gy_chl_2reg_mm(:,:,eom_d_each(1,i-1)+1:eom_d_each(1,i)),3);
            sp_gy_mon_temp_2reg_mm(:,:,i)=mean(sp_gy_temp_2reg_mm(:,:,eom_d_each(1,i-1)+1:eom_d_each(1,i)),3);
            sp_gy_mon_salt_2reg_mm(:,:,i)=mean(sp_gy_salt_2reg_mm(:,:,eom_d_each(1,i-1)+1:eom_d_each(1,i)),3);
%             sp_gy_mon_no3_3reg_mm(:,:,i)=mean(sp_gy_no3_3reg_mm(:,:,eom_d_each(1,i-1)+1:eom_d_each(1,i)),3);
%             sp_gy_mon_nh4_3reg_mm(:,:,i)=mean(sp_gy_nh4_3reg_mm(:,:,eom_d_each(1,i-1)+1:eom_d_each(1,i)),3);
%             sp_gy_mon_do_3reg_mm(:,:,i)=mean(sp_gy_do_3reg_mm(:,:,eom_d_each(1,i-1)+1:eom_d_each(1,i)),3);
%             sp_gy_mon_chl_3reg_mm(:,:,i)=mean(sp_gy_chl_3reg_mm(:,:,eom_d_each(1,i-1)+1:eom_d_each(1,i)),3);
%             sp_gy_mon_temp_3reg_mm(:,:,i)=mean(sp_gy_temp_3reg_mm(:,:,eom_d_each(1,i-1)+1:eom_d_each(1,i)),3);
%             sp_gy_mon_salt_3reg_mm(:,:,i)=mean(sp_gy_salt_3reg_mm(:,:,eom_d_each(1,i-1)+1:eom_d_each(1,i)),3);
        end
        end
    else
        for i =1:11
...
clim2_2_std = clim1.obs_std_gy_din_ft_2(:,3);
    ...
    fig = figure; hold on;
            plot(squeeze(nanmean(sp_gy_mon_no3_mm(:,3,:),1)) + squeeze(nanmean(sp_gy_mon_nh4_mm(:,3,:),1)),'r','linew',2);
%             plot((squeeze(nanmean(sp_gy_no3_mm(:,3,:),1)).*N_MW + squeeze(nanmean(sp_gy_nh4_mm(:,3,:),1)).*N_MW)./N_MW,'k--','linew',2);
            plot(squeeze(nanmean(sp_gy_mon_no3_2reg_mm(:,3,:),1)) + squeeze(nanmean(sp_gy_mon_nh4_2reg_mm(:,3,:),1)),'g','linew',2);
            clearvars *_bound_plt nonan_data_plt discon_p
            nonan_data_plt = find(isnan(clim2)==0);
            nonan_diff=nonan_data_plt(2:end) - nonan_data_plt(1:end-1);
            discon_p = find(nonan_diff ~= 1); % find discontinuous point
...
clim2_2_std = clim1.obs_std_gy_no3_ft_2(:,3);
    ...
    fig = figure; hold on;
            plot(squeeze(nanmean(sp_gy_mon_no3_mm(:,3,:),1)),'r','linew',2);
            plot(squeeze(nanmean(sp_gy_mon_no3_2reg_mm(:,3,:),1)),'g','linew',2);
%             clearvars *_bound_plt nonan_data_plt discon_p temp_d
...
mon_plt_x = [2, 5, 8, 11];    
        clearvars clim2*
for i=1:4
clim2(i)=clim1.obm_gy_nh4_ft_1(plt_filter_1{i}(1),3);
clim2_std(i) = clim1.obs_std_gy_nh4_ft_1(plt_filter_1{i}(1),3);
clim2_2(i)=clim1.obm_gy_nh4_ft_2(plt_filter_1{i}(1),3);
clim2_2_std(i) = clim1.obs_std_gy_nh4_ft_2(plt_filter_1{i}(1),3);
end
    fig = figure; hold on;
            plot(squeeze(nanmean(sp_gy_mon_nh4_mm(:,3,:),1)),'r','linew',2);
            plot(squeeze(nanmean(sp_gy_mon_nh4_2reg_mm(:,3,:),1)),'g','linew',2);
             % 1regime
            clearvars *_bound_plt nonan_data_plt discon_p
            nonan_data_plt = find(isnan(clim2)==0);
            nonan_diff=nonan_data_plt(2:end) - nonan_data_plt(1:end-1);
...
%             discon_p(4) = length(nonan_data_plt); %end point
            ...
            lower_bound_plt = clim2./N_MW - clim2_std./N_MW;
%             for i = 1:4
%             patch([mon_plt_x(i) fliplr(mon_plt_x(i))], [upper_bound_plt(i)' fliplr(lower_bound_plt(i))'],'r');
%             end
            plot(mon_plt_x,clim2./N_MW,'r-','linew',2);
            plot(mon_plt_x,upper_bound_plt,'r-','linew',2);
            plot(mon_plt_x,lower_bound_plt,'r-','linew',2);
...
clim2_2_std = clim1.obs_std_gy_chl_ft_2(:,3);
    ...
    fig = figure; hold on;
            plot(squeeze(nanmean(sp_gy_mon_chl_mm(:,3,:),1)),'r','linew',2);
            plot(squeeze(nanmean(sp_gy_mon_chl_2reg_mm(:,3,:),1)),'g','linew',2);
%             clearvars *_bound_plt nonan_data_plt discon_p temp_d
...
clim2_2_std = clim1.obs_std_gy_temp_ft_2(:,3);
    ...
    fig = figure; hold on;
            plot(squeeze(nanmean(sp_gy_mon_temp_mm(:,3,:),1)),'r','linew',2);
            plot(squeeze(nanmean(sp_gy_mon_temp_2reg_mm(:,3,:),1)),'g','linew',2);
%             clearvars *_bound_plt nonan_data_plt discon_p temp_d
...
clim2_2_std = clim1.obs_std_gy_salt_ft_2(:,3);   
    ...
    fig = figure; hold on; 
            plot(squeeze(nanmean(sp_gy_mon_salt_mm(:,3,:),1)),'r','linew',2);
            plot(squeeze(nanmean(sp_gy_mon_salt_2reg_mm(:,3,:),1)),'g','linew',2);
