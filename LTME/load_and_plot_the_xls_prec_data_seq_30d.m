...
%% seq to 6 days
for i = 1:round(length(sj_prec)/30)
    if i == round(length(sj_prec)/30)
        % take all remains
        sj_prec_c{i,1} = sj_prec((i-1)*30+1:end)';
        sj_trans_out_c{i,1} = sj_trans_out((i-1)*30+1:end)';
    else
        sj_prec_c{i,1} = sj_prec((i-1)*30+1:i*30)';
        sj_trans_out_c{i,1} = sj_trans_out((i-1)*30+1:i*30)';
    end
end
...
%% seq to 6 days
for i = 1:round(length(sj_prec_2)/30)
    if i == round(length(sj_prec_2)/30)
        % take all remains
        sj_prec_2_c{i,1} = sj_prec_2((i-1)*30+1:end)';
        sj_trans_out_2_c{i,1} = sj_trans_out_2((i-1)*30+1:end)';
    else
        sj_prec_2_c{i,1} = sj_prec_2((i-1)*30+1:i*30)';
        sj_trans_out_2_c{i,1} = sj_trans_out_2((i-1)*30+1:i*30)';
    end
end
...
plot(Y_plot,'ro'); hold on; plot(Y_test_plot,'b');

sqrt(mean((Y_plot - Y_test_plot).^2))
...
figure; 
plot(Y_test_plot,f,'k.'); hold on;  
plot(Y_test_plot,Y_plot,'r.'); grid on; plot(Y_test_plot,Y_test_plot,'--','color',[0.5 0.5 0.5]);
