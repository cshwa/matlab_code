...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
variation = 9; %  1 = temperature // 2 = salinity // 3 = NO3 // 4 = NH4 // 5 = chlorophyll
...
% temp_lim = [6 25];
plot_pcolorjw = 1;    temp_lim = [0 35];    salt_lim = [28 35]; den_lim = [20 26]; no3_lim=[0 20]; nh4_lim=[0 2];
                      chl_lim=[0 10];   phy_lim=[0 3];    zoo_lim=[0 3];  oxy_lim=[150 300];
                      tic_lim=[-70 200]; po4_lim=[0 1];
% temp_c  =[6:1:25];
plot_contour  = 1;    color_c  ='-k' ;      temp_c  =[0:1:35];salt_c  =[28:.5:35]; no3_c=[0:2:200]; nh4_c=[0:.2:30];
                      chl_c=[0:.5:20];    phy_c=[0:.1:10];    zoo_c=[0:0.5:3];        oxy_c=[150:2:330];
...
case_name=['3reg_t4_old_',num2str(yy),'_monthly_'];

% gd = grd('pohang_fine');
% gd = grd('pump_30_close');
fpath='J:\장기생태_2021\Dynamic\result\3regime\';
lon_rho  = ncread([fpath,case_name,'01.nc'],'lon_rho');
lat_rho  = ncread([fpath,case_name,'01.nc'],'lat_rho');
mask_rho = ncread([fpath,case_name,'01.nc'],'mask_rho');
topo=ncread([fpath,case_name,'01.nc'],'h');
N = length(ncread([fpath,case_name,'01.nc'],'s_rho'));
% depth=zlevs(h,gd.zeta,gd.theta_s,gd.theta_b,gd.hc,N,1,1,'r');
theta_s=ncread([fpath,case_name,'01.nc'],'theta_s');
theta_b=ncread([fpath,case_name,'01.nc'],'theta_b');
hc=ncread([fpath,case_name,'01.nc'],'hc');
% angle    = gd{'angle'}(:);
% mask_u = gd{'mask_u'}(:);
% mask_v = gd{'mask_v'}(:);
warning off
