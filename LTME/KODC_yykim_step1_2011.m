...
for numyear=1:length(year)
    ...
    NIFS.salt(NIFS.salt > 99) = NaN; % NaN flag is 99.9999...
...
%                         NIFS6.salt75_sta(numyear,numline,numsta,nummon)=mean(NIFS6.salt(numyear,numline,numsta,nummon,1:depthlen),5,'omitnan');  %origin
                        if depthlen >= 6
                          NIFS6.salt75_sta(numyear,numline,numsta,nummon)=mean(NIFS6.salt(numyear,numline,numsta,nummon,1:6),5,'omitnan');
                        else
...
NIFS6.salt75_line(1:length(year),:,:)=squeeze(mean(NIFS6.salt75_sta,3,'omitnan'));
NIFS6.salt_surf_line(1:length(year),:,:)=squeeze(mean(NIFS6.salt_surf_sta,3,'omitnan'));
...
for numline=1:size(NIFS6.salt75_line,1) %year index delet so has to be 1 
...
save(['NIFS_salt_trend_',num2str(min(year),'%04i'),'_',num2str(max(year),'%04i'),'.mat'], 'NIFS6');
