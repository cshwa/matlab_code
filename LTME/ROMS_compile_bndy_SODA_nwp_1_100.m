...
for yind=1:length(year)
    ...
    elseif (linux==1)
        bryname = ['/home/yjtak/Dropbox/source/matlab/Model/ROMS/OBC_SODA/','roms_bndy_',casename,'_SODA_3_4_2_',num2str(yyyy), '_',testname,'.nc'];
    end

    g = grd('NWP_1_100_linux',testname);
    grdname = g.grd_file;
    theta_s = g.theta_s;
    theta_b = g.theta_b;
    hc = g.hc;
    ...
    for mi = mm(1):mm(end);
        ...
        elseif (linux==1)
            filename = ['/home/yjtak/Dropbox/source/matlab/Model/ROMS/OBC_SODA/','SODA_Y', num2str(yyyy,4), 'M', num2str(mi), '_',testname,'.nc'];
        end
            nc = netcdf(filename);
