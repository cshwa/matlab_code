...
file_start=1;  file_end =2;

first_name='spinup'; mid_name = '_monthly_'; foot ='.nc';
% first_name='monthly'; mid_name = '_'; foot ='.nc';

%==========================================================================
variation= 1;  %  1 = temperature , 2 = salinity , 3 = density , 4 = zeta , 5=rain 6=evapo  7=e-p 8 = nh4  9=no3  10=po4 11=do 12=chla  0 == none

level_T  =40;  % 40 = surface , 1 = bottom

current =1; cur_interval=10;

skip_v = 1;     size_v = 2;     color_v = 'k';

plot_pcolorjw = 1;
temp_lim = [0 30];    salt_lim = [28 35];     zeta_lim = [-.3 .3]; dens_lim = [23 27]; rain_lim=[0 10];evap_lim=[0 10]; ep_lim=[-10 10];
nh4_lim=[0 1];no3_lim=[0 10];po4_lim=[0 0.5]; do_lim=[0 10]; chl_lim=[0 6];
level_a  = [0:2:30]; level_b  = [28:1:35];    level_z  = [-.3 :0.01:.1 ]; level_d  = [23:0.5:27]; level_rain=[0:1:10];level_evap=[0:1:10];level_ep=[-10:1:10]; 
level_nh4=[-10:20:200];level_no3=[0:2:10];level_po4=[0:0.1:0.5];level_do=[0:1:10];level_chl=[0:1:10];
color_c  = '-w';  color_g = 'white';   out_type = 'tif';

out_vari='_';

%==========================================================================
grdfile       = './grid/roms_grd_auto_rdrg2_new3_smooth.nc';
...
domaxis=[120 130 30 38];

Mon_T={'Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'};

for yid  = syear:eyear
    in_path =['./spinup_',num2str(yid),'/monthly_mean/'];
    out_path=['./spinup_',num2str(yid),'/monthly_mean/'];
%     in_path =['.\',num2str(yid),'\monthly_mean_rdrg2_hm6_3\'];
%     out_path=['.\',num2str(yid),'\monthly_mean_rdrg2_hm6_3\'];
    y_num = num2str(yid);
    kkk=0;
    tc=1; % �ð� ���Կ� count
...
%         filename = [in_path,mid_name,m_num,foot];
		midd=[num2char(mid,2)];
        filename = [in_path,first_name,num2str(spinup),mid_name,num2str(yid),'_',midd,'.nc'];        
        nc=netcdf(filename);
        temp = nc{'temp'}(:);
        salt = nc{'salt'}(:);
        zeta = nc{'zeta'}(:);
...
%         tPO4 = nc{'tPO4'}(:);
        ...
        if variation == 1
                ...
                caxis_lim=temp_lim;
                cont_level=level_a;
                vari_name='Sea Surface Temperature';
                out_vari='SST';
        else if variation == 2
                ...
                temp_T=mask_rho.*squeeze(DO(level_T,:,:))/0.7/44.661;
                u_T=squeeze(u(level_T,:,:));   v_T=squeeze(v(level_T,:,:));
                [uu_T,vv_T,lon,lat,mask]=uv_vec2rho(u_T,v_T,lon_rho,lat_rho,angle,mask_rho,1,[0 0 0 0]);
                uu_T=mask_rho.*uu_T;  vv_T=mask_rho.*vv_T;
                bar_name='ml/L';
        ...
        ttime = [num2str(yid),'.',num2char(mid,2)];
...
%             'lat',[24 52]);
        ...
        if variation ~=0
            ...
            set(gca,'fontsize',15)
            shading flat
            [C1,h1]=m_contour(lon_rho,lat_rho,temp_T,cont_level,color_c,'linewidth',1);
            clabel(C1,h1,'LabelSpacing',1000,'Rotation',0,'Color','w','fontsize',17)
        end
...
%         timename=[num2str(yid),'.',num2str(mid)];
        ...
        m_text(domaxis(1)+ratio_x,domaxis(4)-(3*ratio_y),vari_name,'color','k','FontSize',16,'fontweight','bold')
        m_text(domaxis(1)+ratio_x,domaxis(4)-(10*ratio_y),{ttime},'fontsize',17,'color','k','fontweight','bold')
%         m_line([124, 128],[33 33],'linewidth',2);
%         m_line([124, 128],[33.4067 33.4067],'linewidth',2);
%         m_text(domaxis(1)+ratio_x,domaxis(4)-(9*ratio_y),Mon_T{kkk},'font
%         size',16,'color','k','fontweight','bold')
        if (current)
    for j=1:cur_interval:length(lat_rho(:,1))
        for i=1:cur_interval:length(lat_rho(1,:))
            if isnan(uu_T(j,i)) == 0
                hh=m_quiver(lon(j,i), lat(j,i), uu_T(j,i), vv_T(j,i), 2, 'Color',[.6 .6 .6],'linewidth',.5);
          %      adjust_quiver_arrowhead_size(hh,8.);
...
m_line([128 120.5],[32 35.5],'linewidth',2);
% m_line([124 121.5],[34 36.5],'linewidth',2,'color',[1 0 0]);
...
%             'Topography',10)
        out=[out_path,'model_monthly',out_vari,num2str(yid),num2char(mid,4)];
