...
filename = 'D:\MEPL\project\NWP\Roms_tools\Topo\etopo1.nc';

%% bathymetry (1/20)
% read data
% lon = ncread(filename,'lon');  %% 17400 ~ 20701 (110 ~ 165)
% lat = ncread(filename,'lat');  %% 6000 ~ 8701 (10 ~ 55)
% depth = ncread(filename,'z');
lon = ncread(filename,'lon', 17400, 3301);  %% 17400 ~ 20701 (110 ~ 165)
lat = ncread(filename,'lat', 6000, 2701);  %% 6000 ~ 8701 (10 ~ 55)
depth = -ncread(filename,'z',[17400 6000], [3301 2701]);
...
titlename = 'Bathymetry of the etopo1';
...
jpgname=strcat(bathyfile,'.jpg'); % ~workdir\figure\bathy_etopo_~.jpg
