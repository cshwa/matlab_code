...
tem=ncread('auto_ERA5_clim01-10_Tair_monthly.nc','Tair');
...
title_name = strcat('yellow-',' 2001~2010.',i_season{i}, '  surf.');
m_proj('mercator','lon',[min_lon max_lon], 'lat',[min_lat max_lat]);
m_pcolor(lon,lat,squeeze(tem(:,:,i)).*(mask_r./mask_r)); shading flat;
% m_quiver(lon(1:interval:end,1:interval:end),lat(1:interval:end,1:interval:end), ...
...
[cs,h]=m_contour(lon,lat,squeeze(tem(:,:,i)),temp_range,'linecolor',[.01 .01 .01],'linewidth',1);
...
% plot_google_map('MapType','terrain','Scale',2,'Resize',2)   % overlay google map
% saveas(gcf,strcat('monthly_mean_',i_season{i},'m_2nd(airt).png'),'png');
% close; 
end
...
%     m_contourf(xx,yy,e,20); m_contour(xx,yy,e,20); %% ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿?
title_name = strcat('yellow-',' 2001~2010.',i_season{i}, '  surf.');
% m_proj('mercator','lon',[126 129], 'lat',[33 35]);
m_pcolor(lon,lat,squeeze(tem(:,:,i)).*(mask_r./mask_r)); shading flat;
% m_quiver(lon(1:interval:end,1:interval:end),lat(1:interval:end,1:interval:end), ...
%     squeeze(u(1:interval:end,1:interval:end,i) .* scale_vec), ...
%     squeeze(v(1:interval:end,1:interval:end,i) .* scale_vec), ...
...
plot_google_map('MapType','terrain','Scale',2,'Resize',2)   % overlay google map
saveas(gcf,strcat('monthly_mean_',i_season{i},'m_part_2nd(airt).png'),'png');
% close; 
end
...
%     m_contourf(xx,yy,e,20); m_contour(xx,yy,e,20); %% ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿?
title_name = strcat('yellow-',' 2001~2010.',i_season{i}, '  surf.');
% m_proj('mercator','lon',[126 129], 'lat',);
pcolor(lon,lat,squeeze(tem(:,:,i)).*(mask_r./mask_r)); shading flat;
xlim([126 129]); ylim([33 35]);
% m_quiver(lon(1:interval:end,1:interval:end),lat(1:interval:end,1:interval:end), ...
%     squeeze(u(1:interval:end,1:interval:end,i) .* scale_vec), ...
%     squeeze(v(1:interval:end,1:interval:end,i) .* scale_vec), ...
%     'AutoScale','off','LineWidth',0.5,'Color','k');

[cs,h]=contour(lon,lat,squeeze(tem(:,:,i)),temp_range,'linecolor',[.01 .01 .01],'linewidth',1);
clabel(cs,h,'fontsize',13,'labelspacing',300,'fontweight','bold');
% grid('linestyle','none','box','fancy','tickdir','in');
% gshhs_i('color','k'); m_gshhs_i('patch',[.6 .6 .6]);
