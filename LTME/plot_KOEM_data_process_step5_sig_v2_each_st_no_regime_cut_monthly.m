...
for varlist = { 'do','chl','ss','si','po4','no3','nh4','din','temp','salt','secchi'}
        ...
        eval(['mtx_regime_',varname,'_s = NaN(size(temp_sur_clim,1),size(temp_sur_clim,2),6);']);  % extracted data OUT [sp_gy, T, sig]
        eval(['mtx_regime_',varname,'_b_s = NaN(size(temp_sur_clim,1),size(temp_sur_clim,2),6);']);  % extracted data OUT [sp_gy, T, sig]
end
...
for varlist = { 'do','chl','ss','si','po4','no3','nh4','din','temp','salt','secchi'}
        ...
        for j=1:sp_gy %% num of spatial point
            for tti=1:length(obs_ti)
            for sig=1:6 %% sigma
                clearvars data data_ref leng_t
                eval(['leng_t = size(', varname,'_sur_clim(j,:),2);']);
                eval(['data_ref = ', varname,'_sur_clim(j,obs_ti(tti):12:leng_t);']);
                eval(['data = ', varname,'_sur_clim(j,obs_ti(tti):12:leng_t);']);
                data(data_ref > nanmean(data_ref) + sig*nanstd(data_ref)) =NaN;
                data(data_ref < nanmean(data_ref) - sig*nanstd(data_ref)) =NaN;
                eval(['mtx_regime_',varname,'_s(j,obs_ti(tti):12:leng_t,sig) = data;']);  % extracted data OUT [sp_gy, T, sig]
                disp(['mtx_regime_',varname,'_s'])
           
                if strcmp(varname,'secchi') == 0 % there are no secchi_bot
                    clearvars data data_ref leng_t
                    eval(['leng_t = size(', varname,'_bot_clim(j,:),2);']);
                    eval(['data_ref = ', varname,'_bot_clim(j,obs_ti(tti):12:leng_t);']);
                    eval(['data = ', varname,'_bot_clim(j,obs_ti(tti):12:leng_t);']);
                    data(data_ref > nanmean(data_ref) + sig*nanstd(data_ref)) =NaN;
                    data(data_ref < nanmean(data_ref) - sig*nanstd(data_ref)) =NaN;
                    eval(['mtx_regime_',varname,'_b_s(j,obs_ti(tti):12:leng_t,sig) = data;']);  % extracted data OUT [sp_gy, T, sig]
                   disp(['mtx_regime_',varname,'_b_s'])
                end
            end
            end
        end
...
for varlist = { 'do','chl','ss','si','po4','no3','nh4','din','temp','salt','secchi'}
    clearvars varname 
    varname = char(varlist); 
    eval(['mtx_regime_',varname,'_',num2str(1),'_s',' = ','mtx_regime_',varname,'_s(:,1:cut_1,:);']);
    eval(['mtx_regime_',varname,'_',num2str(2),'_s',' = ','mtx_regime_',varname,'_s(:,cut_1+1:cut_2,:);']);
    eval(['mtx_regime_',varname,'_',num2str(3),'_s',' = ','mtx_regime_',varname,'_s(:,cut_2+1:end,:);']);

    if strcmp(varname,'secchi') == 0 % there are no secchi_bot
        eval(['mtx_regime_',varname,'_',num2str(1),'_b_s',' = ','mtx_regime_',varname,'_b_s(:,1:cut_1,:);']);
        eval(['mtx_regime_',varname,'_',num2str(2),'_b_s',' = ','mtx_regime_',varname,'_b_s(:,cut_1+1:cut_2,:);']);
        eval(['mtx_regime_',varname,'_',num2str(3),'_b_s',' = ','mtx_regime_',varname,'_b_s(:,cut_2+1:end,:);']);
    end
end

save(['koem_monthly_gyonly_1to6sig_v2_each_st_16p_no_regime_cut_monthly.mat']); % 16 points 
    ...
    for varlist = { 'do','chl','ss','si','po4','no3','nh4','din','temp','salt','secchi'}
        ...
        for i = 1:3 % num of regime
            for j=1:sp_gy %% num of spatial point
                    ...
                    eval(['data_ref = ', varname,'_sur_clim(j,:);']);
                    eval(['data = ', varname,'_sur_clim(j,:);']);
                    eval(['mtx_regime_',varname,'_s(j,:) = data;']);  % no extracted data OUT [sp_gy, T, sig]
                    disp(['mtx_regime_',varname,'_s'])

                    if strcmp(varname,'secchi') == 0 % there are no secchi_bot
                        clearvars data data_ref
                        eval(['data_ref = ', varname,'_bot_clim(j,:);']);
                        eval(['data = ', varname,'_bot_clim(j,:);']);
                        eval(['mtx_regime_',varname,'_b_s(j,:) = data;']);  % no extracted data OUT [sp_gy, T, sig]
                       disp(['mtx_regime_',varname,'_b_s'])
                    end
...
save(['koem_monthly_gyonly_nosig_v2_each_st_16p_no_regime_cut.mat']); % 16 points 
end
...
save(['koem_climate_to06to15_v5_sig_gy_9points_no_regime_cut_monthly.mat'],'obs*','obm*','spm_obs*','tm_obs*'); % full 9points data (extract_sigma_for_each_st)
...
save(['koem_climate_to06to15_v5_nosig_gy_9points_no_regime_cut.mat'],'obs*','obm*','spm_obs*','tm_obs*'); % full 9points data (no extract_sigma_for_each_st)
end
