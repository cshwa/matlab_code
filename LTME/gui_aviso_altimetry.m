function varargout = gui_aviso_altimetry(varargin)
% Boots-up AVISO Altimetry GUI, interacts with user via interface.
% gui_aviso_altimetry M-file for gui_aviso_altimetry.fig
%
% INPUT(S):
%
%   [none]
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @gui_aviso_altimetry_OpeningFcn, ...
                   'gui_OutputFcn',  @gui_aviso_altimetry_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before gui_aviso_altimetry is made visible.
function gui_aviso_altimetry_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to gui_aviso_altimetry (see VARARGIN)

DSI=load('dsi_aviso_altimetry.mat');
handles.DSI=DSI;
...
% Set interface name.
filename = 'get_aviso_altimetry';
txt = ['NASA/REASoN Ocean Data Portal - '];
tmp1 = ['AVISO Altimetry GUI'];
tmp2 = '';
txt = [txt,tmp1];
if (~isdeployed)
...
CX=(NX+8)/ppcx;         % button size in characters including 4 pixel grace margin
CY=(NY+12)/ppcy;
set(handles.pushbuttonNOOP,'CData',img)
pos = get(handles.pushbuttonNOOP,'position'); %units will be in characters
set(handles.pushbuttonNOOP,'position',[pos(1),pos(2),CX,CY]);  

[img,MAP]=imread('logo_aviso.jpg');
[NY,NX,N3]=size(img);
CX=(NX+8)/ppcx;         % button size in characters including 4 pixel grace margin
CY=(NY+12)/ppcy;
set(handles.pushbuttonAviso,'CData',img)
pos = get(handles.pushbuttonAviso,'position'); %units will be in characters
set(handles.pushbuttonAviso,'position',[pos(1),pos(2),CX,CY]);

[img,MAP]=imread('logo_apdrc.jpg');
[NY,NX,N3]=size(img);
CX=(NX+8)/ppcx;         % button size in characters including 4 pixel grace margin
CY=(NY+12)/ppcy;
set(handles.pushbutton4,'CData',img)
pos = get(handles.pushbutton4,'position'); %units will be in characters
set(handles.pushbutton4,'position',[pos(1),pos(2),CX,CY]);
...
% absent.
if ~exist('gui_aviso_altimetry_request.mat','file') 
...
% Choose default command line output for gui_aviso_altimetry
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes gui_aviso_altimetry wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = gui_aviso_altimetry_OutputFcn(hObject, eventdata, handles) 

% Get default command line output from handles structure
varargout{1} = handles.output;

function popupmenuProduct_Callback(hObject, eventdata, handles)
CheckRanges(hObject, eventdata, handles)
function popupmenuProduct_CreateFcn(hObject, eventdata, handles)

function radiobuttonDelayedTime_Callback(hObject, eventdata, handles)
CheckExclusive(hObject, eventdata, handles,{'radiobuttonDelayedTime'},{'radiobuttonNearRealTime'})
CheckRanges(hObject, eventdata, handles)
function radiobuttonNearRealTime_Callback(hObject, eventdata, handles)
CheckExclusive(hObject, eventdata, handles,{'radiobuttonNearRealTime'},{'radiobuttonDelayedTime'})
CheckRanges(hObject, eventdata, handles)
...
filename = 'gui_aviso_altimetry_request.mat';
%if (~isdeployed)
...
get_aviso_altimetry(request);

set(hObject,'Enable','On','String','Get Data','BackgroundColor',[0 1. 0.25]);

function pushbuttonAviso_Callback(hObject, eventdata, handles)
web('http://www.aviso.oceanobs.com/en/data/data-access-services/opendap/index.html');
web 'http://www.aviso.oceanobs.com/en/data/data-access-services/opendap/index.html' -browser
...
function pushbuttonNOOP_Callback(hObject, eventdata, handles)
...
R.Satellite=DSI.Satellite;
R.TimeMode=DSI.TimeMode;
R.SpatialCoverage=DSI.SpatialCoverage;
...
%load dsi_aviso_altimetry.mat
List=handles.DSI.List;

Satellite=R.Satellite;
TimeMode=R.TimeMode;
SpatialCoverage=R.SpatialCoverage;
DataSetBranch=R.DataSetBranch;

set(handles.popupmenuProduct,'String',List.Satellite(:,2))
j = strmatch(Satellite,List.Satellite(:,1),'exact');
set(handles.popupmenuProduct,'Value',j)

j = strmatch(TimeMode,List.TimeMode(:,1),'exact');
if j==1
    set(handles.radiobuttonDelayedTime,'Value',1);
    set(handles.radiobuttonNearRealTime,'Value',0);
else
    set(handles.radiobuttonDelayedTime,'Value',0);
    set(handles.radiobuttonNearRealTime,'Value',1);
end

%SpatialCoverage
[N1,N2]=size(List.SpatialCoverage);
jj=[];
for k=1:N1
j = strmatch(cell2mat(['AVISO_Altimetry_' Satellite '_' TimeMode '_' List.SpatialCoverage(k,1)]),List.DataSetBranch(:,1),'exact');
    if ~isempty(j)
        jj=[jj ;k];
    end
end
SpatialCoverageL=List.SpatialCoverage(jj,:);

set(handles.popupmenuSpatialResolution,'String',SpatialCoverageL(:,2))
j = strmatch(SpatialCoverage,SpatialCoverageL(:,1),'exact');
set(handles.popupmenuSpatialResolution,'Value',j)

% Selected DataSetBranch
eval(['DSI=handles.DSI.DSI_' DataSetBranch ';'])
%DSI.List=handles.DSI.List;
...
if isempty(get(handles.popupmenuMode,'Value'))
...
S=S(j);
j = strmatch(S,List.Satellite(:,2),'exact');
Satellite=List.Satellite(j,1);

j=2;
if get(handles.radiobuttonDelayedTime,'Value')==1
    j=1;
end
TimeMode=List.TimeMode(j,1);

S=get(handles.popupmenuSpatialResolution,'String');
j=get(handles.popupmenuSpatialResolution,'Value');
if ~iscell(S) S={S}; end
S=S(j);
j = strmatch(S,List.SpatialCoverage(:,2),'exact');
SpatialCoverage=List.SpatialCoverage(j,1);

% Selected DataSetBranch
DataSetBranch=cell2mat(['AVISO_Altimetry_' Satellite '_' TimeMode '_' SpatialCoverage]);
eval(['DSI=handles.DSI.DSI_' DataSetBranch ';'])
%DSI.List=List;

function CheckRanges(hObject, eventdata, handles)
% depending on the checked GUI buttons update appearence of GUI menu
%load dsi_aviso_altimetry.mat % List
List=handles.DSI.List;

% Satellite
SatelliteL=List.Satellite;
set(handles.popupmenuProduct,'String',SatelliteL(:,2));
j=get(handles.popupmenuProduct,'Value');
if isempty(j)
    j=1;
end
Satellite=SatelliteL(j,1);

%TimeMode
set(handles.radiobuttonDelayedTime,'Enable','Off')
j = strmatch(cell2mat(['AVISO_Altimetry_' Satellite '_' List.TimeMode(1,1)]),List.DataSetBranch(:,1));
if ~isempty(j)
set(handles.radiobuttonDelayedTime,'Enable','On')
end
set(handles.radiobuttonNearRealTime,'Enable','Off')
j = strmatch(cell2mat(['AVISO_Altimetry_' Satellite '_' List.TimeMode(2,1)]),List.DataSetBranch(:,1));
if ~isempty(j)
set(handles.radiobuttonNearRealTime,'Enable','On')
end

if get(handles.radiobuttonDelayedTime,'Value')==0 & get(handles.radiobuttonNearRealTime,'Value')==0
    set(handles.radiobuttonNearRealTime,'Value',1)
end
if     get(handles.radiobuttonDelayedTime,'Value')==1 & strcmp(get(handles.radiobuttonDelayedTime,'Enable'),'on')
    j=1;
elseif get(handles.radiobuttonDelayedTime,'Value')==1 & strcmp(get(handles.radiobuttonDelayedTime,'Enable'),'off')
    set(handles.radiobuttonDelayedTime,'Value',0)
    set(handles.radiobuttonNearRealTime,'Value',1)
    j=2;
elseif get(handles.radiobuttonNearRealTime,'Value')==1 & strcmp(get(handles.radiobuttonNearRealTime,'Enable'),'on')
    j=2;
elseif get(handles.radiobuttonNearRealTime,'Value')==1 & strcmp(get(handles.radiobuttonNearRealTime,'Enable'),'off')
    set(handles.radiobuttonDelayedTime,'Value',1)
    set(handles.radiobuttonNearRealTime,'Value',0)
    j=1;
end
...
set(handles.popupmenuSpatialResolution,'String',SpatialCoverageL(:,2));
j=get(handles.popupmenuSpatialResolution,'Value');
if j>length(SpatialCoverageL(:,1))
    j=1; set(handles.popupmenuSpatialResolution,'Value',j)
end
SpatialCoverage=SpatialCoverageL(j,1);
...
set(handles.popupmenuTemporalAverages,'String',DSI.TRes,'Value',1);

% CheckBoxes
L=[1:1:9]; % possible checkbox numbers
for k=1:length(L) % first make all field checkboxes inactive
CBNUM=num2str(L(k));
eval(['set(handles.checkbox' CBNUM ',''Enable'',''off'')'])
end
for k=1:length(DSI.Fields) % make active only checked fields
...
% example: Us={'radiobuttonDelayedTime'}; Them={'radiobuttonNearRealTime';'radiobuttonInWksp'}
for k=1:length(Us)
