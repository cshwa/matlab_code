...
% for snu_desktopd
all_testname2 = {'ens01'};
test_name = ['rain_hsimt_2yr']

for testnameind2=1:length(all_testname2)

% testname=all_testname2{testnameind2}  
% testname= num2str(min(inputyear),'%04i')
...
% elseif (strcmp(system_name,'GLNXA64'))
% end 
% filedir = strcat('D:\장기생태\Dynamic\result\yellow'); % % where data files are
% filedir = strcat('J:\fennel_NWP\Output\test1\2011'); % % where data files are
filedir = strcat('J:\fennel_NWP\Output\rain_hsimt\2011\2yr'); % % where data files are
...
%         filename = strcat(filedir,'\', 'spinup',num2str(spin_yr),'_monthly',num2str(tempmonth,'%02i'), '.nc');
        filename = strcat(filedir,'\', 'roms_monthly_avg.nc');
        file_param = strcat(filedir,'\','roms_his_0001.nc');
        % read model data
        if (exist('lon')==0)
            modelinfo=ncinfo(filename);
            lon = ncread(filename,'lon_rho',[1 1],[modelinfo.Dimensions(6).Length,1]);
            lat = ncread(filename,'lat_rho',[1 1],[1,modelinfo.Dimensions(5).Length]);
            ...
            Vtransform= ncread(file_param,'Vtransform');
            Vstretching= ncread(file_param,'Vstretching');
            theta_s= ncread(file_param,'theta_s');
            theta_b= ncread(file_param,'theta_b');
            hc= ncread(file_param,'hc');
            mask= ncread(filename,'mask_rho', [lon_min(1) lat_min(1)], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1]);
            N = modelinfo.Dimensions(10).Length;
        end
        zeta = ncread(filename,'zeta',[lon_min(1) lat_min(1) tempmonth-1], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 1]);
        model_depth_z=zlevs(Vtransform,Vstretching,model_depth,zeta,theta_s,theta_b,hc,N,'r');
%         model_depth_z=zlevs(model_depth,zeta,theta_s,theta_b,hc,N,'r');
        model_depth_z=permute(model_depth_z,[2, 3, 1]);
        data_info = ncinfo(filename, varname);  %% [lon lat depth time] -> [1601 1201 33 1]

        data = ncread(filename,varname,[lon_min(1) lat_min(1) 1 tempmonth-1], [lon_max(1)-lon_min(1)+1 lat_max(1)-lat_min(1)+1 N 1]);
           ...
           for latij=1:size(data,2)
              temp_stddepth=stddepth(stddepth<=max(-model_depth_z(lonij,latij,end:-1:1)));
               if (isnan(data(lonij,latij,40))==1 || length(temp_stddepth)<=1)
                   data_zdepth(lonij,latij,1:length(temp_stddepth))=NaN;
               else
                   %% depth --> 0 10 ...
                    ...
                    if (isnan(NIFS6.lon(yearij,numline,numsta,monthij,1))==1)
                       MODEL6.salt(yearij,numline,numsta,monthij,:)=NaN;
                        ...
                        % nearest point
                            ...
                            clearvars temp_2d near_temp dist_2d near_point
                            temp_2d = sqrt((lon - NIFS6.lon(yearij,numline,numsta,monthij,1)).^2 + (lat - NIFS6.lat(yearij,numline,numsta,monthij,1)).^2);
%                             temp_2d = temp_2d .* (mask./mask); % masking
%                             extrapolate east sea, that should be NaN.
                            near_temp = find(nanmin(nanmin(temp_2d))==temp_2d);
                            near_point = near_temp(1);
                            dist_2d = temp_2d;
                  
                             clearvars lon_sta_min lat_sta_min
                             [lon_sta_min,lat_sta_min]=ind2sub(size(lon),near_point); %% 1d to 2d index
...
%                         lat_sta_min=floor(sta_min/size(lon,1));
                        if length(find(isnan(squeeze(data_zdepth(lon_sta_min,lat_sta_min,:)))==0)) >= 1
                        MODEL6.salt(yearij,numline,numsta,monthij,1:length(stddepth))=squeeze(data_zdepth(lon_sta_min,lat_sta_min,:));
                        else
                        MODEL6.salt(yearij,numline,numsta,monthij,1:length(stddepth))=NaN; 
                        end
                    end
            end
        end
...
%                 if MODEL6.salt(numyear,numline,numsta,nummon,:) ~= 0 & isnan(MODEL6.salt(numyear,numline,numsta,nummon,:)) ~= 1 %cshwa 177~181 added
%                     MODEL6.salt75_sta(numyear,numline,numsta,nummon)=mean(MODEL6.salt(numyear,numline,numsta,nummon,1:6),5);
%                     % below 75m depth point will be NaN;
                    MODEL6.salt75_sta(numyear,numline,numsta,nummon)=mean(MODEL6.salt(numyear,numline,numsta,nummon,1:6),5,'omitnan'); %origin
%                 elseif MODEL6.salt(numyear,numline,numsta,nummon,:) == 0 
%                     MODEL6.salt75_sta(numyear,numline,numsta,nummon)=mean(MODEL6.salt(numyear,numline,numsta,nummon,:));
%                 end
...
save(['model_fennelNWP_',test_name,'_salt_trend_',num2str(min(inputyear),'%04i'),'_',num2str(max(inputyear),'%04i'),'_fix75.mat'], 'MODEL6', 'NIFS6');
