...
for varlist = { 'do','chl','ss','si','po4','no3','nh4','din','temp','salt','secchi'}
        ...
        for j=1:sp_gy %% num of spatial point
            for sig=1:6 %% sigma
                ...
                data(data_ref < nanmean(data_ref) - sig*nanstd(data_ref)) =NaN;
                eval(['mtx_regime_',varname,'_s(j,:,sig) = data;']);  % extracted data OUT [sp_gy, T, sig]
                ...
                if strcmp(varname,'secchi') == 0 % there are no secchi_bot
                    ...
                    data(data_ref > nanmean(data_ref) + sig*nanstd(data_ref)) =NaN;
                    data(data_ref < nanmean(data_ref) - sig*nanstd(data_ref)) =NaN;
                    eval(['mtx_regime_',varname,'_b_s(j,:,sig) = data;']);  % extracted data OUT [sp_gy, T, sig]
...
save(['koem_monthly_gyonly_1to6sig_v2_each_st_16p_no_regime_cut.mat']); % 16 points 
...
save(['koem_climate_to06to15_v5_sig_gy_9points_no_regime_cut.mat'],'obs*','obm*','spm_obs*','tm_obs*'); % full 9points data (extract_sigma_for_each_st)
