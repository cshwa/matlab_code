...
year_ff = {'1982to1986','1987to1992','1993to2004'};

for t_i = 1:3  % regime

clearvars -except t_i year_ff

%% KOEM
%  from polynomial_fitting_on_ecological_variable_v3_3sig(boundary)_chl_1997to2004.m
% there are 2 regimes for nh4 (KOEM 7st.)
koem=load('koem_input_fix_1997to2004_3sig(3regime)_v7.mat'); % 1993to2004 (start year varied for each variable)
if t_i == 1
...
%% it's kodc
%% from plot_KODC_7_20501_polynomial_1982to2004.m
...
%  1982~1986, 1987~1992, 1993~2004
kodc205 = load('kodc_input_fix_1982to2004_3sig(3regime)_20501_v7.mat'); % 1982to2004 (start year varied for each variable)

if t_i == 1
    y_s_no3_205 = kodc205.mon_clim_sur_no3_2_in; %%% mg/L to mM/m^3;
    y_b_no3_205 = kodc205.mon_clim_bot_no3_2_in; %%% mg/L to mM/m^3;
elseif t_i == 2
    y_s_no3_205 = kodc205.mon_clim_sur_no3_2_in; %%% mg/L to mM/m^3;
    y_b_no3_205 = kodc205.mon_clim_bot_no3_2_in; %%% mg/L to mM/m^3;
elseif t_i == 3
    y_s_no3_205 = kodc205.mon_clim_sur_no3_3_in; %%% mg/L to mM/m^3;
    y_b_no3_205 = kodc205.mon_clim_bot_no3_3_in; %%% mg/L to mM/m^3;
end
...
%%  plot_KODC_7_40016_polynomial_partial_1982to2004.m
%  there are 3 regimes for no3 (KODC)
%  1997~2006, 2007~2015, 2016~2019
% kodc400=load('kodc_input_fix_to06to15_3sig(3regime)_40016.mat');
%  1997~2006, 2007~2015, 2016~2020
kodc400=load('kodc_input_fix_1982to2004_3sig(3regime)_40016_v7.mat'); % 1982to2004 (start year varied for each variable)

if t_i == 1
    y_s_no3_400 = kodc400.mon_clim_sur_no3_2_in; %%% mg/L to mM/m^3;
    y_b_no3_400 = kodc400.mon_clim_bot_no3_2_in; %%% mg/L to mM/m^3;
elseif t_i == 2
    y_s_no3_400 = kodc400.mon_clim_sur_no3_2_in; %%% mg/L to mM/m^3;
    y_b_no3_400 = kodc400.mon_clim_bot_no3_2_in; %%% mg/L to mM/m^3;
elseif t_i == 3
    y_s_no3_400 = kodc400.mon_clim_sur_no3_3_in; %%% mg/L to mM/m^3;
    y_b_no3_400 = kodc400.mon_clim_bot_no3_3_in; %%% mg/L to mM/m^3;
end
...
if t_i == 1
    lon_rho = ncread('grid_sumjin_v1970_fix_3m_v3.nc','lon_rho');
    lat_rho = ncread('grid_sumjin_v1970_fix_3m_v3.nc','lat_rho');
    h = ncread('grid_sumjin_v1970_fix_3m_v3.nc','h');
else
    lon_rho = ncread('grid_gy_v11_s.nc','lon_rho');
    lat_rho = ncread('grid_gy_v11_s.nc','lat_rho');
    h = ncread('grid_gy_v11_s.nc','h');
end
...
save(['boundary_bio_input_1982to2004_',char(year_ff(t_i)),'regime_v7.mat'],'*_input*');
save(['boundary_bio_input_for_ini_1982to2004_',char(year_ff(t_i)),'regime_v7.mat'],'*_in');
save(['check_boundary_no3_input_1982to2004_',char(year_ff(t_i)),'regime_v7.mat'],'y_s_no3_205','y_b_no3_205');
end
