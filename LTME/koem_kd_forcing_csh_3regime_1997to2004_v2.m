...
% from plot_KOEM_data_process_step5_sig_v2_each_st_1997to2004.m
% yj=load(['koem_monthly_gyonly_1to6sig_v2_each_st_16p_to06to15.mat']); % 16 points 
yj=load(['koem_monthly_gyonly_1to6sig_v2_each_st_16p_1997to2004.mat']); % 16 points 
...
plot(t,kd_c_1_s(~isnan(kd_c_1_s)),'r'); 
...
figure; hold on;
plot(mon_clim_kd_c_1,'ro');  grid on;
disp(nanmean(mon_clim_kd_c_1)); 
...
figure; hold on;
plot(mon_clim_secchi_1,'r');  grid on;

figure; hold on;
plot(mon_clim_secchi_sp_1,'ro');grid on;
disp(nanmean(mon_clim_secchi_sp_1)); 

figure; hold on;
plot(mon_clim_kd_sp_1,'ro');  grid on;
disp(nanmean(mon_clim_kd_sp_1)); 
...
day_clim_kd_sp_1_in= day_clim_kd_sp_1(366:730); 

day_clim_kdc_sp_1_in= day_clim_kdc_sp_1(366:730); 

figure; hold on;
plot(day_clim_kd_sp_1_in,'r');  grid on;
disp(nanmean(day_clim_kd_sp_1_in)); 
xlim([1 365]); grid on; ylabel('Kd coefficient'); xlabel('days')
plot(day_clim_kdc_sp_1_in,'r--'); grid on;
disp(nanmean(day_clim_kdc_sp_1_in)); 
...
save('Koem_kd_days_climate_1997to2004_v2.mat','day_clim_*');
return
