...
filename = strcat('E:\Data\Model\ROMS\nwp_1_20\test37\',year,'\',testname,'_monthly_',year,'_',month,filename_suffix); %% read grid
...
oisstlon = ncread('E:\Data\Observation\OISST\monthly\avhrr-monthly_201312.nc','lon');  %% read oisst grid
oisstlat = ncread('E:\Data\Observation\OISST\monthly\avhrr-monthly_201312.nc','lat');  %% read oisst grid
...
% % for climate data, from MEPL Ocean server(stlee)
    oiname ='E:\Data\Observation\OISST\OISST_MonthClim_1982-2011.nc';
...
%     oisst(:,:) = ncread(oiname,'interpolated_sst', [1 1 month], [length(oisstlon), length(oisstlat) 1]) * oisst_scale_factor; 
    oisst(:,:) = ncread(oiname,'interpolated_sst', [1 1 str2num(month)], [length(oisstlon), length(oisstlat) 1]); 
