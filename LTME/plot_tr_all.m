...
for i=1:length(year)
    ...
    spinupyear=num2str(year(i)-refyear+1,'%04i');
    for month=1:12
...
for i=1:length(year)
    korea_obs_spinup((12*(i-1))+1:(12*(i-1))+12)=tr_obs_30y(find(year_obs_30y == refyear))';    %% get observation transport at the korea strait
end
...
% % % East Sea transport (spinup)

es4plot=plot(xData, korea_obs_spinup,'y');
hold on
...
for i=1:length(year)
    figdir =strcat('E:\Data\Model\ROMS\nwp_1_20\',testname,'\spinup\',tempyear,'\figures\transport\'); % % where figure files will be saved
...
% % % East China Sea transport (spinup)

ecs4plot=plot(xData, korea_obs_spinup,'y');
hold on
...
% set(gca,'XLim',[1 12*8]);
% set(gca,'XLim',[1 72]);
set(ecs1plot,'LineWidth',4);
set(ecs2plot,'LineWidth',4);
set(ecs3plot,'LineWidth',4);
set(ecs4plot,'LineWidth',4);
