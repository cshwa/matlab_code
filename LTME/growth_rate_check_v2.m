...
% regime3_kdc_zerosw=load('kd_double_3reg.mat','obm_*','gy_*','jj_*','sgy_*','egy_*','obs_std_*','eom_d_each','sp_gy_*','name_tag','sp_gy');
regime3_kdc_zerosw=load('kd_t4_3reg.mat','obm_*','gy_*','jj_*','sgy_*','egy_*','obs_std_*','eom_d_each','sp_gy_*','name_tag','sp_gy');
...
% kd_1reg=ncread('J:\장기생태_2021\Dynamic\result\3regime\day_clim_ERA5_double_1regime_AttFac.nc','AttFac');
% kd_2reg=ncread('J:\장기생태_2021\Dynamic\result\3regime\day_clim_ERA5_double_2regime_AttFac.nc','AttFac');
% kd_3reg=ncread('J:\장기생태_2021\Dynamic\result\3regime\day_clim_ERA5_double_3regime_AttFac.nc','AttFac');
...
mu_0 = 5.0; %Eppley temperature-limited growth param.
coeffi = 0.59 ; %default
% coeffi = 0.69
% coeffi = 2.5
...
% alpha = 0.05; % P-I initial slop default
% alpha = 0.01; % P-I initial slop phyIS
% alpha = 0.007; % P-I initial slop phyIS
...
for i=20:-1:2
    clearvars att
att = squeeze((z_ref(:,i,:)-z_ref(:,i-1,:)).*((kd_times .* kd_1reg(1:size(z_ref,1),1,1:size(z_ref,3))) + (Kchl .* regime1_kdc_zerosw.sp_gy_chl_mid(:,i,:))));
if i==20
...
for i=20:-1:2
    clearvars att
att = squeeze((z_ref_2reg(:,i,:)-z_ref_2reg(:,i-1,:)).*( (kd_times .* kd_2reg(1:size(z_ref_2reg,1),1,1:size(z_ref_2reg,3))) + (Kchl .* regime2_kdc_zerosw.sp_gy_chl_mid(:,i,:))));
if i==20
...
for i=20:-1:2
    clearvars att
att = squeeze((z_ref_3reg(:,i,:)-z_ref_3reg(:,i-1,:)).*((kd_times .* kd_3reg(1:size(z_ref_3reg,1),1,1:size(z_ref_3reg,3))) + (Kchl .* regime3_kdc_zerosw.sp_gy_chl_mid(:,i,:))));
if i==20
