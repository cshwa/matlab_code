...
legend('jinwall','gure');
set(gca,'xtick',[1:12:length(monthly_nh4_jin)]);
set(gca,'xticklabel',2010:2019,'fontsize',10);
xlim([1 length(monthly_nh4_jin)])
...
plot(length(monthly_do_song)-length(monthly_do_jin)+1+36:length(monthly_do_song)-length(monthly_do_jin)+length(mon_in_nh4_jin)+36,mon_in_nh4_jin,'g','linew',1);
...
for i = 1:12
clim_nh4(i) =nanmean(mon_in_nh4_song(i:12:end));
clim_nh4_jin(i) = nanmean(mon_in_nh4_jin(i:12:end));
clim_nh4_recon(i) = nanmean(yp_recon_nh4(i:12:end));
end


figure; hold on;
% yp_recon_nh4 = polyval(pf_nh4,mon_in_nh4_song);
plot(clim_nh4,'b','linew',2);
plot(clim_nh4_jin,'g','linew',2);
plot(clim_nh4_recon,'r','linew',2);
ylabel('NH4 (mmol N /m^3)')
xlabel('month')
title('NH4 reconst.from monthly mean')
legend('songjung','jinwal','recon');
