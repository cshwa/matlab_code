function interp_OGCM_HYC(OGCM_dir,OGCM_prefix,year,month,Roa,interp_method,...
...
% Open the OGCM file
%
nc=netcdf([OGCM_dir,OGCM_prefix,num2str(year),num2char(month,2),'.cdf']);
%
...
zeta=ext_data_OGCM_224(nc,lonT,latT,'ssh',tin,lon,lat,1,Roa,interp_method);
...
for k=1:NZ
  ...
  u2d=ext_data_OGCM_224(nc,lonU,latU,'u',tin,lon,lat,...
                    k,Roa,interp_method);
  v2d=ext_data_OGCM_224(nc,lonV,latV,'v',tin,lon,lat,...
                    k,Roa,interp_method);
  u(k,:,:)=rho2u_2d(u2d.*cosa+v2d.*sina);
  v(k,:,:)=rho2v_2d(v2d.*cosa-u2d.*sina);
  temp(k,:,:)=ext_data_OGCM_224(nc,lonT,latT,'temp',tin,lon,lat,...
                            k,Roa,interp_method);
  salt(k,:,:)=ext_data_OGCM_224(nc,lonT,latT,'salt',tin,lon,lat,...
                            k,Roa,interp_method);

end

dz=-(Z(2:end,1)-Z(1:end-1,1));
dz=[-Z(1,1);dz];
dzU=repmat(dz,[1,M,L-1]);
ubar=squeeze(sum(u.*dzU)./sum(dzU));
dzV=repmat(dz,[1,M-1,L]);
...
%
  if obc(1)==1
    [u_south,v_south,...
     temp_south,salt_south]=vinterp_OGCM_bry224(zr(:,1,:),zu(:,1,:),zv(:,1,:),...
                                             dzr(:,1,:),dzu(:,1,:),dzv(:,1,:),...
  ...
  if obc(2)==1
    [u_east,v_east,...
     temp_east,salt_east]=vinterp_OGCM_bry224(zr(:,:,end),zu(:,:,end),zv(:,:,end),...
  ...
  if obc(3)==1
    [u_north,v_north,...
     temp_north,salt_north]=vinterp_OGCM_bry224(zr(:,end,:),zu(:,end,:),zv(:,end,:),...
                                             dzr(:,end,:),dzu(:,end,:),dzv(:,end,:),...
  ...
  if obc(4)==1
    [u_west,v_west...
     temp_west,salt_west]=vinterp_OGCM_bry224(zr(:,:,1),zu(:,:,1),zv(:,:,1),...
                                           dzr(:,:,1),dzu(:,:,1),dzv(:,:,1),...
