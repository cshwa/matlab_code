...
% 
% r_po4_txt=[r_txt_ud(2:end,23)];
% for i = 1:length(r_po4_txt)
...
% ylabel('po4 (mg/L)','fontsize',13)
% title('sumjin(songjung)-polynomial po4.','fontsize',13)
% grid on
% set(gca,'fontsize',13)
% ylim([0 0.1]) 
% xlim([1 366])
...
% grid on;
% xlim([1 length(raw_tn)])
% title('sumjin(songjung)- po4 raw.','fontsize',13)
% legend('po4')
% set(gca,'xtick',[t_tick_all(1:3:end)]);
% set(gca,'xticklabel',1989:3:2019,'fontsize',10);
% set(gca,'fontsize',13)
% 
% 
% save('sumjin(songjung)_polynomial_climate_to2004_advanced(v4)_3sig_po4.mat'); 
...
%divide the data
% 1989 ~ 2004
regime_po4 = raw_po4(1:t_tick(end));
...
% 2004 ~ 2019
regime_po4_af = raw_po4(t_tick(end)+1:end);
...
for i = 1:length(regime_indx)
    if size(regime_indx{i},1) == 0
        ...
        reg_clim_po4(i) = nanmean(regime_po4(regime_indx{i}));
...
for i = 1:length(regime_indx_af)
    if size(regime_indx_af{i},1) == 0  
        ...
        reg_af_clim_po4(i) = nanmean(regime_po4_af(regime_indx_af{i}));
...
%po4
clearvars idx1
idx1 = find(isnan(reg_clim_po4) == 0);
reg_clim_po4_te=reg_clim_po4;
upper_bc_po4(sig) = mean(reg_clim_po4(idx1)) + sig*std(reg_clim_po4(idx1));
lower_bc_po4(sig) = mean(reg_clim_po4(idx1)) - sig*std(reg_clim_po4(idx1));
reg_clim_po4_te(find(reg_clim_po4 > mean(reg_clim_po4(idx1)) + sig*std(reg_clim_po4(idx1))))=NaN;
reg_clim_po4_te(find(reg_clim_po4 < mean(reg_clim_po4(idx1)) - sig*std(reg_clim_po4(idx1))))=NaN;
nanmean(reg_clim_po4_te(idx1))

clearvars xp_w_po4 pf_w_po4
xp = 1:366;
xp_w_po4 = find(isnan(reg_clim_po4_te)==0);
pf_w_po4 = polyfit(xp_w_po4,reg_clim_po4_te(xp_w_po4),3);
yp_w_po4_04 = polyval(pf_w_po4,xp);

clearvars idx1
idx1 = find(isnan(reg_af_clim_po4) == 0);
reg_af_clim_po4_te=reg_af_clim_po4;
upper_bc_po4_af(sig) = mean(reg_af_clim_po4(idx1)) + sig*std(reg_af_clim_po4(idx1));
lower_bc_po4_af(sig) = mean(reg_af_clim_po4(idx1)) - sig*std(reg_af_clim_po4(idx1));
reg_af_clim_po4_te(find(reg_af_clim_po4 > mean(reg_af_clim_po4(idx1)) + sig*std(reg_af_clim_po4(idx1))))=NaN;
reg_af_clim_po4_te(find(reg_af_clim_po4 < mean(reg_af_clim_po4(idx1)) - sig*std(reg_af_clim_po4(idx1))))=NaN;
nanmean(reg_af_clim_po4_te(idx1))

clearvars xp_w_po4 pf_w_po4
xp = 1:366;
xp_w_po4 = find(isnan(reg_af_clim_po4_te)==0);
pf_w_po4 = polyfit(xp_w_po4,reg_af_clim_po4_te(xp_w_po4),3);
yp_w_po4_af = polyval(pf_w_po4,xp);
...
%% po4
figure;
scatter(1:366,reg_clim_po4_te,'r');
hold on
plot(1:366, yp_w_po4_04,'--','color','r','linew',2)
xlabel('time(days)','fontsize',13)
ylabel('po4 (mg/L)','fontsize',13)
...
% ylim([2 18]) 
xlim([1 366])
scatter(1:366,reg_af_clim_po4_te,'k');
hold on
plot(1:366, yp_w_po4_af,'--','color','k','linew',2)
xlabel('time(days)','fontsize',13)
ylabel('po4 (mg/L)','fontsize',13)
title('sumjin(songjung)-polynomial po4.','fontsize',13)
grid on
...
%% po4
figure;
scatter(1:366,reg_af_clim_po4_te);
hold on
plot(1:366, yp_w_po4_af,'--','color','r','linew',2)
xlabel('time(days)','fontsize',13)
ylabel('po4 (mg/L)','fontsize',13)
title('sumjin(songjung)-polynomial po4.','fontsize',13)
grid on
...
save('sumjin(songjung)_polynomial_climate_to2011_3sig_po4.mat','yp_w_po4_04','yp_w_po4_af'); 
...
% raw
w_po4_04=w_po4(1:t_tick(end));
idx1 = find(isnan(w_po4_04) == 0);
w_po4_042=w_po4_04;
upper_bc(sig) = mean(w_po4_04(idx1)) + sig*std(w_po4_04(idx1));
lower_bc(sig) = mean(w_po4_04(idx1)) - sig*std(w_po4_04(idx1));
mean(w_po4_04(idx1))
w_po4_042(find(w_po4_04 > mean(w_po4_04(idx1)) + sig*std(w_po4_04(idx1))))=NaN;
w_po4_042(find(w_po4_04 < mean(w_po4_04(idx1)) - sig*std(w_po4_04(idx1))))=NaN;
 
%regression
%slope y = b1*x
nonan=w_po4_042(~isnan(w_po4_042));
idx = find(isnan(w_po4_042) == 0);
nanaxisx=find(isnan(w_po4_042) == 1);
...
yCalc2{sig} = [1:length(w_po4_04)].*b1(sig) + b0(sig);
% plot(yCalc2{sig},'-.','color','k','linew',2)
% plot([1:1:length(w_po4_04)], upper_bc(sig).*ones(length(w_po4_04)),'color', color_spec(sig),'linew',2)
% plot([1:1:length(w_po4_04)], lower_bc(sig).*ones(length(w_po4_04)),'color', color_spec(sig),'linew',2)
line([tx_tick(13) length(w_po4_04)], [upper_bc(sig) upper_bc(sig)],'color', color_spec(sig),'linew',2)
line([tx_tick(13) length(w_po4_04)], [lower_bc(sig) lower_bc(sig)],'color', color_spec(sig),'linew',2)
yCalc3 = yCalc2{sig};
% plot([1:1:length(w_po4_04)],mean(w_po4_04(idx1)).*ones(length(w_po4_04)),'-.','color',color_spec(sig-1),'linew',2);
line([tx_tick(13) length(w_po4_04)],[mean(w_po4_04(idx1)) mean(w_po4_04(idx1))],'color',color_spec(sig-1),'linew',2);
%     for i = 1:length(yCalc3)
%             w_po4_recon(i) = yCalc3(i)  + yp_w_po4(indx_366{i});
%     end
% plot([1:length(w_po4)], w_po4_recon,'-.','color',color_spec(sig),'linew',1)
end

for sig=3:3  %sigma
clearvars b1 yCalc1 X b nonan idx nanaxisx *_mth_f idx1 sig_txt *_mi2 *_recon yCalc3
% raw
w_po4_04=w_po4(t_tick(end)+1:end);
idx1 = find(isnan(w_po4_04) == 0);
w_po4_042=w_po4_04;
upper_bc(sig) = mean(w_po4_04(idx1)) + sig*std(w_po4_04(idx1));
lower_bc(sig) = mean(w_po4_04(idx1)) - sig*std(w_po4_04(idx1));
...
b0(sig) = b(1);  b1(sig) = b(2);
yCalc2{sig} = [1:length(w_po4_04)].*b1(sig) + b0(sig);
% plot([t_tick(end)+1:1:t_tick(end)+length(w_po4_04)],yCalc2{sig},'-.','color','k','linew',2)
% plot([tx_tick(25)+1:1:tx_tick(25)+length(w_po4_04)], upper_bc(sig).*ones(length(tx_tick(25)+1:1:tx_tick(25)+length(w_po4_04))),'color', color_spec(sig),'linew',2)
% plot([tx_tick(25)+1:1:tx_tick(25)+length(w_po4_04)], lower_bc(sig).*ones(length(tx_tick(25)+1:1:tx_tick(25)+length(w_po4_04))),'color', color_spec(sig),'linew',2)
line([t_tick(end)+1 t_tick(end)+length(w_po4_04)], [upper_bc(sig) upper_bc(sig)],'color', color_spec(sig),'linew',2)
line([t_tick(end)+1 t_tick(end)+length(w_po4_04)], [lower_bc(sig) lower_bc(sig)],'color', color_spec(sig),'linew',2)
yCalc3 = yCalc2{sig};
line([t_tick(end)+1 t_tick(end)+length(w_po4_04)],[mean(w_po4_04(idx1)) mean(w_po4_04(idx1))],'color',color_spec(sig-1),'linew',2);
...
title(['Namgang po4 raw  ' num2str(sig) '-sigma'],'fontsize',13)
ylim([0 inf])
