function status=plot_ROMS_monthly_horizontal_t_uv(testname, outfile, filedir, section, tempyear, inputmonth, shadlev, conlev, var,  param_script)
% clear all;close all;
%==========================================================================
% % % vert_param;
        ...
        for j=1:length(cut_data(1,1,:))
            if (cut_mask_rho(i,j)==0)
                ...
                if (cut_depth(1,i,j)>refdepth)
                    ...
                    if (z_ind==N)
%                         cut_depth2(1:3)=cut_depth(z_ind-2:z_ind,i,j);  %% if closest cell is first vertical cell, get 3 cells from first to third cell 
...
%                         cut_v2(1:3)=cut_v(z_ind-2:z_ind,i,j);
                    ...
                    elseif (z_ind==1)
                        cut_depth2(1:3)=cut_depth(z_ind:z_ind+2,i,j);  %% if closest cell is bottom vertical cell, get 3 cells from bottom-2 to bottom cell 
...
%     uvplot=m_quiver(reflon, ...
%                         reflat, ...
%                         refu * m_quiver_vector_size, ...
%                         refv * m_quiver_vector_size, ...
%                         'color',m_quiver_vector_color, 'AutoScale','off','LineWidth', m_quiver_LineWidth);                    
