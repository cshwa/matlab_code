...
% temp_lim = [6 25];
plot_pcolorjw = 1;    temp_lim = [-8 8];    salt_lim = [-2 2]; den_lim = [-2 0]; no3_lim=[-3 3]; nh4_lim=[-.5 .5];
                      chl_lim=[-1.2 1.2];   phy_lim=[-3 3];    zoo_lim=[-1 1];  oxy_lim=[-30 30];  v_lim=([-(10^-3)*6 (10^-3)*6]);
                      tic_lim=[-70 200];   po4_lim=[-1 1]; tdet_lim = [-1 1]; ldet_lim =[-1 1]; sdet_lim =[-1 1];
                      DIN_up_lim=[0 4];    z_up_lim=[-1 1]; z_lim=[-1 1];
% temp_c  =[6:1:25];
plot_contour  = 1;    color_c  ='-k' ;      temp_c  =[-35:.5:35]; salt_c  =[-35:.1:35]; no3_c=[-20:.2:20]; nh4_c=[-30:.1:30];
                      ...
                      DIN_up_c=[-10:.2:10]; z_c = [-10:.05:10]; z_up_c = [-10:.05:10];
...
%               % bottom // 16 = Ldet // 17 = Sdet // 18 = total = Ldet +
%               Sdet // 19 = zeta
...
%% 2&3regime
case_name_ref=['ncra_',num2str(yy_ref),'reg_cir_2yr'];
...
%                 value = value1 + value2;
                val_name='velocity';
                unit ='m/s';
                out_name_1=['vertical-GY-velocity-',num2str(yy),'-',num2str(yy+9),'-'];
                val_caxis=v_lim;
                level_c=v_c;
                u=u'; v=v';
...
%                 red_range = [5.0:0.2:5.2];
                ...
                val_name='Ldet';
                unit ='μMol N L^-1';
                out_name_1=['vertical-GY-Ldet-',num2str(yy),'-'];
                val_caxis=ldet_lim;
                level_c=ldet_c;
%                 value=value;
%                clear value;
...
%                 red_range = [5.0:0.2:5.2];
                ...
                val_name='Sdet';
                unit ='μMol N L^-1';
                out_name_1=['vertical-GY-Sdet-',num2str(yy),'-'];
                val_caxis=sdet_lim;
                level_c=sdet_c;
%                 value=value;
%                clear value;
...
%                 red_range = [5.0:0.2:5.2];
                ...
                val_name='total-det';
                unit ='μMol N L^-1';
                out_name_1=['vertical-GY-total-det-',num2str(yy),'-'];
                val_caxis=tdet_lim;
                level_c=tdet_c;
%                 value=value;
%                clear value;
...
%                 white_range = [5.2:0.2:5.4];
%                 red_range = [5.0:0.2:5.2];
                value=nc{char('zeta')}(:); %z
                value_ref=nc_ref{char('zeta')}(:); %z               
                val_name='zeta';
                unit ='m';
                out_name_1=['vertical-GY-zeta-',num2str(yy),'-'];
                val_caxis=z_lim;
                level_c=z_c;              
                val_up_caxis=z_up_lim;
                level_up_c=z_c;
                
                value = permute(value,[2 1]);
                value_ref = permute(value_ref,[2 1]);
%                 value=value;
%                clear value;
        end
...
%                 saveas(gca,[out_name,'.png']);
                print(gcf,['figures\diff\',out_name,'.png'],'-dpng','-r200');
%                 close all;
            end
elseif section == 2
    xLim = lonLim;
...
%                 saveas(gca,[out_name,'_meridional','.png']);
                print(gcf,['figures\diff\',out_name,'_meridional','.png'],'-dpng','-r200');
%                 close all;
            end   
...
%                 saveas(gcf,out_name,out_type);
%                 saveas(gca,[out_name,'_meridional','.png']);
                print(gcf,['figures\diff\',out_name,'_vertical_integral','.png'],'-dpng','-r200');
                close all;
            end
...
%                         end
                ...
                %                 saveas(gcf,out_name,out_type);
                %                 saveas(gca,[out_name,'_meridional','.png']);
                                print(gcf,['figures\diff\',out_name,'_surf','.png'],'-dpng','-r200');
                                close all;
                            end 
...
%                         end
                ...
                %                 saveas(gcf,out_name,out_type);
                %                 saveas(gca,[out_name,'_meridional','.png']);
                                print(gcf,['figures\diff\',out_name,'_bot','.png'],'-dpng','-r200');
                                close all;
                            end 
    case {19} % zeta
        int_diff = value - value_ref;
        figure('Position',figPos*get(groot,'ScreenPixelsPerInch'),'paperunits','inches','paperposition',figPos)
%         set(gcf,'PaperPositionMode','auto')     
        pcolor(lon_rho,lat_rho,int_diff); shading flat;
%         xlim(xLim);
...
%              colormap(jet);
        ...
        set(gcf,'InvertHardCopy','Off'); %remove default "white background"
            ...
            if (switch_save)
                print(gcf,['figures\diff\',out_name,'_vertical_integral_upper_river','.png'],'-dpng','-r200');
                close all;
            end
...
%                         end
                            ...
                            if (switch_save)
                                print(gcf,['figures\diff\',out_name,'_surf_upper_river','.png'],'-dpng','-r200');
                                close all;
                            end 
...
%                         end
                ...
                %                 saveas(gcf,out_name,out_type);
                %                 saveas(gca,[out_name,'_meridional','.png']);
                                print(gcf,['figures\diff\',out_name,'_bot_upper_river','.png'],'-dpng','-r200');
                                close all;
                            end 
...
%         out_name_1=[out_name_1];
        ...
        set(gcf,'InvertHardCopy','Off'); %remove default "white background"
            ...
            if (switch_save)
                print(gcf,['figures\diff\',out_name,'_upper_river','.png'],'-dpng','-r200');
                close all;
            end
