...
%% pick only in the bay station ('±¤¾ç1','±¤¾ç2','±¤¾ç3') except ±¤¾çÇ×
do_sur_clim(5:9,:) = NaN;
no3_sur_clim(5:9,:) = NaN;
temp_sur_clim(5:9,:) = NaN;
salt_sur_clim(5:9,:) = NaN;
chl_sur_clim(5:9,:) = NaN;
nh4_sur_clim(5:9,:) = NaN;
        
po4_sur_clim(5:9,:) = NaN;
po4_bot_clim(5:9,:) = NaN;
        
do_bot_clim(5:9,:) = NaN;
no3_bot_clim(5:9,:) = NaN;
temp_bot_clim(5:9,:) = NaN;
salt_bot_clim(5:9,:) = NaN;
chl_bot_clim(5:9,:) = NaN;
nh4_bot_clim(5:9,:) = NaN;
...
% for i = 1:length(name_tag)
    ...
    mtx_regime_do_3s = reshape(regime_do,size(do_sur_clim,1),size(do_sur_clim,2));
    ...
    mtx_regime_do_2s = reshape(regime_do,size(do_sur_clim,1),size(do_sur_clim,2));
    ...
    mtx_regime_do_1s = reshape(regime_do,size(do_sur_clim,1),size(do_sur_clim,2));
    
    regm_do_3s = nanmean(regime_do)
    
    figure; hold on;
    for i = 1:sp_gy
        plot(do_sur_clim(i,:),'k.');
    end
    color_p={'r','g','b'};
    for i = 1:3
   ...
   clearvars idx1 tempo_data
    ...
    mtx_regime_no3_3s = reshape(regime_no3,size(no3_sur_clim,1),size(no3_sur_clim,2));
    ...
    mtx_regime_no3_2s = reshape(regime_no3,size(no3_sur_clim,1),size(no3_sur_clim,2));
    ...
    mtx_regime_no3_1s = reshape(regime_no3,size(no3_sur_clim,1),size(no3_sur_clim,2));
    
    figure; hold on;
    for i = 1:sp_gy
        plot(no3_sur_clim(i,:)./N_MW,'k.');
    end
    color_p={'r','g','b'};
    for i = 1:3
    yline(no3_up_bound(i)./N_MW,color_p{i}); yline(no3_down_bound(i)./N_MW,color_p{i});
    end
   ylim([0 inf])
    ...
    mtx_regime_temp_3s = reshape(regime_temp,size(temp_sur_clim,1),size(temp_sur_clim,2));
    ...
    mtx_regime_temp_2s = reshape(regime_temp,size(temp_sur_clim,1),size(temp_sur_clim,2));
    ...
    mtx_regime_temp_1s = reshape(regime_temp,size(temp_sur_clim,1),size(temp_sur_clim,2));
    
    
     figure; hold on;
    for i = 1:sp_gy
        plot(temp_sur_clim(i,:),'k.');
    end
    color_p={'r','g','b'};
    for i = 1:3
   ...
   ylim([0 inf])
    ...
    mtx_regime_salt_3s = reshape(regime_salt,size(salt_sur_clim,1),size(salt_sur_clim,2));
    ...
    mtx_regime_salt_2s = reshape(regime_salt,size(salt_sur_clim,1),size(salt_sur_clim,2));
    ...
    mtx_regime_salt_1s = reshape(regime_salt,size(salt_sur_clim,1),size(salt_sur_clim,2));
    
    
    figure; hold on;
    for i = 1:sp_gy
        plot(salt_sur_clim(i,:),'k.');
    end
    color_p={'r','g','b'};
    for i = 1:3
   ...
   ylim([0 inf])
    ...
    mtx_regime_po4_3s = reshape(regime_po4,size(po4_sur_clim,1),size(po4_sur_clim,2));
    ...
    mtx_regime_po4_2s = reshape(regime_po4,size(po4_sur_clim,1),size(po4_sur_clim,2));
    ...
    mtx_regime_po4_1s = reshape(regime_po4,size(po4_sur_clim,1),size(po4_sur_clim,2));
   ...
   ylim([0 inf])
    ...
    mtx_regime_nh4_3s = reshape(regime_nh4,size(nh4_sur_clim,1),size(nh4_sur_clim,2));
    ...
    mtx_regime_nh4_2s = reshape(regime_nh4,size(nh4_sur_clim,1),size(nh4_sur_clim,2));
    ...
    mtx_regime_nh4_1s = reshape(regime_nh4,size(nh4_sur_clim,1),size(nh4_sur_clim,2));
    
    
    figure; hold on;
    for i = 1:sp_gy
        plot(nh4_sur_clim(i,:)./N_MW,'k.');
    end
    color_p={'r','g','b'};
    for i = 1:3
    yline(nh4_up_bound(i)./N_MW,color_p{i}); yline(nh4_down_bound(i)./N_MW,color_p{i});
    end
   ylim([0 inf])
    ...
    mtx_regime_chl_3s = reshape(regime_chl,size(chl_sur_clim,1),size(chl_sur_clim,2));
    ...
    mtx_regime_chl_2s = reshape(regime_chl,size(chl_sur_clim,1),size(chl_sur_clim,2));
    ...
    mtx_regime_chl_1s = reshape(regime_chl,size(chl_sur_clim,1),size(chl_sur_clim,2));
    
    
 figure; hold on;
    for i = 1:sp_gy
        plot(chl_sur_clim(i,:),'k.');
    end
    color_p={'r','g','b'};
    for i = 1:3
   ...
   ylim([0 inf])
    ...
    mtx_regime_do_b_3s = reshape(regime_do_b,size(do_bot_clim,1),size(do_bot_clim,2));
    ...
    mtx_regime_do_b_2s = reshape(regime_do_b,size(do_bot_clim,1),size(do_bot_clim,2));
    ...
    mtx_regime_do_b_1s = reshape(regime_do_b,size(do_bot_clim,1),size(do_bot_clim,2));
        
    figure; hold on;
    for i = 1:sp_gy
        plot(do_bot_clim(i,:),'k.');
    end
    color_p={'r','g','b'};
    for i = 1:3
   ...
   clearvars idx1 tempo_data
    ...
    mtx_regime_no3_b_3s = reshape(regime_no3_b,size(no3_bot_clim,1),size(no3_bot_clim,2));
    ...
    mtx_regime_no3_b_2s = reshape(regime_no3_b,size(no3_bot_clim,1),size(no3_bot_clim,2));
    ...
    mtx_regime_no3_b_1s = reshape(regime_no3_b,size(no3_bot_clim,1),size(no3_bot_clim,2));
    

    figure; hold on;
    for i = 1:sp_gy
        plot(no3_bot_clim(i,:)./N_MW,'k.');
    end
    color_p={'r','g','b'};
    for i = 1:3
    yline(no3_b_up_bound(i)./N_MW,color_p{i}); yline(no3_b_down_bound(i)./N_MW,color_p{i});
    end
   ylim([0 inf])
    ...
    mtx_regime_temp_b_3s = reshape(regime_temp_b,size(temp_bot_clim,1),size(temp_bot_clim,2));
    ...
    mtx_regime_temp_b_2s = reshape(regime_temp_b,size(temp_bot_clim,1),size(temp_bot_clim,2));
    ...
    mtx_regime_temp_b_1s = reshape(regime_temp_b,size(temp_bot_clim,1),size(temp_bot_clim,2));
    
    figure; hold on;
    for i = 1:sp_gy
        plot(temp_bot_clim(i,:),'k.');
    end
    color_p={'r','g','b'};
    for i = 1:3
   ...
   ylim([0 inf])
    ...
    mtx_regime_salt_b_3s = reshape(regime_salt_b,size(salt_bot_clim,1),size(salt_bot_clim,2));
    ...
    mtx_regime_salt_b_2s = reshape(regime_salt_b,size(salt_bot_clim,1),size(salt_bot_clim,2));
    ...
    mtx_regime_salt_b_1s = reshape(regime_salt_b,size(salt_bot_clim,1),size(salt_bot_clim,2));
    
    
     figure; hold on;
    for i = 1:sp_gy
        plot(salt_bot_clim(i,:),'k.');
    end
    color_p={'r','g','b'};
    for i = 1:3
   ...
   ylim([0 inf])
    ...
    mtx_regime_po4_b_2s = reshape(regime_po4_b,size(po4_bot_clim,1),size(po4_bot_clim,2));
    ...
    mtx_regime_po4_b_1s = reshape(regime_po4_b,size(po4_bot_clim,1),size(po4_bot_clim,2));
    
    
      figure; hold on;
    for i = 1:sp_gy
        plot(po4_bot_clim(i,:)./P_MW,'k.');
    end
    color_p={'r','g','b'};
    for i = 1:3
    yline(po4_b_up_bound(i)./P_MW,color_p{i}); yline(po4_b_down_bound(i)./P_MW,color_p{i});
    end
   ylim([0 inf])
    ...
    mtx_regime_nh4_b_3s = reshape(regime_nh4_b,size(nh4_bot_clim,1),size(nh4_bot_clim,2));
    ...
    mtx_regime_nh4_b_2s = reshape(regime_nh4_b,size(nh4_bot_clim,1),size(nh4_bot_clim,2));
    ...
    mtx_regime_nh4_b_1s = reshape(regime_nh4_b,size(nh4_bot_clim,1),size(nh4_bot_clim,2));
    
    
        figure; hold on;
    for i = 1:sp_gy
        plot(nh4_bot_clim(i,:)./N_MW,'k.');
    end
    color_p={'r','g','b'};
    for i = 1:3
    yline(nh4_b_up_bound(i)./N_MW,color_p{i}); yline(nh4_b_down_bound(i)./N_MW,color_p{i});
    end
   ylim([0 inf])
    ...
    mtx_regime_chl_b_3s = reshape(regime_chl_b,size(chl_bot_clim,1),size(chl_bot_clim,2));
    ...
    mtx_regime_chl_b_2s = reshape(regime_chl_b,size(chl_bot_clim,1),size(chl_bot_clim,2));
    ...
    mtx_regime_chl_b_1s = reshape(regime_chl_b,size(chl_bot_clim,1),size(chl_bot_clim,2));
    
   
      figure; hold on;
    for i = 1:sp_gy
        plot(chl_bot_clim(i,:),'k.');
    end
    color_p={'r','g','b'};
    for i = 1:3
...
save(['koem_timeseires_monthly_gy_only_1to3sig_v2_in_bay.mat']);
return
