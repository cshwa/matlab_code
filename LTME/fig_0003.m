% %  NIFS climatology geostrophic velocity W-E section in Feb and in Aug at 102 line
...
sbp1=subplot(2,1,1);
pcolor(lon3,-stddepth(1:10)/20,clima_gvel2(1:10,1:lonmaxind,1));
    shading interp;
...
caxis([-0.5,0.5]);
hold on;
[C,h]=contour(lon3,-stddepth(1:10)/20,clima_gvel2(1:10,1:lonmaxind,1),level_c,'k','linewidth',contwidth);
...
sbp1=subplot(2,1,2);
pcolor(lon3,-stddepth(1:10)/20,clima_gvel2(1:10,1:lonmaxind,4));
    shading interp;
    xlim([min(lon2(lonminind:3)) max(lon2(lonminind:lonmaxind))]);
    set(gca,'yTickLabel',{'-160m', '-120m', '-80m', '-40m', '0m'}, ...
        'YTick', [-8, -6, -4, -2, 0])
    set(gca,'xTickLabel',{'129.8^oE','130.0^oE','130.2^oE','130.4^oE','130.6^oE','130.8^oE'}, ...
        'XTick', (129.8:0.2:130.8))
...
caxis([-0.5,0.5]);
hold on;
[C,h]=contour(lon3,-stddepth(1:10)/20,clima_gvel2(1:10,1:lonmaxind,4),level_c,'k','linewidth',contwidth);
clabel(C,h,'FontSize',confontsize,'Color','k','labelspacing',100000,'Rotation',90,'fontweight','bold');
