function create_roms_forcing_Qair(fname,Qair,time,yy,cycle)%,tframe,cycle)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
nw{var_time}(:)=time';
 ...
 nw{'Qair'} = ncfloat(var_time, 'eta_rho', 'xi_rho');
 nw{'Qair'}.long_name = ncchar('ECMWF - relative humidity');
 nw{'Qair'}.units = ncchar('Percentage');
 nw{'Qair'}.time = ncchar(var_time);
for i=1:1:yy
 nw{'Qair'}(i,:,:)= Qair(i,:,:);
end
 close(nw);
