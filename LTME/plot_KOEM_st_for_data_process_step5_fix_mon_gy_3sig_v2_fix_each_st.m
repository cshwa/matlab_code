...
yoonja=load('yoonjakangs_koem_data_monthly_v2_16points.mat'); % yoonjakangs_koem_data_processing_v2_16points.m
...
for i = 1:sp_gy
    ...
    tempo_data= reshape(secchi_sur_clim(i,:),1,size(secchi_sur_clim(i,:),1)*size(secchi_sur_clim(i,:),2)); 
    idx1 = find(isnan(tempo_data) == 0);
    regime_secchi=tempo_data;
    for sig_ind = 1:3
        secchi_up_bound(sig_ind)= mean(tempo_data(idx1)) + sig_ind*std(tempo_data(idx1));
        secchi_secchiwn_bound(sig_ind)= mean(tempo_data(idx1)) - sig_ind*std(tempo_data(idx1));
    end
    regime_secchi(find(tempo_data > secchi_up_bound(3)))=NaN;
    regime_secchi(find(tempo_data < secchi_secchiwn_bound(3)))=NaN;
    mtx_regime_secchi_3s(i,:) = reshape(regime_secchi,size(secchi_sur_clim(i,:),1),size(secchi_sur_clim(i,:),2));
    
    clearvars tempo_data regime_*
    tempo_data= reshape(secchi_sur_clim(i,:),1,size(secchi_sur_clim(i,:),1)*size(secchi_sur_clim(i,:),2)); 
    regime_secchi=tempo_data;
    regime_secchi(find(tempo_data > secchi_up_bound(2)))=NaN;
    regime_secchi(find(tempo_data < secchi_secchiwn_bound(2)))=NaN;
    mtx_regime_secchi_2s(i,:) = reshape(regime_secchi,size(secchi_sur_clim(i,:),1),size(secchi_sur_clim(i,:),2));
    
    clearvars tempo_data regime_*
    tempo_data= reshape(secchi_sur_clim(i,:),1,size(secchi_sur_clim(i,:),1)*size(secchi_sur_clim(i,:),2)); 
    regime_secchi=tempo_data;
    regime_secchi(find(tempo_data > secchi_up_bound(1)))=NaN;
    regime_secchi(find(tempo_data < secchi_secchiwn_bound(1)))=NaN;
    mtx_regime_secchi_1s(i,:) = reshape(regime_secchi,size(secchi_sur_clim(i,:),1),size(secchi_sur_clim(i,:),2));
    
     clearvars idx1 tempo_data
    tempo_data= reshape(si_sur_clim(i,:),1,size(si_sur_clim(i,:),1)*size(si_sur_clim(i,:),2)); 
    idx1 = find(isnan(tempo_data) == 0);
    regime_si=tempo_data;
    for sig_ind = 1:3
        si_up_bound(sig_ind)= mean(tempo_data(idx1)) + sig_ind*std(tempo_data(idx1));
        si_siwn_bound(sig_ind)= mean(tempo_data(idx1)) - sig_ind*std(tempo_data(idx1));
    end
    regime_si(find(tempo_data > si_up_bound(3)))=NaN;
    regime_si(find(tempo_data < si_siwn_bound(3)))=NaN;
    mtx_regime_si_3s(i,:) = reshape(regime_si,size(si_sur_clim(i,:),1),size(si_sur_clim(i,:),2));
    
    clearvars tempo_data regime_*
    tempo_data= reshape(si_sur_clim(i,:),1,size(si_sur_clim(i,:),1)*size(si_sur_clim(i,:),2)); 
    regime_si=tempo_data;
    regime_si(find(tempo_data > si_up_bound(2)))=NaN;
    regime_si(find(tempo_data < si_siwn_bound(2)))=NaN;
    mtx_regime_si_2s(i,:) = reshape(regime_si,size(si_sur_clim(i,:),1),size(si_sur_clim(i,:),2));
    
    clearvars tempo_data regime_*
    tempo_data= reshape(si_sur_clim(i,:),1,size(si_sur_clim(i,:),1)*size(si_sur_clim(i,:),2)); 
    regime_si=tempo_data;
    regime_si(find(tempo_data > si_up_bound(1)))=NaN;
    regime_si(find(tempo_data < si_siwn_bound(1)))=NaN;
    mtx_regime_si_1s(i,:) = reshape(regime_si,size(si_sur_clim(i,:),1),size(si_sur_clim(i,:),2));
    
    clearvars idx1 tempo_data
    tempo_data= reshape(ss_sur_clim(i,:),1,size(ss_sur_clim(i,:),1)*size(ss_sur_clim(i,:),2)); 
    idx1 = find(isnan(tempo_data) == 0);
    regime_ss=tempo_data;
    for sig_ind = 1:3
        ss_up_bound(sig_ind)= mean(tempo_data(idx1)) + sig_ind*std(tempo_data(idx1));
        ss_sswn_bound(sig_ind)= mean(tempo_data(idx1)) - sig_ind*std(tempo_data(idx1));
    end
    regime_ss(find(tempo_data > ss_up_bound(3)))=NaN;
    regime_ss(find(tempo_data < ss_sswn_bound(3)))=NaN;
    mtx_regime_ss_3s(i,:) = reshape(regime_ss,size(ss_sur_clim(i,:),1),size(ss_sur_clim(i,:),2));
    
    clearvars tempo_data regime_*
    tempo_data= reshape(ss_sur_clim(i,:),1,size(ss_sur_clim(i,:),1)*size(ss_sur_clim(i,:),2)); 
    regime_ss=tempo_data;
    regime_ss(find(tempo_data > ss_up_bound(2)))=NaN;
    regime_ss(find(tempo_data < ss_sswn_bound(2)))=NaN;
    mtx_regime_ss_2s(i,:) = reshape(regime_ss,size(ss_sur_clim(i,:),1),size(ss_sur_clim(i,:),2));
    
    clearvars tempo_data regime_*
    tempo_data= reshape(ss_sur_clim(i,:),1,size(ss_sur_clim(i,:),1)*size(ss_sur_clim(i,:),2)); 
    regime_ss=tempo_data;
    regime_ss(find(tempo_data > ss_up_bound(1)))=NaN;
    regime_ss(find(tempo_data < ss_sswn_bound(1)))=NaN;
    mtx_regime_ss_1s(i,:) = reshape(regime_ss,size(ss_sur_clim(i,:),1),size(ss_sur_clim(i,:),2));
   ...
   clearvars idx1 tempo_data
    ...
    mtx_regime_no3_1s(i,:) = reshape(regime_no3,size(no3_sur_clim(i,:),1),size(no3_sur_clim(i,:),2));
       ...
       clearvars idx1 tempo_data
    tempo_data= reshape(din_sur_clim(i,:),1,size(din_sur_clim(i,:),1)*size(din_sur_clim(i,:),2)); 
    idx1 = find(isnan(tempo_data) == 0);
    regime_din=tempo_data;
    for sig_ind = 1:3
        din_up_bound(sig_ind)= mean(tempo_data(idx1)) + sig_ind*std(tempo_data(idx1));
        din_down_bound(sig_ind)= mean(tempo_data(idx1)) - sig_ind*std(tempo_data(idx1));
    end
    regime_din(find(tempo_data > din_up_bound(3)))=NaN;
    regime_din(find(tempo_data < din_down_bound(3)))=NaN;
    mtx_regime_din_3s(i,:) = reshape(regime_din,size(din_sur_clim(i,:),1),size(din_sur_clim(i,:),2));
    
    clearvars tempo_data regime_*
    tempo_data= reshape(din_sur_clim(i,:),1,size(din_sur_clim(i,:),1)*size(din_sur_clim(i,:),2));
    regime_din=tempo_data;
    regime_din(find(tempo_data > din_up_bound(2)))=NaN;
    regime_din(find(tempo_data < din_down_bound(2)))=NaN;
    mtx_regime_din_2s(i,:) = reshape(regime_din,size(din_sur_clim(i,:),1),size(din_sur_clim(i,:),2));
    
    clearvars tempo_data regime_*
    tempo_data= reshape(din_sur_clim(i,:),1,size(din_sur_clim(i,:),1)*size(din_sur_clim(i,:),2));
    regime_din=tempo_data;
    regime_din(find(tempo_data > din_up_bound(1)))=NaN;
    regime_din(find(tempo_data < din_down_bound(1)))=NaN;
    mtx_regime_din_1s(i,:) = reshape(regime_din,size(din_sur_clim(i,:),1),size(din_sur_clim(i,:),2));
...
%    ylim([0 inf])
   ...
   clearvars idx1 tempo_data
    ...
    mtx_regime_chl_1s(i,:) = reshape(regime_chl,size(chl_sur_clim(i,:),1),size(chl_sur_clim(i,:),2));
     ...
     %% bot
    clearvars idx1 tempo_data
    tempo_data= reshape(si_bot_clim(i,:),1,size(si_bot_clim(i,:),1)*size(si_bot_clim(i,:),2)); 
    idx1 = find(isnan(tempo_data) == 0);
    regime_si_b=tempo_data;
    for sig_ind = 1:3
        si_b_up_bound(sig_ind)= mean(tempo_data(idx1)) + sig_ind*std(tempo_data(idx1));
        si_b_siwn_bound(sig_ind)= mean(tempo_data(idx1)) - sig_ind*std(tempo_data(idx1));
    end
    regime_si_b(find(tempo_data > si_b_up_bound(3)))=NaN;
    regime_si_b(find(tempo_data < si_b_siwn_bound(3)))=NaN;
    mtx_regime_si_b_3s(i,:) = reshape(regime_si_b,size(si_bot_clim(i,:),1),size(si_bot_clim(i,:),2));
    
    clearvars tempo_data regime_*
    tempo_data= reshape(si_bot_clim(i,:),1,size(si_bot_clim(i,:),1)*size(si_bot_clim(i,:),2));
    regime_si_b=tempo_data;
    regime_si_b(find(tempo_data > si_b_up_bound(2)))=NaN;
    regime_si_b(find(tempo_data < si_b_siwn_bound(2)))=NaN;
    mtx_regime_si_b_2s(i,:) = reshape(regime_si_b,size(si_bot_clim(i,:),1),size(si_bot_clim(i,:),2));
    
    clearvars tempo_data regime_*
    tempo_data= reshape(si_bot_clim(i,:),1,size(si_bot_clim(i,:),1)*size(si_bot_clim(i,:),2));
    regime_si_b=tempo_data;
    regime_si_b(find(tempo_data > si_b_up_bound(1)))=NaN;
    regime_si_b(find(tempo_data < si_b_siwn_bound(1)))=NaN;
    mtx_regime_si_b_1s(i,:) = reshape(regime_si_b,size(si_bot_clim(i,:),1),size(si_bot_clim(i,:),2));
    
    clearvars idx1 tempo_data
    tempo_data= reshape(ss_bot_clim(i,:),1,size(ss_bot_clim(i,:),1)*size(ss_bot_clim(i,:),2)); 
    idx1 = find(isnan(tempo_data) == 0);
    regime_ss_b=tempo_data;
    for sig_ind = 1:3
        ss_b_up_bound(sig_ind)= mean(tempo_data(idx1)) + sig_ind*std(tempo_data(idx1));
        ss_b_sswn_bound(sig_ind)= mean(tempo_data(idx1)) - sig_ind*std(tempo_data(idx1));
    end
    regime_ss_b(find(tempo_data > ss_b_up_bound(3)))=NaN;
    regime_ss_b(find(tempo_data < ss_b_sswn_bound(3)))=NaN;
    mtx_regime_ss_b_3s(i,:) = reshape(regime_ss_b,size(ss_bot_clim(i,:),1),size(ss_bot_clim(i,:),2));
    
    clearvars tempo_data regime_*
    tempo_data= reshape(ss_bot_clim(i,:),1,size(ss_bot_clim(i,:),1)*size(ss_bot_clim(i,:),2));
    regime_ss_b=tempo_data;
    regime_ss_b(find(tempo_data > ss_b_up_bound(2)))=NaN;
    regime_ss_b(find(tempo_data < ss_b_sswn_bound(2)))=NaN;
    mtx_regime_ss_b_2s(i,:) = reshape(regime_ss_b,size(ss_bot_clim(i,:),1),size(ss_bot_clim(i,:),2));
    
    clearvars tempo_data regime_*
    tempo_data= reshape(ss_bot_clim(i,:),1,size(ss_bot_clim(i,:),1)*size(ss_bot_clim(i,:),2));
    regime_ss_b=tempo_data;
    regime_ss_b(find(tempo_data > ss_b_up_bound(1)))=NaN;
    regime_ss_b(find(tempo_data < ss_b_sswn_bound(1)))=NaN;
    mtx_regime_ss_b_1s(i,:) = reshape(regime_ss_b,size(ss_bot_clim(i,:),1),size(ss_bot_clim(i,:),2));
...
clearvars idx1 tempo_data
    tempo_data= reshape(din_bot_clim(i,:),1,size(din_bot_clim(i,:),1)*size(din_bot_clim(i,:),2)); 
    idx1 = find(isnan(tempo_data) == 0);
    regime_din_b=tempo_data;
    for sig_ind = 1:3
        din_b_up_bound(sig_ind)= mean(tempo_data(idx1)) + sig_ind*std(tempo_data(idx1));
        din_b_down_bound(sig_ind)= mean(tempo_data(idx1)) - sig_ind*std(tempo_data(idx1));
    end
    regime_din_b(find(tempo_data > din_b_up_bound(3)))=NaN;
    regime_din_b(find(tempo_data < din_b_down_bound(3)))=NaN;
    mtx_regime_din_b_3s(i,:) = reshape(regime_din_b,size(din_bot_clim(i,:),1),size(din_bot_clim(i,:),2));
    
    clearvars tempo_data regime_*
    tempo_data= reshape(din_bot_clim(i,:),1,size(din_bot_clim(i,:),1)*size(din_bot_clim(i,:),2));
    regime_din_b=tempo_data;
    regime_din_b(find(tempo_data > din_b_up_bound(2)))=NaN;
    regime_din_b(find(tempo_data < din_b_down_bound(2)))=NaN;
    mtx_regime_din_b_2s(i,:) = reshape(regime_din_b,size(din_bot_clim(i,:),1),size(din_bot_clim(i,:),2));
    
    clearvars tempo_data regime_*
    tempo_data= reshape(din_bot_clim(i,:),1,size(din_bot_clim(i,:),1)*size(din_bot_clim(i,:),2));
    regime_din_b=tempo_data;
    regime_din_b(find(tempo_data > din_b_up_bound(1)))=NaN;
    regime_din_b(find(tempo_data < din_b_down_bound(1)))=NaN;
    mtx_regime_din_b_1s(i,:) = reshape(regime_din_b,size(din_bot_clim(i,:),1),size(din_bot_clim(i,:),2));
...
regime_din_1s = mtx_regime_din_1s;
...
regime_secchi_1s = mtx_regime_secchi_1s;
regime_ss_1s = mtx_regime_ss_1s;
regime_si_1s = mtx_regime_si_1s;

regime_do_b_1s = mtx_regime_do_b_1s;
regime_no3_b_1s = mtx_regime_no3_b_1s;
regime_din_b_1s = mtx_regime_din_b_1s;
...
regime_ss_b_1s = mtx_regime_ss_b_1s;
regime_si_b_1s = mtx_regime_si_b_1s;
...
regime_din_2s = mtx_regime_din_2s;
...
regime_secchi_2s = mtx_regime_secchi_2s;
regime_ss_2s = mtx_regime_ss_2s;
regime_si_2s = mtx_regime_si_2s;

regime_do_b_2s = mtx_regime_do_b_2s;
regime_no3_b_2s = mtx_regime_no3_b_2s;
regime_din_b_2s = mtx_regime_din_b_2s;
...
regime_ss_b_2s = mtx_regime_ss_b_2s;
regime_si_b_2s = mtx_regime_si_b_2s;
...
regime_din_3s = mtx_regime_din_3s;
...
regime_secchi_3s = mtx_regime_secchi_3s;
regime_ss_3s = mtx_regime_ss_3s;
regime_si_3s = mtx_regime_si_3s;

regime_do_b_3s = mtx_regime_do_b_3s;
regime_no3_b_3s = mtx_regime_no3_b_3s;
regime_din_b_3s = mtx_regime_din_b_3s;
...
regime_ss_b_3s = mtx_regime_ss_b_3s;
regime_si_b_3s = mtx_regime_si_b_3s;
...
save(['koem_timeseires_monthly_gy_only_1to3sig_v2_each_st_16p.mat']); % 16 points 


clearvars regime_*
regime_do_1s = mtx_regime_do_1s(2:end,:);
regime_no3_1s = mtx_regime_no3_1s(2:end,:);
regime_temp_1s = mtx_regime_temp_1s(2:end,:);
regime_salt_1s = mtx_regime_salt_1s(2:end,:);
regime_nh4_1s = mtx_regime_nh4_1s(2:end,:);
regime_chl_1s = mtx_regime_chl_1s(2:end,:);
regime_secchi_1s = mtx_regime_secchi_1s(2:end,:);
regime_ss_1s = mtx_regime_ss_1s(2:end,:);
regime_si_1s = mtx_regime_si_1s(2:end,:);

regime_do_b_1s = mtx_regime_do_b_1s(2:end,:);
regime_no3_b_1s = mtx_regime_no3_b_1s(2:end,:);
regime_temp_b_1s = mtx_regime_temp_b_1s(2:end,:);
regime_salt_b_1s = mtx_regime_salt_b_1s(2:end,:);
regime_nh4_b_1s = mtx_regime_nh4_b_1s(2:end,:);
regime_chl_b_1s = mtx_regime_chl_b_1s(2:end,:);
regime_ss_b_1s = mtx_regime_ss_b_1s(2:end,:);
regime_si_b_1s = mtx_regime_si_b_1s(2:end,:);

regime_po4_1s = mtx_regime_po4_1s(2:end,:);
regime_po4_b_1s = mtx_regime_po4_b_1s(2:end,:);

regime_do_2s = mtx_regime_do_2s(2:end,:);
regime_no3_2s = mtx_regime_no3_2s(2:end,:);
regime_temp_2s = mtx_regime_temp_2s(2:end,:);
regime_salt_2s = mtx_regime_salt_2s(2:end,:);
regime_nh4_2s = mtx_regime_nh4_2s(2:end,:);
regime_chl_2s = mtx_regime_chl_2s(2:end,:);
regime_secchi_2s = mtx_regime_secchi_2s(2:end,:);
regime_ss_2s = mtx_regime_ss_2s(2:end,:);
regime_si_2s = mtx_regime_si_2s(2:end,:);

regime_do_b_2s = mtx_regime_do_b_2s(2:end,:);
regime_no3_b_2s = mtx_regime_no3_b_2s(2:end,:);
regime_temp_b_2s = mtx_regime_temp_b_2s(2:end,:);
regime_salt_b_2s = mtx_regime_salt_b_2s(2:end,:);
regime_nh4_b_2s = mtx_regime_nh4_b_2s(2:end,:);
regime_chl_b_2s = mtx_regime_chl_b_2s(2:end,:);
regime_ss_b_2s = mtx_regime_ss_b_2s(2:end,:);
regime_si_b_2s = mtx_regime_si_b_2s(2:end,:);

regime_po4_2s = mtx_regime_po4_2s(2:end,:);
regime_po4_b_2s = mtx_regime_po4_b_2s(2:end,:);

regime_do_3s = mtx_regime_do_3s(2:end,:);
regime_no3_3s = mtx_regime_no3_3s(2:end,:);
regime_temp_3s = mtx_regime_temp_3s(2:end,:);
regime_salt_3s = mtx_regime_salt_3s(2:end,:);
regime_nh4_3s = mtx_regime_nh4_3s(2:end,:);
regime_chl_3s = mtx_regime_chl_3s(2:end,:);
regime_secchi_3s = mtx_regime_secchi_3s(2:end,:);
regime_ss_3s = mtx_regime_ss_3s(2:end,:);
regime_si_3s = mtx_regime_si_3s(2:end,:);

regime_do_b_3s = mtx_regime_do_b_3s(2:end,:);
regime_no3_b_3s = mtx_regime_no3_b_3s(2:end,:);
regime_temp_b_3s = mtx_regime_temp_b_3s(2:end,:);
regime_salt_b_3s = mtx_regime_salt_b_3s(2:end,:);
regime_nh4_b_3s = mtx_regime_nh4_b_3s(2:end,:);
regime_chl_b_3s = mtx_regime_chl_b_3s(2:end,:);
regime_ss_b_3s = mtx_regime_ss_b_3s(2:end,:);
regime_si_b_3s = mtx_regime_si_b_3s(2:end,:);

regime_po4_3s = mtx_regime_po4_3s(2:end,:);
regime_po4_b_3s = mtx_regime_po4_b_3s(2:end,:);

save(['koem_timeseires_monthly_gy_only_1to3sig_v2_8point.mat']); %extract gwangyang-port point
return
