...
% target_year = 2021; %2021to2030
% target_year = 2031; %2031to2040
% target_year = 2041; %2041to2050
% target_year = 2051; %2051to2060
% target_year = 2061; %2061to2070
% target_year = 2071; %2071to2080
% target_year = 2081; %2081to2090
% target_year = 2091; %2091to2100
temp_t=0;
target_year = [2021; 2031; 2041; 2051; 2061; 2071; 2081; 2091;]
for target_year_i = 1:length(target_year)
temp_t = temp_t +1;
for t_year = 3:3 %before regime change
    clearvars -except t_year target_year temp_t target_year_i
grd_file='grid_gy_v11_s.nc';
% Fname='pohang_tide_inout.nc';
Fname =['river_', num2str(t_year), 'th_reg_bio_det_nosew_mixed_daily_2020_delta_plus_', num2str(target_year(target_year_i)),'.nc'];
ss = 1:365;
cycle=ss(end);
...
%from ykang
sj = load('songjung_climate_days_bio_to07to16_high_airT_mixed_daily_2020.mat'); % 3regime climate (1997~2006, 2007~2015, 2016~2019);
ng = load('namgang_climate_days_bio_to07to16_high_airT_mixed_daily_2020.mat');  % 3regime climate (1997~2006, 2007~2015, 2016~2019);
ng_dis=load('D:\장기생태\Dynamic\06_river\data\Namgang_dam\gawha_gate_discharge_and_compare_with_namgang_2020.mat');
%% RCP85 air temp delta + river recon
% from estimation_of_recon_river_water_plus_delta_RCP85.m
% re_3reg_delta_dd
sj_rcp = load('D:\장기생태\Dynamic\06_river_physics\RCP85_delta_plus_3reg_recon_river_temp.mat');  
ng_rcp = load('D:\장기생태\Dynamic\06_river_physics\RCP85_delta_plus_3reg_recon_river_temp_gawha.mat');  
...
elseif t_year == 3
    ...
    temp_out_sumjin=squeeze(sj_rcp.re_3reg_delta_dd(:,temp_t));
    temp_out_gahwa=squeeze(ng_rcp.re_3reg_delta_dd(:,temp_t));
