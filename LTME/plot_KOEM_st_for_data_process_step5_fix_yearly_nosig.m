...
% port
for i = 1:3
name_tag_1{i} = ['¿©¼ö½ÅÇ× H' num2str(i)] 
end

name_tag_2{1} = ['±¤¾çÇ× H' num2str(1)] 

name_tag_3{1} = ['»ïÃµÆ÷Ç× H' num2str(1)] 
...
for i = 1:25
name_tag_5{i} = ['¼¶Áø°­ÇÏ±¸ ' num2str(i,'%02d')] 
end

for i = 1:2
name_tag_6{i} = ['ÁøÁÖ¸¸ ' num2str(i,'%02d')] 
end

%coastal 
for i = 1:28
name_tag_7{i} = ['´ëÇÑÇØÇù¿¬¾È ' num2str(i,'%02d')] 
end
...
% % for i = 1:length(name_tag_7)-18  % ¿¬¾È 3, 6, 13:28 has to be remove (18)  [it's located at out of domain] 
% %  new_i = [1:28]; new_i(3)=[]; new_i(6)=[]; new_i(1)=[];
% %  name_tag{size_tag+i} = name_tag_7{new_i(i)};
% % end

for i = 1:length(name_tag_7) % ¿¬¾È 3, 6, 13:28 has to be remove (18)  [it's located at out of domain] 
 name_tag{size_tag+i} = name_tag_7{i};
end

%% pick the row on the excel which has same name with tag
[raw_p txt_p]=xlsread('ÇØ¾çÈ¯°æÃøÁ¤¸Á(Ç×¸¸ÃøÁ¤¸Á).xls','sheet','');
...
[raw_es txt_es]=xlsread('ÇØ¾çÈ¯°æÃøÁ¤¸Á(ÇÏÃµ¿µÇâ¹×¹ÝÆó¼â¼ºÇØ¿ªÈ¯°æÃøÁ¤¸Á).xls','sheet','');
...
[raw_co txt_co]=xlsread('ÇØ¾çÈ¯°æÃøÁ¤¸Á(¿¬¾ÈÇØ¿ªÈ¯°æÃøÁ¤¸Á).xls','sheet','');
...
%     
%     clearvars idx1 tempo_data
%     tempo_data= nh4_sur_clim(i,:);
%     idx1 = find(isnan(tempo_data) == 0);
%     regime_nh4=tempo_data;
%     regime_nh4(find(tempo_data > mean(tempo_data(idx1)) + sig*std(tempo_data(idx1))))=NaN;
%     regime_nh4(find(tempo_data < mean(tempo_data(idx1)) - sig*std(tempo_data(idx1))))=NaN;
%     mtx_regime_nh4(i,:) = regime_nh4;
%     regm_nh4(i,:) = nanmean(regime_nh4)
%     
% 
%     %bot
%     clearvars idx1 tempo_data
%     tempo_data= do_bot_clim(i,:);
%     idx1 = find(isnan(tempo_data) == 0);
...
clearvars regime_*
regime_do = do_sur_clim;
regime_no3 = no3_sur_clim;
regime_temp = temp_sur_clim;
regime_salt = salt_sur_clim;
regime_nh4 = nh4_sur_clim;
regime_chl = chl_sur_clim;

regime_do_b = do_bot_clim;
regime_no3_b = no3_bot_clim;
regime_temp_b = temp_bot_clim;
regime_salt_b = salt_bot_clim;
regime_nh4_b = nh4_bot_clim;
regime_chl_b = chl_bot_clim;
% 
% save('koem_timeseires_monthly_nosig.mat');
...
save('koem_timeseires_yearly_nosig.mat');
return
...
set(gca,'xlim',[1 22]);
set(gca,'xticklabel',1997:2:2018);
title('KOEM-Ç¥Ãþ¼ö¿Â ¿¬Æò±Õ','fontsize',13)
grid on
set(gca,'fontsize',13)

% yearly mean less than 10deg
less10_indx=find(regime_temp_yr <= 10);
[row col]=ind2sub(size(regime_temp_yr),less10_indx);

save('koem_just_mean_yearly_linear_trend_nosig.mat','-v7.3');
