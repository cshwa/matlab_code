...
for i = 1:length(r_date_txt)
    r_raw_date_c{i,1} = r_date_txt(i,:); % delete year
end
...
for i = 1989:2019
            k=k+1;
    for j = 1:12
        eom_d_89(k,j) = eomday(i,j); % 1980 is leap-yr
    end
end

%make t-axis for yearly 
t_indx_pre=[];
for i = 1:size(eom_d_89,1); t_indx_pre = [t_indx_pre eom_d_89(i,:)]; end;
...
for i = 1:31
    ...
    for n = 1:12
        m = m+1;       
    for j = 1:eom_d_89(i,n)
        l=l+1; % index for 1~31(or 30 or 29)
        k=k+1; % index for 1~366
        mth_raw_txt(k,:)=[num2str(i+1988) '.' num2str(n,'%02d') '.'  num2str(j,'%02d')];
    end
    end
end
...
for i = 1:length(mth_d_txt)
    mth_d_txt_c{i,1} = mth_d_txt(i,:); % delet year
end

for i = 1:length(mth_raw_txt)
    mth_raw_txt_c{i,1} = mth_raw_txt(i,:); % raw
end
...
for i = 1:length(mth_raw_txt_c)
       indx_raw{i} = find([strcmp(mth_raw_txt_c{i}, r_raw_date_c)] == 1);     
end
% r_raw_date_c mth_raw_txt_c 
...
raw_do_04(1:t_tick(end)) = raw_do(1:t_tick(end)); raw_do_04(t_tick(end)+1:end) = NaN;
raw_chl_04(1:t_tick(end)) = raw_chl(1:t_tick(end)); raw_chl_04(t_tick(end)+1:end) = NaN;
raw_nh4_04(1:t_tick(end)) = raw_nh4(1:t_tick(end)); raw_nh4_04(t_tick(end)+1:end) = NaN;
raw_no3_04(1:t_tick(end)) = raw_no3(1:t_tick(end)); raw_no3_04(t_tick(end)+1:end) = NaN;

% make quasi_climate from 1989 to 2004
for i = 1:length(mth_d_txt_c)
    if size(indx{i},1) == 0
        w_do_04(i) = NaN;
        w_chl_04(i) = NaN; 
        ...
        w_do_04(i) = nanmean(raw_do_04(indx{i}));
        w_chl_04(i) = nanmean(raw_chl_04(indx{i}));
        w_no3_04(i) = nanmean(raw_nh4_04(indx{i}));
        w_nh4_04(i) = nanmean(raw_no3_04(indx{i}));
    end
end
