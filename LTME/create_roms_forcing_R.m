function create_roms_forcing_R(fname,rain,time,yy,cycle)%,tframe,cycle)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
nw{var_time}(:)=time';
 ...
 nw{'rain'} = ncfloat(var_time, 'eta_rho', 'xi_rho');
 nw{'rain'}.long_name = ncchar('ECMWF Total precipitation');
 nw{'rain'}.units = ncchar('kg m-2 s-1');
 nw{'rain'}.time = ncchar(var_time);
for i=1:1:yy
 nw{'rain'}(i,:,:)= rain(i,:,:);
end
 close(nw);
