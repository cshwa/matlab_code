function varargout = get_oceanwind(varargin)
...
%
% REVISION HISTORY:
% 2009/10/03 rewritten from scratch, ms
%==========================================================================
...
if 0 % dsi_flag==1    % disable do not update
...
% browse through time
iTime=[];
for j=R.iTime1:DSI.TimesPerDay*R.DATEINCR:R.iTime2;
    for jj=R.DailyTimes
    iTime(end+1)=j+jj-1;
    end
end

%assignin('base','iTime',iTime);
%assignin('base','R',R);
if(isempty(iTime)) 
...
URLFILE=DSI.URLFILE; if strfind(URLFILE,'URLFILE=') eval(URLFILE); end 
    if isfield(DSI,'PASS')
        PASS=DSI.PASS; if strfind(PASS,'PASS=') eval(PASS); end
    end
    if isfield(DSI,'CVAR')
        CVAR=DSI.CVAR; if strfind(CVAR,'CVAR=') eval(CVAR); end
    else
    CVAR=DSI.Fields_NameDS{j};
    end
    if isfield(DSI,'FIELDR')
        FIELDR=DSI.FIELDR; if strfind(FIELDR,'FIELDR=') eval(FIELDR); end
    else
    FIELDR=DSI.Fields_NameReturned{j};
    end
    if isfield(DSI,'FIELDDS')
        FIELDDS=DSI.FIELDDS; if strfind(FIELDDS,'FIELDDS=') eval(FIELDDS); end
    else
    FIELDDS=DSI.Fields_NameDS{j};
    end
    
CTIME=op_smarturl(DSI.CTIME,FIELD,kT); if strfind(CTIME,'CTIME=') eval(CTIME); end 

    if isfield(DSI,'LOADDAPURL')
        LOADDAPURL=DSI.LOADDAPURL; if strfind(LOADDAPURL,'LOADDAPURL=') eval(LOADDAPURL); end
    else
        LOADDAPURL=[URLSITE URLPATH URLFILE '?' CVAR CTIME R.CLAT R.CLON];
    end
