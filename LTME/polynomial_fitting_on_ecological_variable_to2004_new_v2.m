...
clearvars xp_w_do pf_w_do
xp = 1:366;
xp_w_do = find(isnan(reg_af_clim_do)==0);
pf_w_do = polyfit(xp_w_do,reg_af_clim_do(xp_w_do),3);
...
clearvars xp_w_do pf_w_do
xp = 1:366;
xp_w_chl = find(isnan(reg_af_clim_chl)==0);
pf_w_chl = polyfit(xp_w_chl,reg_af_clim_chl(xp_w_chl),3);
...
clearvars xp_w_do pf_w_do
xp = 1:366;
xp_w_no3 = find(isnan(reg_af_clim_no3)==0);
pf_w_no3 = polyfit(xp_w_no3,reg_af_clim_no3(xp_w_no3),3);
...
clearvars xp_w_do pf_w_do
xp = 1:366;
xp_w_nh4 = find(isnan(reg_af_clim_nh4)==0);
pf_w_nh4 = polyfit(xp_w_nh4,reg_af_clim_nh4(xp_w_nh4),3);
...
for i = 1:22
    if i ==1
         t_tick_nh4(i) = t_tick_pre(i);
    else
        t_tick_nh4(i) = sum(t_tick_pre(1:i));
    end
end
t_tick_nh4 = [1 t_tick_nh4];
...
regime_no3_af = raw_no3(t_tick(end)+1:end);
% 2004 ~ 2011
regime_nh4_af = raw_nh4(t_tick_nh4(16)+1:t_tick_nh4(end));
% 2011 ~ 2019
regime_nh4_af_2 = raw_nh4(t_tick_nh4(end)+1:end);
...
% NH4 divide the date
j=0;
for i = t_tick(end)+1:t_tick(end)+length(regime_nh4_af)
    j=j+1;
regime_date_af_nh4_c{j} = r_raw_date_c{i}; %  2004 ~ 2011
end

% NH4 divide the date
j=0;
for i = t_tick(end)+length(regime_nh4_af)+1:length(raw_do)
    j=j+1;
regime_date_af2_nh4_c{j} = r_raw_date_c{i}; %  2011 ~ 2019
end
...
% pick matched date from water temp date (NH4)
for i = 1:length(mth_d_txt_c)
       regime_indx_af_nh4{i} = find([strcmp(mth_d_txt_c{i}, regime_date_af_nh4_c)] == 1)
end

for i = 1:length(mth_d_txt_c)
       regime_indx_af_nh4_2{i} = find([strcmp(mth_d_txt_c{i}, regime_date_af2_nh4_c)] == 1)
end
...
% NH4
size_c = [];
for i = 1:366; size_c(i)=length(regime_indx_af_nh4{i}); end
bar(size_c); nansum(size_c)

size_c = [];
for i = 1:366; size_c(i)=length(regime_indx_af_nh4_2{i}); end
bar(size_c); nansum(size_c)
...
% make quasi_climate 2004~2011 (NH4)
for i = 1:length(regime_indx_af_nh4)
    if size(regime_indx_af_nh4{i},1) == 0     
        reg_af_clim_nh4(i) = NaN; 
    else
        reg_af_clim_nh4(i) = nanmean(regime_nh4_af(regime_indx_af_nh4{i}));
    end
end

% make quasi_climate 2011~2019 (NH4)
for i = 1:length(regime_indx_af_nh4_2)
    if size(regime_indx_af_nh4_2{i},1) == 0     
        reg_af_clim_nh4_2(i) = NaN; 
    else
        reg_af_clim_nh4_2(i) = nanmean(regime_nh4_af_2(regime_indx_af_nh4_2{i}));
    end
end
...
clearvars xp_w_do pf_w_do
xp = 1:366;
xp_w_nh4 = find(isnan(reg_af_clim_nh4_2)==0);
pf_w_nh4 = polyfit(xp_w_nh4,reg_af_clim_nh4_2(xp_w_nh4),3);
yp_w_nh4_af_2 = polyval(pf_w_nh4,xp);
...
%% nh4
figure;
scatter(1:366,reg_af_clim_nh4_2);
hold on
plot(1:366, yp_w_nh4_af_2,'--','color','r','linew',2)
xlabel('time(days)','fontsize',13)
ylabel('NH4 (mg/L)','fontsize',13)
title('Namgang-polynomial nh4.','fontsize',13)
grid on
...
save('Namgang_polynomial_climate_to2004_NH4.mat'); 
