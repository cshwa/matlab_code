...
grdname=['grid_sumjin_v1970_fix_3m_v3.nc']; % 저장될 grid 이름
topofile='depth_sumjin_v1970_csh2.nc';
% silver='grid_gy_v11_s_0613_depth.nc';
...
load merge_1970_depth_gjb_fix_v2.mat
z=h_merge_1;
z(z<0)=0; % land(z<0) to be zero
z(isnan(z)==1)=0;
nolyang=find(lon_rho  >= 127.85 & lon_rho  <= 127.95 & lat_rho >= 34.94 & lat_rho <= 34.96);
silver_h=ncread('grid_gy_v11_s_0613_depth.nc','h');  %sliver's depth.
silver_h=silver_h';
...
maskr=ncread('grid_sumjin_v1970.nc','mask_rho')';
...
m_grid('box','fancy',...
       ...
       'fontsize',7);
hold off
shading flat; caxis([1 50]);colorbar
%
% End
%
