...
%           num2str(year,'%04i'),num2str(4,'%02i'),'/',filename);
    ...
    %     testname = 'nwp_1_20';
        ...
        time=[tinterval/24:tinterval/24:totalday];
        ...
        switch varname
            ...
            case 'total_precipitation'
                varname_input = 'tp';
                varname_output = 'rain';
                long_name = 'ECMWF ERA5 total precipitation';
                units = 'kgm^-2s^-1';
                time=[0.5:1:totalday-0.5];
                tinterval=24;
            case 'evaporation'
                varname_input = 'e';
                varname_output = 'evaporation';
                long_name = 'ECMWF ERA5 potential evaporation';
                units = 'kgm^-2s^-1';
                time=[0.5:1:totalday-0.5];
                tinterval=24;
        ...
        lon_grd_max = max(max(lon_rho));
        lat_grd_max = max(max(lat_rho));
        lon_grd_min = min(min(lon_rho));
        lat_grd_min = min(min(lat_rho));
        ...
        switch varname
            ...
            case 'total_precipitation'
                raw_data = raw_data*1000/3600;
            case 'evaporation'
                raw_data = raw_data*1000/3600;
            case 'surface_solar_radiation_downwards'
                raw_data =raw_data/3600;
        end

        size_data=size(raw_data);
%         data_daily = ones(size_data(1),size_data(2),totalday)*NaN;
        for dayij = 1: monthday(monthij)*(24/tinterval)
            switch varname
                case 'surface_solar_radiation_downwards'  % time begins from 1/1 07:00. ~ 1/31 23:00
                    data_daily(:,:,dayind)=mean(raw_data(:,:,(dayij-1)*24+1:dayij*24),3); %average of (each accumulated hourly J/m^2 --> W/m^2)
                case 'total_precipitation'
                    data_daily(:,:,dayind)=mean(raw_data(:,:,(dayij-1)*24+1:dayij*24),3); %average of (each accumulated hourly m --> kg/m2s)
                case 'evaporation'
                    data_daily(:,:,dayind)=mean(raw_data(:,:,(dayij-1)*24+1:dayij*24),3); %average of (each accumulated hourly m --> kg/m2s)
                otherwise % time begins from 1/1 0:00. ~ 1/31 23:00
                    data_daily(:,:,dayind)=mean(raw_data(:,:,(dayij-1)*tinterval+1:dayij*tinterval),3); %actually, it's hourly data yet
            end
            dayind=dayind+1;
        end
    end
...
%     end  
    ...
    for i=1:length(time)
        ...
        Z=griddata(double(lon_ECMWF),double(lat_ECMWF),Zi',lon_rho',lat_rho'); %% [x y v'(y,x) xv yv]
