...
% % x1=lon_min; x2=lon_max; y1=lat_min; y2=lat_max;
    ...
    if (lon2-lon1) >= (lat2-lat1)
        lon_line = cut_lon_rho(min(x1(1),x2(1)):max(x1(1),x2(1)),min(y1(1),y2(1)):max(y1(1),y2(1)));  %% for 1/20^o horizontal resolution
...
%         Temp=zeros(gd.N,length(lon_line)); %% initialize temp matrix that size is same with x
        ...
        lat_line=cut_lat_rho(min(x1(1),x2(1)):max(x1(1),x2(1)),min(y1(1),y2(1)):max(y1(1),y2(1)));
